export const locales = ["en", "ar"] as const;
export type Locale = (typeof locales)[number];
export const defaultLocale: Locale = "en";

export function isRtl(locale: Locale): boolean {
  return locale === "ar";
}

const en = {
  // chrome / nav
  signOut: "Sign out",
  settings: "Settings",
  settingsTitle: "Settings",
  settingsIntro: "Your account and (if you're the owner) your garage details.",
  settingsSecProfile: "Profile",
  settingsSecProfileHint: "Your name as it appears to colleagues and customers.",
  settingsProfileName: "Name",
  settingsProfileEmail: "Email (sign-in)",
  settingsProfileEmailWarning: "You'll sign in with the new email from now on. Make sure it's right.",
  settingsProfileEmailCurrent: "Current password (required to change email)",
  settingsProfileSave: "Save",
  settingsProfileSaveName: "Save name",
  settingsProfileSaveEmail: "Save email",
  settingsSecPassword: "Password",
  settingsSecPasswordHint: "Change the password you use to sign in.",
  settingsSecTeam: "Team",
  settingsSecTeamHint: "Add or remove staff and assign branches.",
  settingsSecTeamGo: "Manage team",
  // Garage details editor (AR 2026-08-19, Batch B) — name / TRN /
  // address / default language. Prints on tax invoices; UAE FTA
  // Art. 59 requires supplier name + TRN + address.
  settingsSecGarageDetails: "Garage details",
  settingsSecGarageDetailsHint: "Your shop's identity on every tax invoice and estimate. UAE VAT rules require name, TRN and address on tax invoices.",
  settingsGarageNameLabel: "Shop name",
  settingsGarageTrnLabel: "Tax Registration Number (TRN)",
  settingsGarageTrnHint: "15 digits from the FTA. Required for B2B invoices; the customer needs it to reclaim VAT.",
  settingsGarageAddressLabel: "Address",
  settingsGarageAddressHint: "P.O. Box, building, street, area, city. Prints on tax invoices — the FTA requires it.",
  settingsGarageDefaultLangLabel: "Default customer language",
  settingsGarageDefaultLangHint: "Used only when we can't tell a customer's language yet (brand-new customer, no inbound message). Once a customer writes to you, we detect from their message.",
  settingsGarageDefaultLangUnset: "Not set — falls through to English",
  settingsGarageDefaultLangEn: "English",
  settingsGarageDefaultLangAr: "Arabic",
  settingsGarageCountryLabel: "Country",
  settingsGarageCountryValueUae: "United Arab Emirates",
  settingsGarageCountryLocked: "Country change re-derives VAT strategy and currency; contact support to move a shop.",
  settingsGarageVatLabel: "VAT rate",
  settingsGarageVatValueUae: "5% (UAE FTA)",
  settingsGarageVatLocked: "Fixed by UAE VAT law; not editable.",
  settingsOkGarageName: "Shop name saved.",
  settingsOkGarageTrn: "TRN saved.",
  settingsOkGarageAddress: "Address saved.",
  settingsOkGarageDefaultLang: "Default customer language saved.",
  // Batch F1 (2026-08-25) — shop main phone.
  settingsGaragePhoneLabel: "Shop contact number",
  settingsGaragePhonePlaceholder: "+971 50 000 0000",
  settingsGaragePhoneHint: "Prints on estimates and invoices as the callback number for the assigned advisor. If an advisor has no personal number set, the shop number is used.",
  settingsOkGaragePhone: "Shop contact number saved.",
  settingsErrPhoneTooLong: "Contact number is too long (max 40 characters).",
  settingsErrGarageNameRequired: "Shop name can't be empty.",
  settingsErrGarageNameTooLong: "Shop name is too long (max 80 characters).",
  settingsErrTrnTooLong: "TRN is too long (max 40 characters).",
  settingsErrAddressTooLong: "Address is too long (max 400 characters).",
  settingsErrGarageLangInvalid: "Choose English, Arabic, or leave unset.",
  // Garage logo (Phase C).
  settingsSecGarageLogo: "Garage logo",
  settingsSecGarageLogoHint: "Appears on customer invoices, estimates, and at the top of every dashboard. PNG, JPEG or WEBP, up to 500 KB.",
  settingsLogoPick: "Choose a file",
  settingsLogoPicked: "Picked",
  settingsLogoReplace: "Choose a different file",
  settingsLogoUpload: "Upload logo",
  settingsLogoUploading: "Uploading…",
  settingsLogoRemove: "Remove logo",
  settingsOkLogoSaved: "Logo saved.",
  settingsOkLogoRemoved: "Logo removed — using the default brand.",
  // Pricing defaults (AR 2026-08-12) — shop-wide markup hint used by
  // the advisor's cost-based line editor at line-create time. Internal;
  // never printed on customer surfaces.
  settingsSecPricing: "Pricing defaults",
  settingsSecPricingHint: "Shop-wide starting values for the advisor's estimate line editor. Advisors can override per line.",
  settingsPricingMarkupLabel: "Default parts markup",
  settingsPricingMarkupPlaceholder: "40",
  settingsPricingMarkupHint: "Leave blank to require the advisor to enter markup on every part.",
  settingsSave: "Save",
  settingsOkMarkup: "Default markup saved.",
  settingsErrRole: "Only the owner can change this.",
  settingsErrMarkupInvalid: "Markup must be a number.",
  settingsErrMarkupRange: "Markup must be between 0 and 999.99 %.",
  // Labour hourly cost (AR 2026-08-12, profit reporting Phase 1).
  settingsPricingLaborCostLabel: "Labour hourly cost",
  settingsPricingLaborCostPlaceholder: "60",
  settingsPricingLaborCostHint: "Leave blank to skip labour profit — the report will show it as unknown, not zero.",
  settingsOkLaborCost: "Labour hourly cost saved.",
  settingsErrLaborCostInvalid: "Labour hourly cost must be a number.",
  settingsErrLaborCostRange: "Labour hourly cost must be zero or more.",
  settingsErrLogoMissing: "No file was attached.",
  settingsErrLogoEmpty: "The selected file is empty.",
  settingsErrLogoTooLarge: "Logo must be {limit} KB or smaller.",
  settingsErrLogoBadMime: "Logo must be a PNG, JPEG, or WEBP image.",
  profitCardTitle: "Profit (staff view)",
  profitCardRevenue: "Revenue",
  profitCardCost: "Cost",
  profitCardGross: "Gross profit",
  profitCardMargin: "Margin",
  // Markup label on the parts block (AR 2026-08-16). Shown next to
  // margin so the owner can compare directly against the "default
  // parts markup %" setting on /settings. Same profit, different
  // denominator (cost, not revenue).
  profitCardMarkup: "Markup",
  profitCardParts: "Parts",
  profitCardLabour: "Labour",
  profitCardHeadlineIncomplete:
    "Cost and profit can't be calculated — some lines have no cost data. See the coverage lines below.",
  profitCardCoverageParts: "{covered} of {total} parts have cost data",
  profitCardCoverageLabour: "{covered} of {total} work sessions have cost data",
  // Direct-fit receipt coverage (AR 2026-08-16, rewritten same day
  // after INV-2026-0048 review). Two categories of receipt signal;
  // NEITHER suppresses the margin — they're warnings. See
  // docs/direct-fit-receive-spec.md.
  //
  // Mismatch: invoice HAS cost, but a later receipt shows the shop
  // actually paid something different. Delta tells the owner by how
  // much the invoiced cost may be stale.
  //   {count}     — # of mismatched receipts
  //   {direction} — "understating" or "overstating" (from
  //                 profitCardCoverageReceiptsDirectionUnder/Over)
  //   {delta}     — absolute AED delta, formatted by money()
  profitCardCoverageReceiptsMismatch:
    "{count} receipt(s) show actual paid costs differ from what was invoiced — invoiced cost may be {direction} parts cost by {delta}.",
  profitCardCoverageReceiptsDirectionUnder: "overstating",
  profitCardCoverageReceiptsDirectionOver: "understating",
  // Unlinkable: receipt exists on this job but can't be checked
  // against an invoice line (manual PO, source line without
  // unitCost, or estimate not invoiced). Lighter tone.
  profitCardCoverageReceiptsUnlinkable:
    "{count} direct-fit receipt(s) not linked to an invoice line — verify the invoiced cost matches what you paid.",
  profitCardNoLabourSessions: "No work sessions recorded on this job.",
  profitCardLabourRateMissing: "Labour cost is unknown — labour rate not set.",
  profitCardLabourRateMissingCta: "Set labour rate",
  profitCardNotePartsCost:
    "Margins use weighted-average part cost, not per-unit cost.",
  profitCardNoteLabourCost:
    "Labour cost is the configured hourly rate applied to recorded work time — not cash out the door.",
  // AR 2026-08-16 — explain the two figures on the parts block.
  // A 15% markup on cost renders as a ~13% margin on revenue; both
  // are correct, they measure the same profit against different
  // denominators.
  profitCardNoteMarginVsMarkup:
    "Margin is profit as a % of selling price; markup is profit as a % of cost. A 15% markup shows as a ~13% margin.",
  profitCardUnknown: "unknown",
  settingsErrLogoBadMagic: "That file doesn't look like a real image.",
  settingsErrLogoMimeMismatch: "The file's content doesn't match its declared type.",
  settingsOkName: "Name updated.",
  settingsOkEmail: "Email updated. You'll use the new email next time you sign in.",
  settingsOkEmailUnchanged: "Email is already set to that — nothing to do.",
  settingsErrNameRequired: "Name is required.",
  settingsErrNameTooLong: "Name is too long (max 80 characters).",
  settingsErrEmailMissing: "Email and current password are both required.",
  settingsErrEmailInvalid: "That doesn't look like a valid email address.",
  settingsErrEmailCurrentWrong: "Current password is incorrect.",
  settingsErrEmailTaken: "That email is already in use.",
  settingsErrEmailNoPassword: "Your account has no password set yet — set one first, then change your email.",
  changePassword: "Change password",
  changePasswordTitle: "Change password",
  changePasswordIntro: "Update the password you use to sign in. You stay signed in on this device; the new password is required the next time you sign in.",
  currentPassword: "Current password",
  newPasswordLabel: "New password (min 6)",
  confirmNewPassword: "Confirm new password",
  changePasswordSubmit: "Change password",
  passwordChanged: "Password changed.",
  pwdErrCurrent: "Current password is incorrect.",
  pwdErrShort: "New password must be at least 6 characters.",
  pwdErrMismatch: "New password and confirmation don't match.",
  pwdErrMissing: "All fields are required.",
  roleOWNER: "Owner",
  roleADVISOR: "Service Advisor",
  roleTECH: "Technician",
  roleCASHIER: "Cashier",
  roleMASTER: "Master Admin",
  tabDashboard: "Dashboard",
  tabTeam: "Team",
  tabJobs: "Jobs",
  tabIntake: "Intake",
  tabEstimates: "Estimates",
  advisorEstimatesTitle: "Estimates",
  advisorEstimatesIntro: "Estimates you've priced or are still pricing — handed off to the cashier for invoicing once the customer approves.",
  advisorEstSecJobsToPrice: "Jobs to price",
  advisorEstSecJobsToPriceHint: "Technician handed these off — start a draft estimate.",
  advisorEstSecDrafts: "Drafts",
  advisorEstSecDraftsHint: "In progress — finish pricing and send to the customer.",
  advisorEstSecSent: "Sent",
  advisorEstSecSentHint: "Awaiting customer decision over WhatsApp.",
  advisorEstSecApproved: "Approved",
  advisorEstSecApprovedHint: "Customer approved — cashier will invoice once the work is complete.",
  advisorEstSecRejected: "Rejected",
  advisorEstSecRejectedHint: "Customer rejected — re-price with a new draft.",
  advisorEstEmpty: "Nothing here.",
  tabBookings: "Bookings",
  tabWorkshop: "Workshop",
  tabAccounts: "Accounts",
  tabInventory: "Inventory",
  tabSuppliers: "Suppliers",
  tabPurchasing: "Purchasing",
  tabBilling: "Billing",
  // inventory — parts catalog (Phase 1)
  inventoryTitle: "Parts inventory",
  inventorySubtitle: "Your parts catalog and stock levels.",
  addPart: "Add part",
  partSku: "Part number",
  partName: "Description",
  partCost: "Cost",
  partPrice: "Price",
  partStock: "In stock",
  partReorderLevel: "Reorder at",
  openingStock: "Opening stock",
  noPartsYet: "No parts yet. Add your first part above.",
  lowStockTag: "Low",
  partAutoBadge: "Auto",
  savePart: "Save part",
  // inventory 1b — edit part + manual stock adjustment
  editPart: "Edit part",
  saveChanges: "Save changes",
  save: "Save",
  backToInventory: "← Inventory",
  adjustStock: "Adjust stock",
  currentStock: "Current stock",
  adjustDirection: "Add or remove",
  adjustAdd: "Add",
  adjustRemove: "Remove",
  adjustQty: "Quantity",
  adjustReason: "Reason",
  adjustReasonHint: "e.g. received, damaged, correction",
  applyAdjustment: "Apply adjustment",
  recentMovements: "Recent stock changes",
  noMovements: "No stock changes recorded yet.",
  movementWhen: "When",
  movementChange: "Change",
  // inventory OCR import — photograph a supplier invoice → review → catalog
  importFromPhoto: "Import from invoice photo",
  importFromPhotoHint: "Snap a supplier invoice — we read the lines, you check them before saving.",
  importScan: "Scan invoice",
  importReviewTitle: "Review imported parts",
  importDetected: "lines detected",
  importFlagged: "need a check",
  // AR 2026-08-22 Batch 8 — pluralize()-compatible variants.
  // Same 6 CLDR categories on the English side as the Arabic block so
  // the MessageKey type stays consistent across locales; English only
  // ever picks _one or _other at runtime (Intl.PluralRules("en")
  // never selects zero/two/few/many).
  importDetected_zero: "{n} lines detected",
  importDetected_one: "{n} line detected",
  importDetected_two: "{n} lines detected",
  importDetected_few: "{n} lines detected",
  importDetected_many: "{n} lines detected",
  importDetected_other: "{n} lines detected",
  importFlagged_zero: "{n} need a check",
  importFlagged_one: "{n} needs a check",
  importFlagged_two: "{n} need a check",
  importFlagged_few: "{n} need a check",
  importFlagged_many: "{n} need a check",
  importFlagged_other: "{n} need a check",
  importFlaggedHint: "OCR wasn't confident about this line — please verify.",
  importInclude: "Add",
  importPartNo: "Part number",
  importDescription: "Description",
  importNamePlaceholder: "Description",
  importSkuAuto: "auto",
  importConfirm: "Confirm & save to catalog",
  importConfirmHint: "Only checked rows are saved. Nothing was saved until now.",
  importDiscard: "Discard this import",
  importNoLines: "No lines were read from this photo. Discard it and add parts manually, or try a clearer photo.",
  importSourcePhoto: "Source photo",
  importCreated: "Imported {n} parts into the catalog.",
  importSkipped: "Skipped {n} (blank name or duplicate SKU).",
  // singular forms (n === 1) — never show "1 parts" / "1 lines" / "1 need"
  importDetectedOne: "line detected",
  importFlaggedOne: "needs a check",
  importCreatedOne: "Imported 1 part into the catalog.",
  importStatus_CONFIRMED: "This import was confirmed — the parts are in your catalog.",
  importStatus_DISCARDED: "This import was discarded.",
  // inventory 1c — supplier directory
  suppliersTitle: "Suppliers",
  suppliersSubtitle: "Who you buy parts from.",
  supplierName: "Supplier name",
  supplierContact: "Contact person",
  supplierPhone: "Phone",
  supplierEmail: "Email",
  supplierTrn: "TRN",
  supplierAddress: "Address",
  supplierParts: "Parts",
  saveSupplier: "Save supplier",
  noSuppliersYet: "No suppliers yet. Add your first supplier above.",
  suppliersInactive: "Removed",
  backToSuppliers: "← Suppliers",
  editSupplier: "Edit supplier",
  supplierInactiveTag: "Removed",
  supplierPartsLinked: "parts linked",
  deactivateSupplier: "Remove supplier",
  deactivateSupplierHint: "Removes it from your lists. History is kept, and you can restore it anytime.",
  restoreSupplier: "Restore supplier",
  restoreSupplierHint: "Make this supplier active again.",
  deactivate: "Remove",
  partSupplier: "Supplier",
  partSupplierNone: "— none —",
  // inventory 1d — dashboard low-stock reorder shortlist
  lowStockHeading: "Low stock",
  viewInventory: "View inventory",
  lowStockNone: "All parts are above their reorder level.",
  moreLowStock: "more below reorder",
  // inventory 2a — purchasing (purchase orders)
  purchasingTitle: "Purchasing",
  documentPurchaseOrder: "Purchase order",
  documentPurchaseOrderDraft: "Purchase Order (draft)",
  documentRfq: "Request for Quotation",
  printPo: "Print",
  sendViaWhatsApp: "Send via WhatsApp",
  supplierNoPhoneReason: "Supplier phone number is missing — add it on the supplier record to enable WhatsApp.",
  // Sent history — one row per send attempt on a PO, newest first.
  // See src/components/PoSentHistory.tsx. Copy is deliberately honest
  // about the difference between "we handed it to WhatsApp on your
  // phone" (we DON'T know what happened next) and "we sent it via
  // email" (we know Resend accepted the message).
  sendHistoryTitle: "Sent history",
  sendHistoryEmpty: "No send history yet.",
  sendChannel_WHATSAPP: "WhatsApp",
  sendChannel_EMAIL: "Email",
  sendDocChip_PO: "PO",
  sendDocChip_RFQ: "RFQ",
  sendStatus_HANDED_OFF_whatsapp: "Opened in WhatsApp — delivery not confirmed",
  sendStatus_HANDED_OFF_email: "Handed to email provider — delivery not confirmed",
  sendStatus_SENT_email: "Sent by email",
  sendStatus_FAILED: "Failed",
  emailSentOk: "Sent by email.",
  emailSentError: "Email failed to send.",
  // Whitelisted failure codes surfaced from the send action. The URL
  // and DOM never carry the provider's own error string — real error
  // text is logged server-side only. Unknown code → emailErr_generic.
  emailErr_generic: "Try again in a moment. If it keeps failing, contact support.",
  emailErr_no_recipient: "This supplier has no email address on file — add one on the supplier record.",
  emailErr_unverified_domain: "Sender domain not verified yet. Ask your admin to complete the domain verification.",
  emailErr_not_configured: "Email sending isn't set up on this shop yet.",
  emailErr_provider_error: "Email provider returned an error. It was logged; try again in a moment.",
  printedOn: "Printed on",
  printSupplierContact: "Supplier contact",
  printTotalsLabel: "Totals",
  purchasingSubtitle: "Purchase orders to your suppliers.",
  newPurchaseOrder: "New purchase order",
  newQuotation: "New quotation",
  // Copy for the two-mode purchasing entry. Both buttons land on the
  // same shell form (/owner/purchasing/new?mode=…) then flow into the
  // detail page. Server behavior is identical — both create a DRAFT
  // PO — but the UX asks different things:
  //   quote  — cost blank is fine; we're asking the supplier
  //   order  — cost required; we're placing an order from a known price
  // Landing on DRAFT (per AR's rule 2026-08-02): the user reviews +
  // clicks Mark Ordered themselves. No auto-stamp, so a typo doesn't
  // force cancel-and-restart on a document that hasn't been sent.
  newQuotationHint: "Ask a supplier to price parts we haven't quoted yet.",
  newPurchaseOrderHint:
    "Place an order at prices you already have — for a regular supplier on a known price list.",
  createQuotation: "Create quotation",
  createPurchaseOrder: "Create purchase order",
  orderModeCostRequired:
    "This is a purchase order — enter the price from your supplier's list.",
  purchasingOpen: "Open",
  purchasingClosed: "Closed",
  noPurchaseOrders: "No purchase orders yet.",
  // Stale-write banners on the PO detail page. editPoLineAction
  // narrows its UPDATE by the row's `updatedAt`; if another tab has
  // saved since this form loaded, the count is 0 and the action
  // redirects with one of these codes. The table below the banner
  // re-renders from the DB, so the fresh values are visible on the
  // same page.
  poLineStaleError: "Someone else changed this line while you had it open. The row below shows the current values — re-apply your edit if you still want to change it.",
  poLineNotFoundError: "This line was removed while you had it open. Reload the page to see the current lines.",
  notFoundTitle: "Not found",
  notFoundBody: "This page doesn’t exist or the link has expired.",
  notFoundGoHome: "Go home",
  // Purchasing list — new columns + filter pills, added with the
  // widened list view. sendDocChip_PO / sendDocChip_RFQ / sendChannel_*
  // are reused from the send-audit work.
  poDocNumberCol: "Document #",
  poKindCol: "Kind",
  poVehicleCol: "Vehicle / Job",
  poCreatedCol: "Created",
  poLastSentCol: "Last sent",
  poVehicleMultiple: "{count} vehicles",
  poNotSent: "Not sent",
  poWaitingForQuote: "Waiting {days}d for quote",
  purchasingFilterKindLabel: "Kind:",
  purchasingFilterKindAll: "All",
  purchasingFilterKindPO: "PO",
  purchasingFilterKindRFQ: "RFQ",
  purchasingFilterSentLabel: "Sent:",
  purchasingFilterSentAll: "All",
  purchasingFilterSentSent: "Sent",
  purchasingFilterSentUnsent: "Not sent",
  poSupplier: "Supplier",
  poReference: "Reference",
  supplierRef: "Supplier ref",
  poStatus: "Status",
  poLines: "Lines",
  poTotal: "Total",
  poUnitCost: "Unit cost",
  poLineTotal: "Line total",
  poNote: "Note",
  poStatus_DRAFT: "Draft",
  poStatus_ORDERED: "Ordered",
  poStatus_PARTIALLY_RECEIVED: "Partly received",
  poStatus_RECEIVED: "Received",
  poStatus_CANCELLED: "Cancelled",
  // Display-only labels for a DRAFT that has already been sent to
  // the supplier (AR 2026-08-16). Underlying status stays DRAFT —
  // Mark Ordered is still the commitment click. See
  // poStatusDisplayKey in src/lib/po-doc-kind.ts.
  poStatusDraftSent_RFQ: "Sent — awaiting quote",
  poStatusDraftSent_PO: "Sent — awaiting order",
  // Purchasing-page tab labels for the DRAFT split (AR 2026-08-17).
  // The previous single "Draft" tab lumped never-sent RFQs with
  // sent-and-awaiting-supplier RFQs; the split gives the operator
  // an at-a-glance answer to "what needs my action?". Wording is
  // action-oriented — parallels the invoice-side "handed off" copy
  // (both docs read the same when the hand-off boundary matters).
  poTab_UNSENT_DRAFT: "To send",
  poTab_AWAITING_SUPPLIER: "Awaiting supplier",
  backToPurchasing: "← Purchasing",
  noSuppliersForPo: "Add a supplier first, in",
  choosePlaceholder: "Choose…",
  createDraftPo: "Create draft order",
  // Estimate → PO conversion (docs/Estimate-to-PO-Spec.md)
  convertFromEstimate: "Convert from estimate",
  convertFromEstimateTitle: "Convert estimate to purchase order",
  convertFromEstimateHint: "Look up a job by number, tick which parts to order, pick a supplier.",
  jobCardNumberLabel: "Job card number",
  jobCardNumberPlaceholder: "e.g. 42 or JC-2026-0042",
  lookupJob: "Look up",
  jobCardNotFound: "Job card not found in this shop.",
  jobConfirmHeader: "Job",
  estimateNone: "This job has no estimate yet. Ask the advisor to price it first.",
  estimateAllRejected: "This job has {count} estimate(s), but all were rejected by the customer.",
  estimateMultipleHeader: "Multiple estimates on this job — pick one to convert:",
  estimateLinesCount: "{count} line(s)",
  estimateUseThis: "Use this →",
  estimateStatus_APPROVED: "APPROVED",
  estimateStatus_SENT: "SENT",
  estimateStatus_DRAFT: "DRAFT",
  estimateStatus_REJECTED: "REJECTED",
  estimateChosenHeader: "Estimate used",
  estimateChosenApproved: "Approved {date}",
  estimateChosenSent: "Sent {date}",
  estimateChosenDraft: "Draft, last edited {date}",
  partsToConvertHeader: "Parts to include",
  partsToConvertHint: "Cost prefills from your inventory when known, otherwise from the estimate — the label under each cost shows which. Customer-facing prices are never used. Edit if the supplier's number differs.",
  noConvertibleParts: "This estimate has no parts that can be converted.",
  skippedNoInventoryHeader: "{count} part(s) skipped — no inventory link",
  skippedNoInventoryHint: "add to inventory first",
  goToInventory: "Go to inventory →",
  // Replaces the "Add these to inventory" panel (2026-07-31 removal).
  // Free-text estimate lines that aren't linked to a catalogue Part are
  // now listed read-only — the owner links or creates the Part in
  // /owner/inventory first, then re-visits the estimate.
  skippedFreeTextHeader:
      "{count} free-text part(s) — not in your catalogue",
  skippedFreeTextHint:
      "These lines have no linked Part, so they can't go on a PO. Create the Part in inventory first (or edit the estimate line to link an existing one), then convert this estimate again.",
  colSku: "SKU",
  colName: "Name",
  colCost: "Cost",
  colPrice: "Price",
  skippedDeclinedHeader: "{count} part(s) declined by the customer",
  createDraftPoFromEstimate: "Create draft PO",
  createRfqFromEstimate: "Create request for quotation",
  // Rendered under the "Create purchase order" button on
  // /owner/purchasing/from-estimate when any included line is
  // unpriced. `{n}` is substituted client-side with the count.
  poRequiresAllPricedReason:
    "Purchase order needs every included line priced — {n} still without a cost. Type a cost, or click Create quotation to send a request instead.",
  lineUnpricedTag: "quote please",
  approxTotal: "Approx. total",
  // Cost-source provenance on /owner/purchasing/from-estimate.
  // Rendered as a small caption under each cost input so the owner
  // sees where the prefill came from — the catalogue (what the shop
  // has actually paid, rolling weighted-avg) or the estimate (what
  // the advisor typed as the expected supplier cost). Different
  // levels of confidence; the label makes that visible.
  costSourceCatalogue: "from catalogue",
  costSourceEstimate: "from estimate",
  addPoLine: "Add line",
  addPoLinePlaceholder: "Part name, or type free text",
  addPoLineHint:
    "Pick from your catalogue, or type free text for a part you don't stock.",
  // Doc-level default vehicle widget (2026-08-02) on New quotation +
  // New purchase order, and per-line override on the detail page's
  // Add line form.
  poDefaultVehicleLegend: "For which vehicle?",
  poDefaultVehicleHint:
    "Pick a plate already in the garage, or type make and model for a car we haven't seen. All fields optional — an advisor asking a supplier to quote often has only make and model.",
  // AR 2026-08-22 Batch 9 — hydrate quotation lines from a job's
  // technician-requested parts. Removes the two-screen detour: no
  // more create-empty-shell → separate from-estimate step.
  hydrateJobLegend: "Load parts from a job (optional)",
  hydrateJobLoad: "Load",
  hydrateJobLoading: "Loading…",
  hydrateJobClear: "Clear",
  hydrateJobLoaded: "Loaded {count} part(s) from {number} — {vehicle}. Edit or remove before submit.",
  hydrateJobNotFound: "Job card #{n} not found in this garage. Check the number and try again.",
  hydrateJobEmpty: "No open part requests on this job. Add lines by hand below, or use a different job.",
  hydrateJobError: "Couldn't load parts. Please try again.",
  hydrateJobColDescription: "Part",
  hydrateJobColQty: "Qty",
  hydrateJobColRemove: "Remove line",
  // Source-chip labels. "findings" = the tech's "parts required"
  // list (JobPart, common case); "request" = the separate
  // PartRequest widget (less common). Chip lets the operator see
  // which list a hydrated row came from at a glance.
  hydrateJobSourceFindings: "tech findings",
  hydrateJobSourceRequest: "part request",
  vehiclePlateLabel: "Plate",
  vehiclePlatePlaceholder: "e.g. A 12345",
  vehicleMakeLabel: "Make",
  vehicleModelLabel: "Model",
  vehicleYearLabel: "Year",
  vehicleEngineLabel: "Engine",
  vehicleVinLabel: "VIN",
  vehicleJobNumberLabel: "Job card #",
  vehicleJobNumberPlaceholder: "e.g. 42",
  techHoursSessionsCol: "Sessions (time in → time out)",
  techHoursSessionLive: "in progress",
  movementFromPo: "From PO",
  poMovementsTitle: "Stock movements from this order",
  poMovementsKind: "Kind",
  poMovementsReceipt: "Receipt",
  poMovementsReturn: "Return",
  vehicleMatchLabel: "Matched garage record:",
  vehicleMatchDismiss: "Dismiss match",
  // /advisor/customers/[id] — the ONE customer-detail surface.
  customerDetailKicker: "Customer",
  customerEditLegend: "Customer details",
  customerEditHint:
    "Fields belong to the customer, not the car — updating here changes every vehicle they own in one step.",
  customerNameLabel: "Name",
  customerPhoneLabel: "Phone",
  // AR 2026-08-23. Rendered under the phone field on the customer
  // detail page when Customer.phoneNeedsReview is true — the number
  // was accepted (nothing is refused at write time) but can't be
  // resolved to a wa.me-compatible E.164, so sends will fall through
  // to the WhatsApp contact picker. Fixing the number here clears
  // the flag and restores direct-send.
  customerPhoneNeedsReview:
    "This number couldn't be read as a valid mobile. WhatsApp sends fall back to the contact picker until it's corrected.",
  customerPhonePatternHint:
    "Digits only, optional leading + or 0. Spaces and dashes are OK. UAE mobile: 0501234567 or +971501234567.",
  customerSaveButton: "Save",
  customerVehiclesHeading: "Vehicles",
  customerNoVehicles: "No vehicles on this customer yet.",
  customerJobsHeading: "Job history",
  customerNoJobs: "No visits yet.",
  customerTotalPaid: "Total paid:",
  poAddLineVehicleFor: "For",
  poAddLineVehicleChange: "Change",
  poAddLineVehicleUseDefault: "Use document default",
  poAddLineNoDefaultVehicle: "No document vehicle. Set one per line if needed.",
  noPoLines: "No lines yet. Add a part above.",
  noPartsForPo: "Add a part to your catalog first, in",
  markOrdered: "Mark as ordered",
  markOrderedNeedsPricesReason:
    "Every line needs a supplier price before you can order. A blank price means the quote hasn't landed yet; 0 is fine for a warranty or courtesy replacement.",
  cancelPo: "Cancel order",
  poReceivingSoon: "Receiving into stock arrives next.",
  // 2b partial receiving
  poOrdered: "Ordered",
  poReceived: "Received",
  poOutstanding: "Outstanding",
  noVehicleLinkedShort: "—",
  noVehicleLinkedReason:
    "No vehicle linked. This line's part isn't tied to a job (e.g. pre-existing stock).",
  poReceivedLower: "received",
  poOutstandingLower: "outstanding",
  poReceiveHeading: "Receive delivery",
  poReceiveHint: "Enter how many of each part arrived now. Less than ordered is fine — receive the rest later.",
  poReceiveNow: "Receive now",
  poReceiveButton: "Receive into stock",
  // Direct-fit receive (AR 2026-08-16, see docs/direct-fit-receive-spec.md).
  // Rendered only when a PO line has no catalogue part linked. The
  // owner picks per-line: Direct-fit (default) writes a receipt
  // against the job's ledger and never touches the catalogue;
  // Stock item defers to the existing "link a catalogue part on the
  // line-edit form first" flow.
  poReceiveDirectOption: "Direct-fit (fitted to the job, no shelf)",
  poReceiveDirectHelp:
    "Ball joint for a specific car, timing belt for one repair — arrives and is fitted, never stocked. Records the supplier's cost against this job.",
  poReceiveStockOption: "Stock item (add to inventory)",
  poReceiveStockHelp:
    "Consumables kept on the shelf across many jobs. Link this line to a catalogue part on the line-edit form first, then come back.",
  poReceiveDirectCostLabel: "Actual per-unit cost paid",
  poReceiveDirectPartNoLabel: "Supplier part number (optional)",
  // Catalogue-name hint on an unlinked line. Rendered only when the
  // line's description normalizes to an existing Part in the shop's
  // catalogue. Non-authoritative — the default stays Direct-fit; the
  // owner flips to Stock if this really is a shelf item.
  poReceiveCatalogueHint:
    "You stock a part called \"{name}\" ({sku}) — is this the same? If yes, pick Stock item.",
  poReceivedBanner: "Fully received on",
  // 2c purchase returns
  poReturned: "Returned",
  poReturnedLower: "returned",
  poReturnableLower: "returnable",
  poReturnHeading: "Return to supplier",
  poReturnHint: "Enter how many of each received part to send back. Stock drops by that amount; you can't return more than was received.",
  poReturnNow: "Return now",
  poReturnButton: "Return to supplier",
  billing: "Billing & plan",
  statusLabel: "Status",
  renewsLabel: "renews",
  perMonth: "mo",
  onPilotNotBilled: "You’re on a pilot — not billed.",
  billingManual: "During the pilot, billing is handled manually by Garage Os — you won’t be charged through the app.",
  billingNotConfigured: "Billing isn’t configured yet.",
  subscribe: "Subscribe",
  cancelSubscription: "Cancel subscription",
  tabChats: "Chats",
  tabWhatsapp: "WhatsApp",
  tabMore: "More",
  tabBranches: "Branches",
  branchesTitle: "Branches",
  branchesIntro: "Each branch has its own staff, WhatsApp number and queue. You see them all aggregated here.",
  addBranch: "Add branch",
  branchName: "Branch name",
  branchMainTag: "main",
  assignBranchLabel: "Branch",
  perBranchBilling: "Billing is per branch — each branch is one subscription seat.",
  branchError: "Enter a branch name.",
  chats: "Conversations",
  noChats: "No conversations yet.",
  needsHuman: "needs human",
  humanMode: "you",
  botMode: "AI",
  // startTestConvo removed 2026-08-19 — the "Start a test
  // conversation (no Meta needed)" panel + its
  // startTestConversationAction were deleted; the string had no
  // remaining reader. See rule 7 in docs/business-rules.md.
  send: "Send",
  takeOver: "Take over",
  release: "Back to AI",
  pendingDraft: "AI draft — approve before it sends",
  approveSend: "Approve & send",
  discard: "Discard",
  message: "Message",
  // simulateInbound removed 2026-08-19 alongside the deletion of
  // simulateInboundAction. See rule 7.
  chatSimulatedTag: "SIMULATED",
  // AR 2026-08-19 — banner on a specific draft card when
  // approveDraftAction refused to send because the body still
  // contained a `___` placeholder or a [draft] marker. Advisor
  // edits the textarea to fill in the real value, then re-clicks
  // Approve. See src/lib/chat-draft.ts.
  draftUnfilledTitle: "Can't send this draft yet",
  draftUnfilledBody: "This draft still contains a placeholder (\"___\") or a \"[draft]\" marker. Fill in the real value or remove the marker, then approve again.",
  waConnect: "Connect WhatsApp",
  waConnectIntro: "Connect your garage's WhatsApp number so messages send from your own number.",
  waConnected: "Connected",
  waDisconnect: "Disconnect",
  waConnectHint: "In production this opens the WhatsApp signup popup. For now, enter a number to simulate connecting.",
  waConnectBtn: "Connect WhatsApp",
  waConnectBtnSandbox: "Connect (sandbox)",
  waProdMode: "Production mode (Meta BSP)",
  waSandboxMode: "Sandbox mode (no Meta credentials)",
  waProdHint: "Click the button below to launch Meta Embedded Signup. Your customer (you, here) adds your WhatsApp Business number in the popup. The connection persists after you close the popup.",
  waHelpNeedCreds: "To switch to production mode, set META_WHATSAPP_APP_ID, META_WHATSAPP_CONFIG_ID, and META_WHATSAPP_APP_SECRET (server) + NEXT_PUBLIC_META_WHATSAPP_APP_ID and NEXT_PUBLIC_META_WHATSAPP_CONFIG_ID (client) on the Vercel project, then redeploy.",
  waStatusTitle: "WhatsApp",
  waStatusSubtitleAdvisor: "Connection status and your recent outbound messages. Connect / disconnect is done from the owner's settings.",
  waStatusSubtitleCashier: "Connection status and your garage's recent outbound messages. Connect / disconnect is done from the owner's settings.",
  waStatusNotConnected: "Not connected",
  waStatusAskOwner: "The garage owner can connect the number from the Owner → WhatsApp page.",
  waStatusReadyAdvisor: "The connection is live. Reminders you send will go out from this number.",
  waStatusReadyCashier: "The connection is live. Invoices and estimates you send will go out from this number.",
  waStatusOwnerControls: "Connect / disconnect is owner-only at",
  waRecentOutbound: "Recent outbound",
  waRecentEmpty: "No messages have been sent yet.",
  waStatusGoReminders: "Go to Reminders →",
  waStatusGoChats: "Go to Chats →",
  waStatusGoInvoices: "Go to Invoices →",
  waStatusGoEstimates: "Go to Estimates →",

  // Friendly job-status badges shown on every dashboard (advisor, tech,
  // cashier, owner) so every staff member sees the same handoff stage.
  fs_WAITING_FOR_TECH: "Waiting for technician",
  fs_TECH_DIAGNOSING: "Technician diagnosing",
  fs_ESTIMATE_UNDER_PROCESS: "Estimate under process",
  fs_AWAITING_CUSTOMER_APPROVAL: "Awaiting customer approval",
  fs_APPROVED_IN_PROGRESS: "Approved work in progress",
  fs_EXTRA_WORK_AWAITING_APPROVAL: "Extra work — awaiting approval",
  fs_COMPLETE_AWAITING_INVOICE: "Complete — awaiting invoice",
  fs_AWAITING_PAYMENT: "Awaiting payment",
  fs_WAITING_TECH_COMPLETE: "Waiting for technician to mark complete with a photo or findings",
  fs_READY_FOR_PICKUP: "Ready for pickup",
  fs_COMPLETE: "Collected",
  fs_ON_HOLD: "On hold",
  fs_CANCELLED: "Cancelled",
  // job statuses
  st_ARRIVED: "Arrived",
  st_INSPECTION: "Inspection",
  st_ESTIMATE: "Estimate",
  st_APPROVED: "Approved",
  st_REPAIR: "Repair",
  st_TECH_COMPLETE: "Tech complete",
  st_INVOICED: "Invoiced",
  st_DELIVERED: "Delivered",
  st_ON_HOLD: "On hold",
  st_CANCELLED: "Cancelled",

  // auth
  signInTitle: "Staff sign in",
  email: "Email",
  password: "Password",
  signIn: "Sign in",
  invalid: "Invalid email or password.",
  garageCreated: "Garage created — sign in with your owner account.",
  newGarageQ: "New garage?",
  setOneUp: "Set one up",

  // signup
  signupTitle: "Set up your garage",
  signupIntro: "Creates your garage and your owner account.",
  garageName: "Garage name",
  vatTrnOptional: "VAT TRN (optional)",
  yourNameOwner: "Your name (owner)",
  passwordMin6: "Password (min 6)",
  createGarage: "Create garage",
  alreadySetup: "Already set up?",
  emailExists: "That email is already registered.",
  fillAllFields: "Please fill all fields (password min 6 chars).",

  // team
  team: "Team",
  addStaff: "Add staff",
  name: "Name",
  tempPassword: "Temp password (min 6)",
  add: "Add",
  remove: "Remove",
  ownerTag: "owner",
  optAdvisor: "Advisor",
  optTechnician: "Technician",
  optCashier: "Cashier",
  optMaster: "Master Admin",
  estimatePrint: "🖨 Print estimate",
  estimateViewPrint: "View / print estimate",
  // AR 2026-08-15 estimate-honesty pass. Old copy "Sent to the customer"
  // read as delivered; wa.me hands off to the operator's WhatsApp and
  // Meta returns no delivery signal (see src/lib/wa.ts and the
  // JobCard.invoiceSentAt schema comment for the full lifecycle).
  // Renamed key + wording mirrors invoiceSentAt / invoiceHandedOffBanner
  // exactly so estimate and invoice read identically.
  estimateSentAt: "Handed to your WhatsApp at",
  estimateDeliveredAt: "Delivered to customer at",
  estimateHandedOffBanner:
    "Handed to your WhatsApp — press Send inside WhatsApp on your phone. The customer hasn't received it yet, and you can still edit the estimate.",
  estimateDeliveredBanner:
    "Delivered to the customer. You can still update the estimate — a new version will need to be sent.",
  imOnThisCar: "I'm on this car",
  onThisCarNow: "On this car",
  floorNow: "Floor right now",
  floorNowEmpty: "No one is on a car right now.",
  floorNowIdle: "Not on a car",
  floorNowUnavailable: "Live floor unavailable — try again in a moment.",
  workshopTime: "Workshop time",
  workshopTimeSummary: "{time} across {n} sessions",
  workshopTimeNone: "No workshop time recorded yet.",
  workshopTimeLive: "live",
  workshopTimeStale: "Unusually long session",
  wrenchTime: "Wrench time",
  wrenchDay: "Today",
  wrenchWeek: "This week",
  wrenchCar: "car",
  wrenchCars: "Cars",
  wrenchAvg: "Avg / car",
  // English only uses _one + _other. The _two / _few / _many keys
  // exist in the type only so the Arabic dictionary can define them;
  // Intl.PluralRules("en").select() never returns those categories.
  wrenchStaleCount: "{n} long session(s)",
  wrenchStaleCount_one: "1 long session",
  wrenchStaleCount_two: "{n} long sessions",
  wrenchStaleCount_few: "{n} long sessions",
  wrenchStaleCount_many: "{n} long sessions",
  wrenchStaleCount_other: "{n} long sessions",
  wrenchExcludedCount: "{n} session(s) excluded ({mins}) — over 8h, unknown work time",
  viewHours: "View hours",
  techHoursTitle: "Work history",
  techHoursRange: "Range",
  techHoursWeek: "This week",
  techHoursMonth: "This month",
  techHoursTotalTime: "Total time",
  techHoursTotalCars: "Cars touched",
  techHoursAvgDay: "Avg / day",
  techHoursDays: "Days active",
  techHoursJobCol: "Job #",
  techHoursJob: "Job #{n}",
  techHoursNone: "No work sessions in this range.",
  techHoursBack: "Back",
  techHoursLookupTitle: "Technician Hours",
  techHoursLookupIntro: "Look up any technician's day-by-day work history.",
  techHoursPickTech: "Technician",
  techHoursPickFrom: "From",
  techHoursPickTo: "To",
  techHoursLookupBtn: "View hours",
  techHoursNoTechs: "No technicians in this garage yet.",
  tabHours: "Hours",
  teamError: "Check the fields (password min 6).",

  // booking (customer)
  bookTitle: "Book a service",
  bookIntro: "Tell us what’s wrong in your own words — we’ll propose a fix.",
  // Photo rejection banner on the public booking form. Enum codes
  // come from validateImageUpload / LogoValidationError; anything not
  // in the render-side whitelist falls back to `bookPhotoErr_generic`.
  // Server-side detail (provider text, exception message) NEVER
  // reaches this — only the code.
  bookPhotoErrorTitle: "Your photo wasn’t accepted",
  bookPhotoErr_generic: "Try a different photo, or continue without one.",
  bookPhotoErr_EMPTY: "The photo file appears to be empty.",
  bookPhotoErr_TOO_LARGE: "The photo is too large — please try a smaller one (up to 5 MB).",
  bookPhotoErr_BAD_MIME: "That file type isn’t supported. Please use PNG, JPEG, or WEBP.",
  bookPhotoErr_BAD_MAGIC: "The photo doesn’t look like a real image. Please try another.",
  bookPhotoErr_MIME_MISMATCH: "The photo type doesn’t match the file contents. Please try another.",
  yourName: "Your name",
  phone: "Phone",
  make: "Make",
  model: "Model",
  plate: "Plate",
  describe: "Describe the problem (e.g. ‘AC not cooling when hot’)",
  optionalPhoto: "Optional photo",
  getProposal: "Get a proposal",
  thanksGotIt: "Thanks — we’ve got it",
  youToldUs: "You told us:",
  ourFirstRead: "Our first read:",
  suggested: "Suggested",
  urgencyLabel: "Urgency",
  bookingConfirmed: "✅ An advisor confirmed your booking.",
  bookingRejected: "An advisor will call you to discuss.",
  bookingPending: "🕒 An advisor will confirm shortly. This is a proposal, not a final quote.",

  // advisor
  activeJobs: "Active jobs",
  newJobCard: "+ New job card",
  newBookings: "New bookings",
  noActiveJobs: "No active jobs. Create one to start the timeline.",
  bookingsIntro: "AI proposed — you confirm. Confirming creates a job card.",
  confirmCreateJob: "Confirm → create job",
  reject: "Reject",
  noPendingBookings: "No pending bookings.",
  newJobTitle: "New job card",
  pickVehicle: "Pick the vehicle that arrived.",
  start: "Start →",
  noVehicles: "No vehicles yet. They arrive via customer booking.",
  backActiveJobs: "← Active jobs",

  // new-customer intake (Moulkia OCR + plate lookup)
  newCustomerMoulkia: "New customer (Moulkia)",
  moulkiaHint: "Photograph the Moulkia — we'll auto-fill owner, plate, make, model, year.",
  moulkiaConsent: "Customer consents to extracting vehicle & owner details from the Moulkia photo.",
  scanMoulkia: "Scan Moulkia",
  repeatCustomer: "Repeat customer (plate lookup)",
  plateLookupHint: "Enter the plate — we'll auto-fill from the existing record.",
  lookupBtn: "Look up",
  orPickExisting: "Or pick an existing vehicle",
  confirmCustomerTitle: "Confirm details",
  confirmHint: "Review the auto-filled details, correct anything, then start the job.",
  ownerName: "Owner name",
  mobile: "Mobile number",
  yearLabel: "Year",
  vinLabel: "VIN",
  startJobCard: "Start job card",
  // Reception form's submit button + the confirmation screen the advisor
  // sees after the handoff is recorded.
  sendToTechnician: "Send to Technician →",
  handoffTitle: "Handed off to the workshop",
  handoffShared: "The job is now in the shared technician pool — any available technician can claim it.",
  // Slice 3 — intake disambiguation panel copy. Replaces the slice-5
  // handoff warning (which was a non-blocking notice AFTER the job
  // card was written); the panel intercepts BEFORE the write and
  // asks the advisor to classify the collision.
  intakePanelTitle: "This plate is already on record",
  intakePanelSubtitle: "You entered {plate}. Compare it to the existing record below and pick what fits.",
  intakePanelRecordHeading: "Existing record",
  intakePanelOwnerLabel: "Owner",
  intakePanelPhoneLabel: "Mobile",
  intakePanelPlateLabel: "Plate",
  intakePanelVehicleLabel: "Vehicle",
  intakePanelVinLabel: "Chassis",
  intakePanelChoiceSameOwnerTitle: "Same car, same owner",
  intakePanelChoiceSameOwnerSubtitle: "Load {owner}'s car and continue.",
  intakePanelChoiceNewOwnerTitle: "Same car, owner has changed",
  intakePanelChoiceNewOwnerSubtitle: "Keep the car and its full service history. Update the owner's name and mobile.",
  intakePanelChoicePlateMovedTitle: "Different car — same plate number",
  intakePanelChoicePlateMovedSubtitle: "This plate was on {owner}'s car. Release it and start a new intake for this car.",
  intakePanelChoiceWrongPlateTitle: "Wrong plate — search again",
  intakePanelChoiceWrongPlateSubtitle: "Go back and correct the plate.",
  intakeErrorDismiss: "Dismiss",
  handoffSpecific: "Sent to {name}. They'll see it in their workshop queue.",
  handoffJobNo: "Job Card No.",
  handoffViewJob: "View job →",
  handoffCreateAnother: "+ Create another job card",
  handoffBackToActive: "← Back to active jobs",
  // Tech 'Send for Estimate' button + confirmation screen
  sendForEstimate: "Send for Estimate →",
  sentToAdvisorTitle: "Job sent to advisor for estimate",
  sentToAdvisorSubtitle: "The advisor will price the job and send the estimate to the customer.",
  jobApprovedStartWorkTitle: "Approved — start work",
  jobApprovedStartWorkSubtitle: "The estimate has been approved. Begin your work — add photos, notes, or voice updates as you go.",
  beginYourWorkHint: "Mark Complete unlocks after your first photo, voice note, or finding update.",
  sentToCashierForInvoiceTitle: "Sent to cashier for invoice",
  sentToCashierForInvoiceSubtitle: "The cashier will prepare the final invoice.",
  jobCompleteNoActionTitle: "Job complete — no further action needed",
  jobCompleteNoActionSubtitle: "This job has been invoiced and closed. Nothing more to do on the floor.",
  jobCancelledTitle: "This job was cancelled.",
  sentToCashierViewJob: "View job →",
  sentToCashierBackToWorkshop: "← Back to workshop",
  // Soft errors on the tech job detail
  errAlreadySentForEstimate: "This job is already with the advisor or further along.",
  errNotYourJob: "This isn't your job — only the technician who claimed it can send it for estimate.",
  errCannotComplete: "This job isn't in the work-in-progress stage — can't mark it complete.",
  errCannotReestimate: "This job isn't in the work-in-progress stage — can't send for re-estimate.",
  // Tech 'Send for Re-estimate' button + confirmation screen
  sendForReestimate: "Send for Re-estimate →",
  sentForReestimateTitle: "Sent for re-estimate — extra work flagged",
  sentForReestimateSubtitle: "The advisor will price the extra work and send the customer a new estimate to approve.",
  // Extras panel on the tech job detail page (mid-job extra parts)
  extrasPanelTitle: "Extras found mid-job (awaiting approval)",
  extrasPanelEmpty: "No extras added yet. Add any new parts or issues found during the approved work.",
  extrasDescriptionPlaceholder: "Extra part or issue found",
  extrasAdd: "Add extra",
  extrasRemove: "Remove extra",
  extrasSendForApproval: "Send {count} extra(s) for approval →",
  extrasSendForApproval_one: "Send 1 extra for approval →",
  extrasSendForApproval_two: "Send {n} extras for approval →",
  extrasSendForApproval_few: "Send {n} extras for approval →",
  extrasSendForApproval_many: "Send {n} extras for approval →",
  extrasSendForApproval_other: "Send {n} extras for approval →",
  errExtraDescriptionRequired: "Add a description for the extra part / issue.",
  errCannotAddExtra: "Can only add extras while the approved work is in progress.",
  errNoExtras: "Add at least one extra item before sending for approval.",
  // Tech 'Mark complete' button + confirmation screen (Stage 7→8)
  markCompleteAndSend: "Finish →",
  markedCompleteTitle: "Marked complete — sent to cashier",
  markedCompleteSubtitle: "The cashier will finalise the invoice and send it to the customer.",
  markedCompleteCompletedAt: "Completed at",
  // "Send via WhatsApp" wa.me button (invoice + estimate + reminder etc)
  waSend: "Send via WhatsApp",
  waSendDisabled: "Customer phone number is missing or invalid",
  // MASTER-only label for the advisor jobs entry point. MASTER wears
  // three hats (advisor + tech + cashier) so the tab is labelled by
  // role rather than "Jobs" (which is what non-MASTER advisors see).
  tabAdvisor: "Advisor",
  // Cashier 'Send invoice to customer' button + confirmation screen (Stage 8)
  sendInvoiceToCustomer: "Send via WhatsApp →",
  // Handed-off wording (2026-08-09). The Send button now opens
  // wa.me pre-filled with the invoice message; the operator taps Send
  // inside their own WhatsApp. Until that Send is tapped the customer
  // has NOTHING. When the Meta Cloud API commit lands and the send
  // becomes real, this reverts to "sent" — one-line i18n change, no
  // surface code.
  invoiceSentTitle: "Invoice handed to WhatsApp",
  invoiceSentSubtitle:
    "Open WhatsApp on your phone and press Send. Until you do, the customer hasn't received anything.",
  invoiceSentMockNote:
    "Record payment here once the customer pays. If they say they never got the invoice, resend from WhatsApp — the message is still in the chat draft.",
  invoicePreviewNote: "Review the customer-facing invoice before sending.",
  invoicePreviewButton: "Preview →",
  // AR 2026-08-25 — post-send route back to /invoices/[id]/preview
  // on the sent invoice, matching the estimate editor's
  // "View / print estimate" that persists after send.
  invoiceViewPrint: "View / print invoice",
  invoicePreviewGoBack: "← Go Back",
  invoiceDownloadPdf: "Download PDF",
  invoiceDownloadPdfHint:
    "Save a PDF copy — same layout the customer sees, ready to attach anywhere WhatsApp can carry a file.",
  invoicePreviewMockNote:
    "Send via WhatsApp opens the customer's WhatsApp on your phone with the message pre-filled. You still need to press Send there — the customer receives nothing until you do.",
  invoiceSentAt: "Handed to your WhatsApp at",
  invoiceDeliveredAt: "Delivered to customer at",
  invoiceHandedOffBanner:
    "Handed to your WhatsApp — press Send inside WhatsApp on your phone. The customer hasn't received it yet, and you can still edit the invoice.",
  invoiceDeliveredBanner:
    "Delivered to the customer. Line edits are locked — use Void & correct to issue a replacement.",
  invoiceResendViaWhatsApp: "Resend via WhatsApp →",
  invoiceBadgeVoid: "VOID",
  invoiceReplacesLabel: "Replaces",
  invoiceReplacedByLabel: "Replaced by",
  invoiceVoidedAt: "Voided at",
  invoiceVoidCorrectHint:
    "This invoice has been delivered — lines are locked. Void it to issue a corrected replacement. The void keeps its number for audit; the correction takes the next number.",
  invoiceVoidCorrectButton: "Void this invoice",
  invoiceReissueHint:
    "This invoice was voided. Issue a correction to charge what should have been on the original — the new invoice will reference this one so an auditor can follow the trail.",
  invoiceReissueButton: "Issue correction →",
  invoiceAlreadySent:
    "Invoice was handed to your WhatsApp — press Send in WhatsApp on your phone if you haven't already.",
  invoiceBackToInvoice: "← Back to invoice",
  invoiceBackToCashier: "← Back to accounts",
  // Workflow duration captions (claim → send-for-estimate → estimate-sent)
  durDiagnosis: "Diagnosis",
  durPricing: "Pricing",
  durSoFar: "so far",
  // Job-detail timeline section
  jobTimelineTitle: "Workflow timeline",
  jobTimelineClaimedAt: "Tech claimed",
  jobTimelineSentForEstimateAt: "Sent for estimate",
  // jobTimelineEstimateSentAt removed (AR 2026-08-15) — key had no
  // consumers; the live label is tlEstimateSent via
  // src/lib/job-timeline-labels.ts.
  jobTimelineNotYet: "—",
  prefilledFromRecord: "Auto-filled from the existing record.",
  errConsent: "Please confirm customer consent first.",
  errNoFile: "Select a Moulkia photo.",
  errFields: "Fill owner, mobile, plate, make and model.",
  errOcrFailed: "Couldn't read the Moulkia — please enter the details manually.",
  // Tier 1 OCR taxonomy (AR 2026-08-14) — surfaces when the failure is
  // one of the two actionable categories rather than a plain unreadable
  // photo. The generic errOcrFailed / errOcrBackFailed remain the
  // fallback for anything not classified.
  errOcrBilling: "The Moulkia scanner is temporarily unavailable — please contact the shop owner. Enter the details manually for this customer.",
  errOcrTemporary: "The Moulkia scanner hit a temporary problem. Please try again in a moment, or enter the details manually.",
  errPlateBelongsToAnotherCustomer: "That plate is already registered to a different customer. Double-check the plate you entered.",
  tryAgain: "Try again",
  // PhotoCapture (one-tap camera/mic) — UX labels
  takePhoto: "Take photo",
  takePhotoMoulkia: "Take photo of Moulkia",
  takePhotoMoulkiaFront: "Take FRONT of Moulkia",
  takePhotoMoulkiaBack: "Take BACK of Moulkia",
  step1of2: "Step 1 of 2 — front side (owner + plate)",
  step2of2: "Step 2 of 2 — back side (VIN, make, model, year)",
  frontCaptured: "✓ Front captured",
  backOfMoulkia: "Back of Moulkia",
  skipBackLink: "Skip the back — I'll enter the details manually",
  errOcrBackFailed: "Couldn't read the back of the Moulkia — please fill the vehicle details manually.",
  backSkipped: "Back side skipped — fill the vehicle details (VIN, make, model, year) by hand.",
  optimizingPhoto: "Optimising photo…",
  retake: "Retake",
  usePhoto: "Use this photo →",
  recordVoice: "Record voice note",
  fileTooBig: "Photo is too big — please retake at a lower size.",
  wrongFileType: "Wrong file type — please pick a photo or use the camera.",
  // Moulkia consent rephrased — by-action rather than tick-box (legal review pending)
  moulkiaConsentInline: "By scanning, the customer agrees to extract their vehicle owner details for this service.",
  moulkiaOrManualHint: "No Moulkia? Use Enter manually below.",
  manualEntry: "Enter manually (no Moulkia)",
  manualEntryHint: "If the customer has no Moulkia, enter the vehicle details by hand.",
  enterManually: "Enter manually →",
  manualEntryIntro: "Enter the vehicle and customer details by hand.",
  // reception form (Job-Card-Data-Model.md)
  secCustomer: "Customer",
  secVehicle: "Vehicle",
  secComplaint: "Complaint",
  secCondition: "Vehicle condition at check-in",
  secValuables: "Valuables left in vehicle",
  secAssignment: "Assignment (optional)",
  secConsent: "Consent",
  jobNoLabel: "Job Card No",
  timeInLabel: "Time in",
  advisorLabel: "Advisor",
  branchLabel: "Branch",
  mileageInLabel: "Mileage (km)",
  checkInLabel: "Check-in",
  oilTypeLabel: "Oil type",
  fuelLevelLabel: "Fuel level",
  fuelTypeLabel: "Fuel type",
  engineSizeLabel: "Engine size",
  fuelType_PETROL: "Petrol / Gasoline",
  fuelType_DIESEL: "Diesel",
  fuelType_HYBRID: "Hybrid",
  fuelType_ELECTRIC: "Electric",
  fuelType_OTHER: "Other",
  vinDecodeBtn: "Decode VIN",
  vinDecoding: "Decoding…",
  vinInvalid: "VIN must be 17 alphanumeric characters (no I, O, or Q).",
  vinNotFound: "NHTSA returned no usable data for this VIN. Manual entry still works.",
  vinNetworkError: "Couldn't reach the VIN decoder. Try again or fill manually.",
  vinDecodedNote: "Tap to auto-fill Make / Model / Year / Fuel type from the VIN.",
  vinFilledFields: "Decoded from VIN — please verify",
  tabVehicles: "Vehicles",
  vehiclesTitle: "Vehicles",
  vehiclesSubtitle: "Search by plate, owner name, or phone to see a vehicle's full service history.",
  vehiclesSearchPlaceholder: "Plate, owner name, or phone…",
  vehiclesSearchBtn: "Search",
  vehiclesSearchClear: "Clear",
  vehiclesEmptyHint: "Type something to search.",
  vehiclesNoMatch: "No vehicles found.",
  vehiclesVisitSingular: "visit",
  vehiclesVisitPlural: "visits",
  vehiclesBackToSearch: "Back to vehicle search",
  // AR 2026-08-25, Batch A — printable vehicle-history lookup at
  // /advisor/vehicles/[id]/history. Screen-first with print CSS;
  // customer-facing on paper more than any other doc in the app, so
  // cost + margin are hidden by default on-screen and always on print.
  printLabel: "Print",
  vehicleHistoryBack: "Back to vehicle",
  vehicleHistoryTitle: "Vehicle service history",
  vehicleHistoryCurrentOwner: "Current owner",
  vehicleHistoryChangedHands: "This vehicle has changed hands",
  vehicleHistoryEmpty: "No visits recorded on this vehicle yet.",
  vehicleHistoryColDate: "Date",
  vehicleHistoryColJc: "JC #",
  vehicleHistoryColMileage: "Mileage in",
  vehicleHistoryColWork: "Work done",
  vehicleHistoryColParts: "Parts fitted",
  vehicleHistoryColRevenue: "Amount",
  vehicleHistoryColCost: "Cost",
  vehicleHistoryColMargin: "Margin",
  vehicleHistoryColStatus: "Status",
  vehicleHistoryEstimatedNotInvoiced: "estimated, not invoiced",
  vehicleHistoryUnpaid: "Unpaid",
  vehicleHistoryTotalsLabel: "Lifetime",
  vehicleHistoryVisits: "visits",
  vehicleHistoryPrintableLink: "View printable history",
  // Mileage-decrease markers on /advisor/vehicles/[id]/history.
  // Screen + print. Neutral wording per AR 2026-08-25 — plain-text
  // dagger on flagged rows, top-of-page note when any row is
  // flagged, footnote at the bottom explaining the dagger. No
  // emoji, no colour dependency.
  vehicleHistoryMileageNoteHeading: "Note:",
  vehicleHistoryMileageNoteBody:
    "One or more visits below record a mileage reading lower than a previous visit. Marked with † in the mileage column.",
  vehicleHistoryMileageFootnote:
    "Mileage lower than a previous visit. Possible reasons include odometer replacement or an entry mistake.",
  // AR 2026-08-25 Batch B — printable customer statement + customers
  // list surface for the ADVISOR/OWNER/MASTER/CASHIER nav entry.
  tabCustomers: "Customers",
  customersListHeading: "Customers",
  customersSearchLabel: "Search by name, phone, or TRN",
  customersSearchPlaceholder: "e.g. Ahmed, 971501234567, 100000000000003",
  customersSearchGo: "Search",
  customersEmptyHint: "No customers yet.",
  customersNoMatch: "No customers match that search.",
  customersRecentPrefix: "Recently updated customers, most recent first.",
  customersRecentTruncated: "Showing the 50 most recent — search to find older records.",
  customersOpenCta: "Open",
  customersVehicleSingular: "vehicle",
  customersVehiclePlural: "vehicles",
  statementShortLink: "Statement",
  statementTitle: "Customer statement",
  statementBackToCustomer: "Back to customer",
  statementCustomerLabel: "Customer",
  statementCustomerTrnLabel: "TRN",
  statementAsOfLabel: "Statement as of",
  statementVehiclesLabel: "Vehicles owned",
  statementNoInvoices: "No invoices on this customer's vehicles.",
  statementColDate: "Date",
  statementColInvoice: "Invoice",
  statementColVehicle: "Vehicle",
  statementColTotal: "Total",
  statementColPaid: "Paid",
  statementColOutstanding: "Outstanding",
  statementColDaysOverdue: "Aging",
  statementColCost: "Cost",
  statementColMargin: "Margin",
  statementBucketCurrent: "Current",
  statementBucket1_30: "1–30 days",
  statementBucket31_60: "31–60 days",
  statementBucket61_90: "61–90 days",
  statementBucket90_plus: "90+ days",
  statementAdvancesHeading: "Advance payments (credit)",
  statementAdvanceJobCol: "Job",
  statementAdvanceMethodCol: "Method",
  statementAdvanceAmountCol: "Amount",
  statementAgingHeading: "Aging summary",
  statementInvoicesOutstandingLabel: "Invoices outstanding",
  statementAdvancesCreditLabel: "Advance payments (credit)",
  statementNetBalanceLabel: "Net balance owed",
  statementInCreditNote: "Customer is in credit — advance payments exceed outstanding invoices.",
  vehiclesVisits: "Visits",
  vehiclesTotalSpend: "Total spend",
  vehiclesLastVisit: "Last visit",
  vehiclesLastOilChange: "Last oil change",
  vehiclesAtMileage: "at",
  vehiclesNoHistory: "No previous visits for this vehicle.",
  vehiclesVisitHistory: "Visit history",
  vehiclesComplaint: "Complaint",
  vehiclesWorkDone: "Work done",
  vehiclesPartsReplaced: "Parts replaced",
  vehiclesLabor: "Labor",
  vehiclesPaid: "Paid",
  vehicleBeenHere: "This vehicle has been here before.",
  vehicleViewHistory: "View history",
  vehiclesLoadErrorTitle: "Couldn't load vehicle history",
  vehiclesLoadErrorBody: "Something went wrong reading the past visits for this vehicle. The error details are below — please share them with your administrator. You can still use the rest of the app.",
  complaintLabel: "What's the problem?",
  exteriorLabel: "Exterior",
  interiorLabel: "Interior",
  remarksLabel: "Remarks",
  valuablesNoteLabel: "Other (describe)",
  oil_KM_5000: "5,000 km",
  oil_KM_10000: "10,000 km",
  oil_NONE: "None",
  fuel_EMPTY: "Empty",
  fuel_QUARTER: "¼",
  fuel_HALF: "½",
  fuel_THREE_QUARTER: "¾",
  fuel_FULL: "Full",
  ext_NO_DAMAGE: "No damage",
  ext_SCRATCHES: "Scratches",
  ext_DENTS: "Dents",
  ext_BROKEN_LIGHT: "Broken light",
  ext_CRACKED_WINDSHIELD: "Cracked windshield",
  int_CLEAN: "Clean",
  int_DIRTY: "Dirty",
  int_WARNING_LIGHT: "Warning light on",
  int_OTHER: "Other",
  val_NONE: "None",
  val_DOCUMENTS: "Documents",
  val_CASH: "Cash",
  val_MOBILE_CHARGER: "Mobile charger",
  val_OTHER: "Other",

  // job timeline
  backJob: "← Job",
  advanceTo: "Advance to",
  putOnHold: "Put on hold",
  resume: "Resume",
  sendBackToEstimate: "Send back to Estimate",
  cancelJob: "Cancel job",
  jobCancelled: "🔴 This job was cancelled.",
  deliveredComplete: "🟢 Delivered — this job is complete.",
  noFurtherActions: "No further actions.",
  estimates: "Estimates",
  newEstimate: "+ New estimate",
  noEstimates: "No estimates yet.",
  estimate: "Estimate",
  invoiceArrow: "invoice →",
  techActivity: "Technician activity",

  // technician
  tapJob: "Tap a job to work on it.",
  noActiveJobsTech: "No active jobs right now.",
  backWorkshop: "← Workshop",
  addPhoto: "Add Photo",
  requestPart: "Request Part",
  finish: "Finish",
  pickPart: "Pick a part…",
  request: "Request",
  activity: "Activity",
  nothingYet: "Nothing yet.",
  upload: "Upload",
  waiting: "Waiting",
  inProgressMine: "In progress (yours)",
  take: "Take",
  releaseCar: "Release",
  open: "Open",
  alreadyTaken: "That car was just taken by someone else.",
  noWaiting: "No cars waiting.",
  forYou: "for you",
  assignTo: "Assign to",
  unassigned: "Unassigned (shared pool)",
  technicianLabel: "Technician",
  working: "working now",
  assignedLabel: "assigned",
  reassign: "Reassign",
  priorityLabel: "Priority",
  setPriority: "Set",
  prNormal: "Normal",
  prUrgent: "Urgent",
  prEmergency: "Emergency",
  // multiple technicians (Tier 2 #4)
  joinJob: "Join",
  leaveJob: "Leave",
  otherInProgress: "Other jobs in progress",
  helpingTag: "helping",
  helpersLabel: "Helpers",
  primaryTag: "primary",
  // technician findings & parts required (Job-Card-Data-Model.md)
  complaintFromReception: "Customer complaint",
  jcFindings: "Findings & diagnosis",
  findingsLabel: "Findings",
  diagnosisLabel: "Diagnosis",
  saveDraft: "Save",
  partsRequired: "Parts required",
  partNoLabel: "Part No",
  addPartLine: "Save",
  submitToCashier: "Send to Cashier for Estimate →",
  submittedToCashier: "Submitted to cashier",
  findingsLocked: "Submitted — read only.",
  photographDamaged: "Photograph the damaged parts.",
  noPartsRequired: "No parts listed yet.",
  catalogPartOptional: "Catalog part (optional)",
  // AR 2026-08-17 — hint shown next to the Unit price input when a picked
  // catalogue part has no price set on the Part row. The `required` attr +
  // parseMoney already block the submit; this line explains WHY the field
  // stayed blank instead of pre-filling, so the advisor knows to type it or
  // fix the Inventory row.
  catalogPartNoPriceHint: "This part has no price in inventory — type the price, or set one in Inventory.",
  // Price-this-part inline form (AR 2026-08-19). Advisor types cost +
  // price on each tech-required JobPart row, no catalogue write.
  priceThisPart_unitCost: "Unit cost",
  priceThisPart_unitPrice: "Unit price",
  priceThisPart_save: "Save",
  priceThisPart_cancel: "Cancel",
  priceThisPart_priced: "Priced",
  // AR 2026-08-22 — hint under the Markup % field when the value
  // was prefilled from Garage.defaultPartsMarkupPct rather than
  // typed by the advisor. Hides the moment the advisor changes it.
  priceThisPart_markupFromDefault: "from garage default",
  // AR 2026-08-18 — banner shown on /estimates/[id] and /invoices/[id]
  // when a server action redirects with ?formError=<code>. Every
  // code in LINE_FORM_ERROR_CODES (src/lib/billing.ts) needs a matching
  // key here — the render-side whitelist falls through to
  // lineFormErr_generic if a code is missing.
  lineFormErrorTitle: "Couldn't save that line",
  lineFormErr_generic: "That line couldn't be saved. Check the values and try again.",
  lineFormErr_price_required: "Unit price is required — a blank isn't a real zero. Type the price, or if the line is genuinely free (courtesy/warranty), type 0 explicitly.",
  lineFormErr_price_negative: "Unit price can't be negative. For a customer discount, use the Discount line type instead.",
  lineFormErr_price_not_numeric: "Unit price must be a number.",
  lineFormErr_desc_required: "Description is required.",
  lineFormErr_qty_invalid: "Quantity must be a positive number.",
  lineFormErr_kind_unknown: "Choose a valid line type (Labor / Part / Fee / Discount).",
  // Exit-gate messages — AR 2026-08-18. The banner lists the offending
  // line descriptions after the copy (see how the preview + estimate
  // pages render this). Keep the leading sentence generic; the shop-
  // facing detail (line names) is appended by the page layer.
  lineFormErr_zero_part_lines_estimate: "This estimate has PART lines priced at AED 0.00. Set real prices, mark the lines as declined, or use \"Send anyway\" if they're genuinely free (warranty/courtesy).",
  lineFormErr_zero_part_lines_invoice: "This invoice would carry PART lines priced at AED 0.00. Set real prices, mark the lines as declined, or use \"Generate anyway\" if they're genuinely free. The invoice number is consumed from a gapless sequence — correcting after issue needs void + reissue.",
  // AR 2026-08-23 — sendEstimateToCustomer refuses to touch an
  // already-decided row. Was silently reverting APPROVED → SENT
  // on any resend before this gate landed.
  lineFormErr_estimate_already_approved: "The customer has already approved this estimate. Re-sending would wipe the approval. Open the WhatsApp thread for a follow-up message instead of re-sending the estimate.",
  lineFormErr_estimate_already_rejected: "The customer has already rejected this estimate. Create a new estimate to send a revised quote instead of re-sending this one.",
  // AR 2026-08-23 — soft nudge above the WhatsApp Send button on
  // estimate / invoice preview pages when the customer's phone can't
  // be normalised to E.164. The send still works (falls through to
  // WhatsApp's contact picker); this banner tells the operator why
  // WhatsApp is about to ask them to pick the recipient.
  phoneNudgeHeading: "Customer phone couldn't be read as a valid number. WhatsApp will ask you to pick the recipient. Fix it on the customer record so it works next time.",
  phoneNudgeRawLabel: "On file:",
  phoneNudgeRawNone: "(none captured)",
  lineFormErr_zeroPartsHeading: "Lines priced at AED 0.00:",
  lineFormErr_sendAnyway: "Send anyway (these are genuinely free)",
  lineFormErr_generateAnyway: "Generate anyway (these are genuinely free)",
  // Pre-flight warning — AR 2026-08-18. Rendered on the estimate edit
  // + preview pages BEFORE the advisor clicks Send/Generate Invoice,
  // whenever any non-declined PART line is 0.00. Warning-yellow (not
  // danger-red) — this is informational; the post-click gate is what
  // enforces. Keeps semantic separation between "you might want to
  // check this before you click" and "we refused, take action".
  zeroPartsPreflightTitle: "Unpriced PART lines",
  zeroPartsPreflightBody: "This estimate has PART lines priced at AED 0.00. Set real prices or mark them declined before you send. If they're genuinely free (warranty/courtesy), you'll be asked to confirm on send.",
  zeroPartsChip: "⚠ No price",
  techFindingsPanel: "Technician findings & parts required",
  enterFindingsFirst: "Enter findings before submitting.",
  // repair stage (parts used + work notes)
  jcRepair: "Repair — work completed",
  workNotesLabel: "Work completed notes",
  partsUsed: "Parts used",
  noPartsUsed: "No parts used yet.",
  markComplete: "Mark Complete →",
  workCompletedBadge: "Work completed",
  repairLockedNote: "Work complete — read only.",
  partsUsedPanel: "Parts used & work notes",
  priceThisPart: "Price this part →",
  // quality control
  jcQc: "Quality control",
  signOffQc: "Sign off QC",
  qcPassedBadge: "QC passed",
  qcInspector: "Inspector",
  qc_REPAIR_COMPLETED: "Repair completed",
  qc_ROAD_TEST: "Road test completed",
  qc_NO_WARNING_LIGHTS: "No warning lights",
  qc_VEHICLE_CLEANED: "Vehicle cleaned",
  // voice dictation
  dictateStart: "Dictate",
  dictateStop: "Stop",
  dictateListening: "Listening… (tap to stop)",
  dictateError: "Couldn't hear that — try again or type.",
  // delivery (Job-Card-Data-Model.md)
  jcDelivery: "Delivery",
  mileageOutLabel: "Mileage out (km)",
  recordDelivery: "Record delivery",
  markDelivered: "Mark delivered",
  deliveredByLabel: "Delivered by",
  deliveredAtLabel: "Delivered at",
  collectionAwaiting: "Awaiting customer confirmation",
  collectionConfirmed: "Customer confirmed collection",
  // customer-facing /c/delivery
  collectionTitle: "Confirm collection",
  documentDelivery: "Delivery",
  collectionIntro: "Please confirm you've collected your vehicle.",
  confirmCollectionBtn: "Confirm collection",
  collectionThanks: "✅ Thank you — collection confirmed.",
  // bays / ramps (Tier 2 #7)
  tabBays: "Bays",
  baysTitle: "Bays & ramps",
  baysIntro: "Your workshop's physical capacity. Assign a car to a bay; the count shows free space.",
  addBay: "Add bay",
  bayNamePh: "Bay name (e.g. Lift 1)",
  bayLabel: "Bay",
  noBay: "No bay",
  baysInUse: "in use",
  bayEmpty: "Empty",
  bayMultipleOccupants: "⚠ {n} active jobs on this bay — data glitch, please investigate",
  chatSearchPlaceholder: "Search by customer, phone, or message",
  chatSearchClear: "Clear",
  chatNoSearchResults: "No chats match your search.",
  chatUnreadTag: "{n} unread",
  chatUnreadTag_one: "1 unread",
  chatUnreadTag_two: "{n} unread",
  chatUnreadTag_few: "{n} unread",
  chatUnreadTag_many: "{n} unread",
  chatUnreadTag_other: "{n} unread",
  analyticsCsvDownload: "Download CSV",
  bayExists: "That bay name already exists.",

  // estimate editor + invoice
  colItem: "Item",
  colQty: "Qty",
  colUnit: "Unit",
  colTotal: "Total",
  // Cost-based pricing (AR 2026-08-12) — advisor-only labels on the
  // estimate line editor's tri-input mode for PART lines. Never
  // rendered on customer or invoice surfaces.
  costLabel: "Cost",
  markupLabel: "Markup %",
  marginLabel: "Margin",
  colAmount: "Amount",
  colDescription: "Description",
  // Batch F2 — printed-doc header wording (F2.4). "Particulars"
  // reads more like a shop's own document than the generic
  // "Description"; S/N adds a per-row line index that shops rely
  // on when reading the doc aloud to a customer. Deliberately
  // separate keys from colDescription so unrelated on-screen
  // tables keep their existing labels.
  colParticulars: "Particulars",
  colSerialNumber: "S/N",
  // Batch F1 (2026-08-25) — VIN on printed estimate + invoice.
  documentVinLabel: "VIN",
  // Batch F2 (2026-08-25) — totals wording on the printed doc.
  // Deliberately separate from the terse "Subtotal / VAT / Total"
  // strings used on the busy edit surfaces, so the printed doc reads
  // like the shop's own template.
  totalGrossLabel: "Gross total",
  totalVatLabel: "5% VAT",
  totalNetLabel: "Net total",
  // Batch F2 (2026-08-25) — centred main title on the printed estimate.
  docTitleRepairEstimate: "Repair Estimate",
  colKind: "Type",
  colMake: "Make",
  colModel: "Model",
  colYear: "Year",
  colActions: "Actions",
  colPart: "Part",
  kindLabor: "Labor",
  kindPart: "Part",
  kindSublet: "Sublet",
  kindFee: "Fee",
  kindDiscount: "Discount",
  addLineTitle: "Add line item",
  addLineButton: "Add",
  removeLine: "Remove",
  noLineItems: "No line items yet.",
  subtotal: "Subtotal",
  subtotalBeforeDiscount: "Subtotal (before discount)",
  subtotalAfterDiscount: "Subtotal (after discount)",
  netSubtotal: "Net Subtotal",
  vat5: "VAT (5%)",
  discountSectionTitle: "Discount",
  discountPercentLabel: "Discount %",
  discountAmountLabel: "Discount Amount",
  discountApply: "Apply",
  discountRemove: "Remove discount",
  discountNone: "No discount applied",
  discountCurrentPercent: "Currently applied: {pct}%",
  discountCurrentFixed: "Currently applied: {amount}",
  discountRow: "Discount",
  discountRowPercent: "Discount ({pct}%)",
  discountRowFixed: "Discount",
  total: "Total",
  description: "Description",
  addLine: "Add line",
  labor: "Labor",
  part: "Part",
  // AR 2026-08-25 Batch C — SUBLET line kind (outside work,
  // consumables, services). Distinct from FEE. See docs on the
  // enum in prisma/schema.prisma.
  sublet: "Sublet",
  fee: "Fee",
  // Batch C — estimate document restructure.
  estimateSectionParts: "Parts",
  estimateSectionSublet: "Sublet / Consumables / Services",
  estimateSectionLabour: "Labour",
  estimateSectionSubtotal: "Subtotal",
  estimateTotalsSumOfSections: "Sections total",
  estimateRemarksHeading: "Remarks",
  estimateRemarksLabel: "Remarks",
  estimateRemarksPlaceholder: "e.g. This estimate is prepared only for the required work to make the engine start and run. Further diagnosis will be required for other faults.",
  estimatePaymentTermsHeading: "Payment terms",
  estimatePaymentTermsLabel: "Payment terms",
  estimatePaymentTermsPlaceholder: "e.g. 50% advance and 50% on delivery",
  estimatePaymentTermsUsingDefaultHint: "Blank uses the shop default set in Settings.",
  estimateAdvisorHeading: "Service advisor",
  estimateHeaderSaveButton: "Save remarks + terms",
  // Settings — Batch C shop-wide default payment terms.
  settingsPaymentTermsLabel: "Default payment terms",
  settingsPaymentTermsPlaceholder: "e.g. 50% advance and 50% on delivery",
  settingsPaymentTermsHint: "Printed on every estimate. Advisors can override per estimate.",
  // Settings — split-terms (AR 2026-08-25). Two sections, one per
  // document type. Wording tuned so each set only carries clauses
  // that apply to its document. Both keep the same responsibility
  // banner (settingsTermsResponsibility) — the message is generic.
  settingsSecEstimateTerms: "Estimate terms",
  settingsSecEstimateTermsHint: "Prints at the bottom of every estimate. Free text — write your own numbered clauses.",
  settingsEstimateTermsLabel: "Estimate terms & conditions",
  settingsEstimateTermsPlaceholder: "1. This estimate is valid for 7 days.\n2. Prices subject to change.\n3. …",
  settingsEstimateTermsHint: "Line breaks are preserved. Leave empty to hide the block on the estimate.",
  // AR 2026-08-25 — rewrite after the "prefill masks unsaved" trap
  // (the seeded sample rendered as the textarea's defaultValue so it
  // was visually indistinguishable from a saved value; three shops
  // saw "populated" fields in Settings but nothing printed on the
  // doc). The current UX shows the sample in a separate "Suggested
  // wording (not yet saved)" block with an explicit Adopt button;
  // the textarea itself only carries the shop's saved wording (empty
  // by default). This copy reflects that split.
  settingsTermsResponsibility: "These are your garage's own terms and your responsibility. Nothing prints on the customer's document until you save wording in the field below.",
  settingsTermsSuggestedHeading: "Suggested wording — not yet saved",
  settingsTermsSuggestedHint: "This is a starting point, not a saved value. To use it as-is, click below. Or type your own wording in the field and Save.",
  settingsTermsAdoptButton: "Use this as my terms",
  // AR 2026-08-25 — print-notice on the editor pages. Editors were
  // never meant to print; Ctrl+P out of habit produced a broken
  // multi-page render with nav, add-line form, and per-row edit
  // controls. On print, the whole editor is hidden and this notice
  // shows instead, pointing the user at the Preview page which IS
  // the sanctioned print surface.
  printEditorNoticeHeading: "This is the editor view",
  printEditorNoticeBody: "The customer's document lives on the Preview page. Return to the app, open the estimate or invoice, click Preview, and print from there.",
  settingsEstimateTermsDefaultText:
    "1. This estimate is valid for 7 days from the date shown above.\n2. Prices are subject to change without prior notice.\n3. No warranty is provided on parts supplied by the customer.\n4. Any additional work identified during the repair will be quoted and requires customer approval before proceeding.\n5. The final invoice amount may vary from this estimate if additional work is authorised.\n6. Parts are classified as OEM (Original Equipment Manufacturer), OES (Original Equipment Supplier), or Aftermarket, as marked on each line.\n7. The customer authorises the workshop to road-test the vehicle as required to verify the repair.",
  settingsOkEstimateTerms: "Estimate terms saved.",
  settingsSecInvoiceTerms: "Invoice terms",
  settingsSecInvoiceTermsHint: "Prints at the bottom of every invoice. Free text — write your own numbered clauses.",
  settingsInvoiceTermsLabel: "Invoice terms & conditions",
  settingsInvoiceTermsPlaceholder: "1. Workshop labour warranty as agreed.\n2. Parts warranty per supplier policy.\n3. …",
  settingsInvoiceTermsHint: "Line breaks are preserved. Leave empty to hide the block on the invoice.",
  // Short by design. Anything invented on the shop's behalf (specific
  // warranty periods, retention-of-title clauses) is deliberately left
  // out — a shop's terms are the shop's own document, not ours.
  settingsInvoiceTermsDefaultText:
    "1. Workshop labour warranty as agreed at the time of repair.\n2. Parts warranty is per the manufacturer's or supplier's own policy — the workshop passes through what the supplier provides, no more.\n3. No warranty applies to parts supplied by the customer.\n4. Payment terms as agreed.",
  settingsOkInvoiceTerms: "Invoice terms saved.",
  documentTermsHeading: "Terms & conditions",
  // Batch E — deploy-freshness banner (ship-hidden until the Vercel
  // env flag flips; keys still need to exist so a mid-deploy flag
  // flip doesn't render `[MISSING]` fallback labels).
  versionCheckMessage: "A newer version is available. Reload to get the latest — your unsaved input will be lost.",
  versionCheckReload: "Reload",
  versionCheckDismiss: "Later",
  discount: "Discount",
  skip: "Skip",
  restore: "Restore",
  // Estimate-line edit/delete (DRAFT only; locked once SENT/APPROVED)
  editLine: "Edit",
  deleteLine: "Delete",
  saveLine: "Save",
  cancelLine: "Cancel",
  confirmDeleteLine: "Delete this line?",
  onHold: "On hold",
  hrAwaitingPart: "Awaiting part",
  hrAwaitingCustomer: "Awaiting customer",
  hrAwaitingApproval: "Awaiting approval",
  hrOther: "Other",

  // Workflow stepper labels
  stage_CHECK_IN: "Check-in",
  stage_DIAGNOSIS: "Diagnosis",
  stage_ESTIMATE: "Estimate",
  stage_APPROVAL: "Approval",
  stage_REPAIR: "Repair",
  stage_COMPLETE: "Complete",
  stage_INVOICE: "Invoice",
  stage_PAID: "Paid",
  stage_DELIVERED: "Delivered",
  workflowCancelled: "Job cancelled",
  workflowPaused: "Paused",

  // Job timeline (audit log)
  tlTitle: "Job timeline",
  tlNoActor: "—",
  tlToday: "Today",
  tlRole_ADVISOR: "Advisor",
  tlRole_TECH: "Technician",
  tlRole_CASHIER: "Cashier",
  tlRole_OWNER: "Owner",
  tlVehicleCheckedIn: "Vehicle checked in",
  tlConsentRecorded: "Moulkia consent recorded",
  tlClaimedByTech: "Claimed by technician",
  tlPhotoLogged: "Photo logged",
  tlVoiceLogged: "Voice note logged",
  tlPartRequested: "Part requested",
  tlFindingsSubmitted: "Findings submitted",
  // KEY DECISION #5 (2026-06-23): estimate pricing moved from cashier
  // to advisor. Re-labelled so HISTORICAL timeline rows on existing
  // job cards stop saying "cashier" — the JobAction enum value didn't
  // change, only the human-readable label.
  tlSentForEstimate: "Sent to advisor for estimate",
  tlSentForReestimate: "Sent to advisor for re-estimate (extras)",
  tlEstimateCreated: "Estimate created",
  // Estimate-honesty pass (AR 2026-08-15). Was "Estimate sent to
  // customer" — same wa.me hand-off / no delivery signal as invoices.
  // Mirrors tlInvoiceSent / tlInvoiceDelivered exactly.
  tlEstimateSent: "Estimate handed off to WhatsApp",
  tlEstimateDelivered: "Estimate delivered to customer",
  tlEstimateApproved: "Estimate approved by customer",
  tlWorkCompleted: "Work completed",
  tlQcPassed: "QC passed",
  tlInvoiceIssued: "Invoice issued",
  tlInvoiceSent: "Invoice handed off to WhatsApp",
  tlInvoiceDelivered: "Invoice delivered to customer",
  tlAdvancePayment: "Advance payment received",
  tlPaymentRecorded: "Payment recorded",
  tlDelivered: "Delivered to customer",
  tlCollectionConfirmed: "Customer confirmed collection",
  tlJobCancelled: "Job cancelled",
  tlJobHeld: "Job put on hold",
  paused: "Paused",
  approvedOn: "Approved",
  holdNote: "Note (optional)",
  skipTo: "Skip to",
  sendToCustomer: "Send to customer",
  estimatePreviewButton: "Preview Estimate →",
  estimatePreviewGoBack: "← Go Back",
  jobCardPrintTitle: "Job card",
  jobCardPrintBtn: "🖨 Print job card",
  printJobCard: "🖨 Print job card",
  signatureCustomer: "Customer signature",
  signatureAdvisor: "Advisor signature",
  signatureDate: "Date",
  moulkiaConsentRecorded: "Moulkia consent recorded",
  disputeShieldNote:
    "This receipt confirms the vehicle condition and valuables at drop-off. Any additional damage claims must be raised before you take the vehicle away.",
  estimateSendToCustomer: "Send Estimate to Customer →",
  // Resend affordance on the SENT-state preview (AR 2026-08-16
  // estimate-send fix). Mirrors invoiceResendViaWhatsApp so
  // estimate + invoice read the same on hand-off retries.
  estimateResendViaWhatsApp: "Resend via WhatsApp →",
  estimatePreviewMockNote: "Send Estimate opens WhatsApp on your device with the message ready — tap Send in WhatsApp to actually deliver.",
  markApproved: "Mark approved",
  markRejected: "Mark rejected",
  generateInvoice: "Generate invoice",
  estimateApprovedSentToTech: "Approved — sent to technician for work.",
  continueToWorkshop: "Continue to Workshop →",
  confirmCancelJob: "Cancel this job? It will disappear from Workshop.",
  paginationShowing: "Showing {from}–{to} of {total}",
  paginationRowsPerPage: "Rows per page",
  paginationPrev: "Prev",
  paginationNext: "Next",
  suppliersInactiveCap: "Showing first {shown} of {total}.",
  viewInvoice: "View invoice →",
  taxInvoice: "Tax Invoice",
  customerTrnLabel: "Customer TRN",
  customerTrnEditLegend: "Customer VAT registration (TRN)",
  customerTrnEditHint:
    "If the customer is VAT-registered, enter their TRN so their invoice carries it — they need this to reclaim the VAT.",
  customerTrnSave: "Save",
  customerTrnSaved: "Saved",
  billTo: "Bill to",
  issued: "Issued",
  due: "Due",
  clearance: "Clearance",
  paid: "Paid",
  balance: "Balance",
  recordPayment: "Record payment",
  amount: "Amount",
  methodCash: "Cash",
  methodCard: "Card",
  methodTransfer: "Bank transfer",
  methodCardPos: "Card (POS)",
  methodOnlineLink: "Online Link",
  markAsPaid: "Mark as Paid",
  soon: "soon",
  qrPlaceholder: "QR placeholder",

  // customer estimate / invoice
  yourEstimate: "Your estimate",
  approve: "Approve",
  decline: "Decline",
  estimateApprovedMsg: "✅ You approved this estimate.",
  estimateDeclinedMsg: "❌ You declined this estimate.",
  estimateNotReady: "This estimate isn’t ready for approval yet.",
  yourInvoice: "Your invoice",
  paidThanks: "✅ Paid — thank you!",
  payAtGarage: "Please pay at the garage (cash or card)",

  // cashier
  accounts: "Accounts",
  cashierToInvoiceTitle: "Ready for Invoice",
  cashierGenerateInvoice: "Generate Invoice →",
  cashierGoToInvoice: "Open invoice →",
  cashierReestimateTitle: "Extra work — re-estimate requested",
  cashierApprovedEstimatesTitle: "Approved estimates",
  cashierApprovedEstimatesEmpty: "No approved estimates waiting to be invoiced.",
  cashierFilterActive: "Filtered",
  cashierClearFilter: "Clear filter ×",
  cashierWaitingDiagnosisTitle: "Waiting for technician diagnosis",
  cashierWaitingDiagnosisCaption: "The advisor will price this once the technician finishes diagnosis.",
  cashierWorkInProgressTitle: "Work in progress (customer approved)",
  cashierWorkInProgressCaption: "Work in progress — waiting for technician to complete.",
  cashierToSendInvoiceTitle: "Invoice ready — send to customer",
  cashierSendInvoiceToCustomer: "Send via WhatsApp →",
  // Phase 5: read-only roll-up of advisor-owned estimate work shown
  // to the cashier so they see what's queued, without any actionable
  // button (pricing / sending all live on [[/advisor/estimates]]).
  cashierComingUpTitle: "Coming up",
  cashierComingUpHint: "Advisor is pricing or waiting for the customer — these will land here once approved + tech completes.",
  cashierComingUpEmpty: "Nothing in the pipeline right now.",
  createEstimate: "Create estimate",
  continuePricing: "Continue pricing",
  rePrice: "Re-price",
  advisorPleasePrice: "The technician handed this off — your turn to price the estimate.",
  advisorDraftInProgress: "Draft estimate in progress — finish pricing and send to the customer.",
  pricingByAdvisor: "Pricing is set by the advisor.",
  awaitingPriceTag: "awaiting price",
  mRevenue: "Revenue",
  mVatCollected: "VAT collected",
  mCashIn: "Cash in",
  mArOutstanding: "AR outstanding",
  receivables: "Receivables (unpaid)",
  noInvoices: "No invoices yet.",
  noUnpaidInvoices: "No unpaid invoices — everything cleared.",
  paidInvoicesTab: "Paid Invoices →",
  cashierSearchPlaceholder: "Search invoice, job #, plate, or customer",
  cashierFilterFrom: "From",
  cashierFilterTo: "To",
  cashierFilterClear: "Clear",
  cashierTabEstimates: "Estimates",
  cashierTabInvoices: "Invoices",
  cashierTabPayments: "Payments",
  cashierTabReports: "Reports",
  cashierTabReportsHeading: "Reports",
  counterApprovedEstimates: "Approved:",
  counterUnpaidInvoices: "Unpaid Invoices:",
  counterPaidInvoices: "Paid Invoices:",
  counterOverdueInvoices: "Overdue:",
  counterPartiallyPaidInvoices: "Partially Paid:",
  counterReadyForInvoice: "Ready for Invoice:",
  invoiceBadgeOverdue: "Overdue",
  invoiceBadgePartial: "Partially Paid",
  invoiceBadgePaid: "Paid",
  invoiceAdvancePaid: "Advance/Paid",
  invoiceBalanceDue: "Balance Due",
  advancePaymentsTitle: "Advance Payments",
  advancePaymentsCeiling: "Approved estimate total",
  advancePaymentsFullyCovered: "Advance covers the full approved estimate. Balance due is 0.",
  receiveAdvancePayment: "Receive Advance Payment",
  method: "Method",
  invoicePrintInvoice: "Print Invoice",
  invoicePrintReceipt: "Print Receipt",
  // Kept in the dictionary because the Email Invoice button is still
  // rendered (behind `hidden`) — the copy lives here so re-enabling
  // is a one-line UI change once SMTP is wired.
  invoiceEmailInvoice: "Email Invoice",
  invoiceLinkInvalidTitle: "This link isn't working",
  invoiceLinkInvalidBody:
    "The invoice link you tapped is invalid or expired. Reply to the WhatsApp message you received and ask the garage to send a fresh link.",
  invoiceLinkInvalidBodyNamed:
    "The invoice link you tapped is invalid or expired. Reply to the WhatsApp message you received from {garage} and ask them to send a fresh link.",
  invoiceReceiptTitle: "Receipt",
  invoiceReceiptCustomer: "Customer",
  invoiceReceiptInvoiceNumber: "Invoice #",
  invoiceReceiptAmountPaid: "Amount paid",
  invoiceReceiptDate: "Date",
  invoiceReceiptMethod: "Method",
  invoiceReceiptBalance: "Balance",
  invoiceReceiptTotal: "Total",
  invoiceReceiptThanks: "Thank you for your business.",
  paidInvoicesTitle: "Paid Invoices",
  paidInvoicesEmpty: "No paid invoices yet.",
  paidInvoicesBack: "← Back to dashboard",
  colCustomer: "Customer",
  colVehicle: "Vehicle",
  colVehicles: "Vehicles on this order",
  colVat: "VAT",
  colLineTotal: "Total",
  colMethod: "Method",
  colDatePaid: "Paid on",
  paidLower: "paid",
  dueLower: "due",
  ledgerNote: "All figures are generated from auto-posted ledger entries — no manual bookkeeping.",

  // parts (Tier 1 #3 — part-request loop)
  tabParts: "Parts",
  partsQueue: "Part requests",
  partRequests: "Part requests",
  cancelPartRequestConfirm: "Cancel this part request?",
  noOpenParts: "No open part requests.",
  inStockShort: "in stock",
  inStockTag: "in stock",
  outOfStock: "out of stock",
  orTypePart: "or type a part",
  requestedByLabel: "requested by",
  actFulfill: "Fulfill",
  actOrder: "Order",
  actArrived: "Mark arrived",
  actReorder: "Wrong / re-order",
  actCancelReq: "Cancel",
  supplierNote: "Supplier / ETA (optional)",
  pr_REQUESTED: "Requested",
  pr_ORDERED: "Ordered",
  pr_ARRIVED: "Arrived",
  pr_FULFILLED: "Fulfilled",
  pr_CANCELLED: "Cancelled",

  // maintenance reminders (Workflow-Spec step 16)
  tabReminders: "Reminders",
  remindersTitle: "Maintenance reminders",
  scheduleReminders: "Schedule maintenance reminders",
  scheduledReminders: "Scheduled reminders",
  serviceDateLabel: "Service date",
  scheduleBtn: "Schedule",
  noReminders: "No reminders scheduled.",
  dueNow: "Due now",
  upcoming: "Upcoming",
  sentTab: "Sent",
  sendReminderBtn: "Send",
  sendDueNow: "Send due now",
  dueOn: "due",
  sentOn: "sent",
  cancelReminderBtn: "Cancel",
  remindersBucketOverdue: "Overdue",
  remindersBucketDueSoon: "Due soon (next 7 days)",
  remindersBucketDueMonth: "Due this month",
  remindersBucketUpcoming: "Upcoming",
  remindersBulkSendOverdue: "Send to all overdue →",
  // AR 2026-08-19 — the new send-in-view bulk button carries the
  // visible count so an advisor filtering to one customer sees "Send
  // 1 shown", not the misleading unfiltered wording. Confirmation
  // prompt is honest about hand-off vs delivery (rule 4).
  remindersBulkSendOverdueN: "Send {n} shown →",
  remindersBulkSendConfirm: "Hand off {n} WhatsApp reminder(s) to Meta? (One message per customer. wa.me doesn't confirm delivery — see docs/business-rules.md rule 4.)",
  remindersSendToCustomer: "Send to customer →",
  remindersReminderSingular: "reminder",
  remindersReminderPlural: "reminders",
  remindersCarSingular: "car",
  remindersCarPlural: "cars",
  remindersFilterPlaceholder: "Filter by service, vehicle, plate, or customer",
  remindersFilterApply: "Filter",
  remindersFilterClear: "Clear",
  remindersFilterNoMatch: "No reminders match this filter.",
  remindersPrevMonth: "Previous month",
  remindersNextMonth: "Next month",
  remindersJumpToMonth: "Jump to month",
  remindersSummaryDueIn: "due in",
  remindersMonthEmpty: "No reminders due in this month.",
  remindersOtherMonths: "Other months with reminders",
  rt_OIL_5000: "Oil (5,000 km)",
  rt_OIL_10000: "Oil (10,000 km)",
  rt_BATTERY: "Battery",
  rt_TIRE_ROTATION: "Tyre rotation",
  rt_BRAKES: "Brakes",
  rt_AC_SERVICE: "AC service",
  rt_AIR_FILTER: "Air filter",
  rt_COOLANT: "Coolant",
  rt_TRANSMISSION: "Transmission",
  rs_SCHEDULED: "Scheduled",
  rs_SENT: "Sent",
  rs_CANCELLED: "Cancelled",

  // end-of-day view + check-in photo (Tier 3) + collection nudge (Tier 2 #9)
  tabEod: "End of day",
  eodTitle: "End of day",
  eodIntro: "What's still open before you close — and what each car is waiting on.",
  eodEmpty: "Nothing open — all clear. 🎉",
  eod_IN_PROGRESS: "In progress",
  eod_WAITING_PARTS: "Waiting on parts",
  eod_WAITING_CUSTOMER: "Waiting on customer",
  eod_READY: "Ready for collection",
  eod_OTHER: "On hold",
  nudgeCollect: "Nudge to collect",
  nudged: "Nudged ✓",
  checkInPhotoPrompt: "Photograph the car at check-in — protects against “you damaged it” claims.",
  addCheckInPhoto: "Add check-in photo",
  checkInDone: "Check-in photo on file ✓",

  // owner
  ownerDashboard: "Owner dashboard",
  // Money-at-a-glance tiles (AR 2026-08-30). All values are
  // ledger-derived — see src/lib/owner-dashboard.ts.
  dashCashLabel: "Cash received this month",
  dashRevenueLabel: "Revenue this month",
  dashUnpaidLabel: "Unpaid invoices",
  dashProfitLabel: "Gross profit this month",
  dashJobsLabel: "Jobs completed this month",
  dashAgingCurrent: "Current",
  dashAging1_30: "1–30d",
  dashAging31_60: "31–60d",
  dashAging61plus: "61d+",
  dashProfitIncompleteValue: "Cost data incomplete",
  dashProfitIncompleteSub: "{missing} of {total} missing cost. Revenue {revenue}.",
  dashProfitCompleteSub: "{margin}% margin · {costed} of {total} costed",
  dashTrendUp: "▲ {pct}% vs last month",
  dashTrendDown: "▼ {pct}% vs last month",
  dashTrendFlat: "= same",
  mRevenueMo: "Revenue (mo)",
  mProfitMo: "Profit (mo)",
  mCarsToday: "Cars today",
  mSatisfaction: "Satisfaction",
  mInventory: "Inventory",
  low: "low",
  thisWeek: "This week",
  vsLastWeek: "vs last week",
  satisfactionSoon: "Satisfaction tracking arrives with reviews.",
  askCopilot: "Ask the copilot",
  ask: "Ask",
  copilotReadonly: "Read-only · scoped to your garage.",
  pilotMetrics: "Pilot metrics & AI usage",
  intakeAcceptance: "Intake acceptance",
  avgConfirm: "Avg booking→confirm",
  aiCalls: "AI calls (mo)",
  aiCost: "AI cost (mo)",
  aiCostNote: "AI cost is metered per call (AiEvent) so usage can’t silently outrun the subscription.",
  noTechActivity: "No technician activity logged yet.",
  colTechnician: "Technician",
  colJobs: "Jobs",
  colSteps: "Steps",
  colAvgTimeTitle: "Average time per completed job (claimed → marked complete)",
  colTodayTimeTitle: "Total time on completed jobs today",
  colTodayJobsTitle: "Jobs completed today",
  colAvgShort: "avg",
  colTodayTime: "today ⏱",
  colTodayJobs: "today #",
  colPhotosTitle: "Photos logged",
  colVoiceTitle: "Voice notes logged",
  colPartsTitle: "Part requests logged",
  colFinishesTitle: "Finish steps logged",
  colApprovedTitle: "Estimates approved by customer",
  colRejectedTitle: "Estimates rejected by customer",
  advisorActivity: "Advisor activity",
  noAdvisorActivity: "No advisor activity yet.",
  colAdvisor: "Advisor",
  colEstimatesSent: "Sent",
  colApprovalRate: "rate",
  colApprovalRateTitle: "Approval rate (approved ÷ decided estimates)",
  colApprovalTimeShort: "avg",
  colApprovalTimeTitle: "Average time from estimate sent to customer decision",
  tabLedger: "Ledger",
  // AR 2026-08-23. Owner-only accounting-export surface — five CSVs
  // (COA / journal / invoices / payments / customers) + a companion
  // chart-of-accounts mapping doc. Sits in the More sheet next to
  // Ledger.
  tabAccounting: "Accounting export",
  tabErpSync: "ERPNext sync",
  acctExportHeading: "Accounting export",
  acctExportIntro: "Download the shop's financial data as CSVs for import into your accounting system. Every download is logged.",
  acctExportFromLabel: "From (inclusive)",
  acctExportToLabel: "To (inclusive)",
  acctExportApplyRange: "Apply range",
  acctExportDownloadCta: "Download CSV",
  acctExportMappingPointer: "For account codes, types, and the Cash/Bank split rule, see docs/accounting-chart-of-accounts.md in the repo.",
  acctExportChartLabel: "Chart of accounts (setup)",
  acctExportChartDesc: "One-time setup for your accounting system — account codes, names, types, normal balances. Import this first.",
  acctExportJournalLabel: "General journal",
  acctExportJournalDesc: "One row per ledger entry in the range: date, journal ref, account code, debit, credit, customer, memo.",
  acctExportInvoicesLabel: "Invoices",
  acctExportInvoicesDesc: "Every invoice issued in the range with subtotal, VAT, total, paid, balance, and customer TRN for B2B.",
  acctExportPaymentsLabel: "Payments",
  acctExportPaymentsDesc: "Payments and advance payments in the range. Advances show which invoice they later settled.",
  acctExportCustomersLabel: "Customers (range-scoped)",
  acctExportCustomersDesc: "Only customers who appear on invoices in the range. Not the full customer list — reduces personal data leaving the system.",
  ledgerTitle: "Account ledger",
  ledgerSubtitle: "Full double-entry ledger with per-payment audit. Read-only — entries are written by the system, never by hand.",
  ledgerFrom: "From",
  ledgerTo: "To",
  ledgerApply: "Apply",
  ledgerReset: "Reset",
  ledgerByAccount: "By account",
  ledgerAccount: "Account",
  ledgerDebit: "Debit",
  ledgerCredit: "Credit",
  ledgerNet: "Net",
  ledgerTrialBalance: "Trial balance",
  ledgerPayments: "Individual payments",
  ledgerPaymentsEmpty: "No payments recorded in this date range.",
  // Ledger account display names (localized labels for the ACCOUNTS
  // constants, which are stored raw in the DB — see accountLabel()).
  acctCashBank: "Cash/Bank",
  acctAR: "Accounts Receivable",
  acctVatPayable: "VAT Payable",
  acctSales: "Sales Revenue",
  acctDeposits: "Customer Deposits",
  ledgerAdvanceChip: "advance",
  ledgerMigratedChip: "migrated",
  tabAnalytics: "Analytics",
  analyticsTitle: "Analytics",
  analyticsSubtitle: "Daily trends — revenue, VAT, jobs, average ticket. Pick a window above.",
  analyticsRevenue: "Revenue",
  analyticsVat: "VAT collected",
  analyticsJobs: "Jobs created",
  analyticsAvgTicket: "Avg ticket",
  analyticsRevenuePerDay: "Revenue per day",
  analyticsVatPerDay: "VAT per day",
  analyticsJobsPerDay: "Jobs per day",
  analyticsAvgTicketPerDay: "Average ticket per day",
  analyticsWindowSelector: "Time window",
  // AR 2026-08-22 Batch 5 Step 6 — per-period profit widget.
  periodProfitTitle: "Profit — last {days} days",
  periodProfitRevenue: "Revenue",
  periodProfitProfit: "Profit",
  periodProfitCoverage: "Coverage",
  periodProfitCoverageOf: "{covered} of {total} invoices",
  periodProfitFootnote:
    "Margins use weighted-average part cost, not per-unit cost. Invoices with any missing cost data are excluded from Profit but count against Coverage.",
  // AR 2026-08-22 Batch 5 Step 7 — per-part report.
  partProfitTitle: "Per-part profit",
  partProfitSubtitle:
    "Which catalogue parts earn, and which sell at or below cost. Same coverage rule as the per-period widget.",
  partProfitLinkFromAnalytics: "Per-part profit report →",
  partProfitBackToAnalytics: "Back to analytics",
  partProfitEmpty:
    "No PART lines were invoiced in this window. Widen the window to see historical rollups.",
  partProfitEarners: "Earners",
  partProfitEarnersSubtitle: "Parts with positive profit, largest first.",
  partProfitEarnersEmpty: "No parts turned a profit in this window.",
  partProfitLosers: "Break-even or losing",
  partProfitLosersSubtitle:
    "Parts selling at or below their weighted-average cost. Biggest loss first.",
  partProfitLosersEmpty:
    "No parts sold at or below cost in this window.",
  partProfitUnknowns: "Coverage incomplete",
  partProfitUnknownsSubtitle:
    "At least one invoice line for this part is missing cost data. Profit and margin can't be computed until every line has a cost.",
  // AR 2026-08-22 Batch 5 Step 8 — margin chip on advisor jobs list.
  marginChipLabel: "margin",
  min: "min",
  sampleUpDown: "Are we up or down this week?",
  sampleProfit: "How much profit this month?",
  sampleOwes: "Who owes us money?",
  ansProfit: "This month’s profit is {v} so far (revenue minus parts cost).",
  ansProfitIncomplete: "Can’t give you a profit figure yet — {missing} of {total} invoices this month are missing PART cost. Revenue so far is {revenue}.",
  ansNobodyOwes: "Nobody owes you money right now — all invoices are paid. 🟢",
  ansOwes: "{n} customer(s) owe {total} in total — {list}.",
  ansTrend: "You’re {dir} this week: {a} vs {b} last week ({d}).",
  ansInvoiceSummary:
    "This month: {n} invoices totalling {total} ({paid} paid, {sent} sent, {draft} draft, {voided} voided).",
  ansVatMonth: "VAT collected this month: {v}.",
  ansAdvancesNone: "No outstanding advances — every advance has been applied to an invoice.",
  ansAdvancesOpen: "{n} advance(s) outstanding totalling {total} — {list}.",
  ansTechRanking: "Top technicians: {list}.",
  ansTechRankingNone: "No technician activity yet.",
  ansAdvisorRanking: "Top advisors: {list}.",
  ansAdvisorRankingNone: "No advisor activity yet.",
  ansLedgerBalance: "Cash on hand: {cash} · Outstanding receivables (AR): {ar}.",
  dirUp: "up",
  dirDown: "down",
  dirFlat: "flat",
  copilotUnknown:
    "I can answer about profit, receivables, weekly trend, invoice summaries, VAT, outstanding advances, top technicians or advisors, and the cash/AR balance.",
};

const ar: typeof en = {
  signOut: "تسجيل الخروج",
  settings: "الإعدادات",
  settingsTitle: "الإعدادات",
  settingsIntro: "حسابك و(إذا كنت المالك) تفاصيل ورشتك.",
  settingsSecProfile: "الملف الشخصي",
  settingsSecProfileHint: "اسمك كما يظهر للزملاء والعملاء.",
  settingsProfileName: "الاسم",
  settingsProfileEmail: "البريد الإلكتروني (تسجيل الدخول)",
  settingsProfileEmailWarning: "ستستخدم البريد الإلكتروني الجديد لتسجيل الدخول من الآن فصاعداً. تأكد من صحته.",
  settingsProfileEmailCurrent: "كلمة المرور الحالية (مطلوبة لتغيير البريد الإلكتروني)",
  settingsProfileSave: "حفظ",
  settingsProfileSaveName: "حفظ الاسم",
  settingsProfileSaveEmail: "حفظ البريد الإلكتروني",
  settingsSecPassword: "كلمة المرور",
  settingsSecPasswordHint: "غيّر كلمة المرور التي تستخدمها لتسجيل الدخول.",
  settingsSecTeam: "الفريق",
  settingsSecTeamHint: "إضافة أو إزالة الموظفين وتعيين الفروع.",
  settingsSecTeamGo: "إدارة الفريق",
  // Garage details — logo upload (Phase C).
  settingsSecGarageDetails: "بيانات الورشة",
  settingsSecGarageDetailsHint: "هوية ورشتك على كل فاتورة ضريبية وتقدير. تتطلب قوانين ضريبة القيمة المضافة في الإمارات ذكر الاسم والرقم الضريبي والعنوان على الفواتير الضريبية.",
  settingsGarageNameLabel: "اسم الورشة",
  settingsGarageTrnLabel: "الرقم الضريبي (TRN)",
  settingsGarageTrnHint: "15 رقمًا من الهيئة الاتحادية للضرائب. مطلوب لفواتير الشركات؛ يحتاجه العميل لاسترداد الضريبة.",
  settingsGarageAddressLabel: "العنوان",
  settingsGarageAddressHint: "صندوق البريد، المبنى، الشارع، المنطقة، المدينة. يُطبع على الفواتير الضريبية — تطلبه الهيئة الاتحادية للضرائب.",
  settingsGarageDefaultLangLabel: "اللغة الافتراضية للعميل",
  settingsGarageDefaultLangHint: "تُستخدم فقط عندما لا نستطيع تحديد لغة العميل بعد (عميل جديد، لا توجد رسائل واردة). بمجرد أن يكتب العميل إليك، نكتشف من رسالته.",
  settingsGarageDefaultLangUnset: "غير محددة — تعود إلى الإنجليزية",
  settingsGarageDefaultLangEn: "الإنجليزية",
  settingsGarageDefaultLangAr: "العربية",
  settingsGarageCountryLabel: "الدولة",
  settingsGarageCountryValueUae: "الإمارات العربية المتحدة",
  settingsGarageCountryLocked: "تغيير الدولة يُعيد اشتقاق استراتيجية ضريبة القيمة المضافة والعملة؛ اتصل بالدعم لنقل الورشة.",
  settingsGarageVatLabel: "نسبة ضريبة القيمة المضافة",
  settingsGarageVatValueUae: "5% (الهيئة الاتحادية للضرائب)",
  settingsGarageVatLocked: "ثابتة بموجب قانون ضريبة القيمة المضافة الإماراتي؛ غير قابلة للتعديل.",
  settingsOkGarageName: "تم حفظ اسم الورشة.",
  settingsOkGarageTrn: "تم حفظ الرقم الضريبي.",
  settingsOkGarageAddress: "تم حفظ العنوان.",
  settingsOkGarageDefaultLang: "تم حفظ اللغة الافتراضية للعميل.",
  // Batch F1 (2026-08-25).
  settingsGaragePhoneLabel: "رقم هاتف الورشة",
  settingsGaragePhonePlaceholder: "+971 50 000 0000",
  settingsGaragePhoneHint: "يُطبع على عروض الأسعار والفواتير كرقم للاتصال بالمستشار المسؤول. إذا لم يكن للمستشار رقم شخصي مسجّل، يُستخدم رقم الورشة.",
  settingsOkGaragePhone: "تم حفظ رقم الورشة.",
  settingsErrPhoneTooLong: "رقم الاتصال طويل جداً (بحد أقصى ٤٠ حرفاً).",
  settingsErrGarageNameRequired: "اسم الورشة لا يمكن أن يكون فارغًا.",
  settingsErrGarageNameTooLong: "اسم الورشة طويل جدًا (80 حرفًا كحد أقصى).",
  settingsErrTrnTooLong: "الرقم الضريبي طويل جدًا (40 حرفًا كحد أقصى).",
  settingsErrAddressTooLong: "العنوان طويل جدًا (400 حرف كحد أقصى).",
  settingsErrGarageLangInvalid: "اختر الإنجليزية أو العربية أو اتركها غير محددة.",
  settingsSecGarageLogo: "شعار الورشة",
  settingsSecGarageLogoHint: "يظهر على فواتير العملاء، التقديرات، وأعلى كل لوحة معلومات. PNG أو JPEG أو WEBP، حتى 500 كيلوبايت.",
  settingsLogoPick: "اختر ملفاً",
  settingsLogoPicked: "تم الاختيار",
  settingsLogoReplace: "اختر ملفاً مختلفاً",
  settingsLogoUpload: "رفع الشعار",
  settingsLogoUploading: "جاري الرفع…",
  settingsLogoRemove: "إزالة الشعار",
  settingsOkLogoSaved: "تم حفظ الشعار.",
  settingsOkLogoRemoved: "تمت إزالة الشعار — يتم استخدام العلامة الافتراضية.",
  // Pricing defaults (AR 2026-08-12).
  settingsSecPricing: "إعدادات التسعير الافتراضية",
  settingsSecPricingHint: "قيم بداية على مستوى الورشة لمحرر بنود التقدير. يمكن للمستشار تعديلها لكل بند.",
  settingsPricingMarkupLabel: "نسبة الربح الافتراضية على قطع الغيار",
  settingsPricingMarkupPlaceholder: "40",
  settingsPricingMarkupHint: "اتركها فارغة لطلب إدخال النسبة يدوياً في كل بند.",
  settingsSave: "حفظ",
  settingsOkMarkup: "تم حفظ نسبة الربح الافتراضية.",
  settingsErrRole: "فقط المالك يمكنه تغيير هذا.",
  settingsErrMarkupInvalid: "يجب أن تكون النسبة رقماً.",
  settingsErrMarkupRange: "يجب أن تكون النسبة بين 0 و 999.99٪.",
  // Labour hourly cost.
  settingsPricingLaborCostLabel: "تكلفة ساعة العمل",
  settingsPricingLaborCostPlaceholder: "60",
  settingsPricingLaborCostHint: "اتركها فارغة لتخطي حساب ربح العمالة — سيظهر التقرير كـ‬ غير معروف، وليس صفراً.",
  settingsOkLaborCost: "تم حفظ تكلفة ساعة العمل.",
  settingsErrLaborCostInvalid: "يجب أن تكون تكلفة ساعة العمل رقماً.",
  settingsErrLaborCostRange: "يجب أن تكون تكلفة ساعة العمل صفراً أو أكثر.",
  settingsErrLogoMissing: "لم يتم إرفاق ملف.",
  settingsErrLogoEmpty: "الملف المختار فارغ.",
  settingsErrLogoTooLarge: "يجب أن يكون الشعار {limit} كيلوبايت أو أصغر.",
  settingsErrLogoBadMime: "يجب أن يكون الشعار صورة PNG أو JPEG أو WEBP.",
  profitCardTitle: "الربح (عرض داخلي للفريق)",
  profitCardRevenue: "الإيرادات",
  profitCardCost: "التكلفة",
  profitCardGross: "إجمالي الربح",
  profitCardMargin: "الهامش",
  profitCardMarkup: "هامش الربح على التكلفة",
  profitCardParts: "قطع الغيار",
  profitCardLabour: "العمالة",
  profitCardHeadlineIncomplete:
    "لا يمكن احتساب التكلفة والربح — بعض البنود بدون بيانات تكلفة. راجع سطور التغطية أدناه.",
  profitCardCoverageParts: "{covered} من {total} قطع بها بيانات تكلفة",
  profitCardCoverageReceiptsMismatch:
    "{count} إيصال(ات) توضح أن التكلفة الفعلية المدفوعة تختلف عمّا ورد في الفاتورة — قد تكون تكلفة الفاتورة {direction} تكلفة القطع بمقدار {delta}.",
  profitCardCoverageReceiptsDirectionUnder: "أعلى من",
  profitCardCoverageReceiptsDirectionOver: "أقل من",
  profitCardCoverageReceiptsUnlinkable:
    "{count} إيصال تركيب مباشر غير مرتبط ببند فاتورة — تحقق يدويًا من مطابقة التكلفة على الفاتورة لما دفعته.",
  profitCardCoverageLabour: "{covered} من {total} جلسات عمل بها بيانات تكلفة",
  profitCardNoLabourSessions: "لا توجد جلسات عمل مسجلة على هذه المهمة.",
  profitCardLabourRateMissing: "تكلفة العمالة غير معروفة — لم يتم ضبط سعر الساعة.",
  profitCardLabourRateMissingCta: "ضبط سعر ساعة العمل",
  profitCardNotePartsCost:
    "تُحتسب الهوامش على أساس متوسط تكلفة القطعة المرجّح، وليس التكلفة الفعلية للوحدة.",
  profitCardNoteLabourCost:
    "تكلفة العمالة هي سعر الساعة المُعدّ مضروبًا في وقت العمل المسجل — وليست نقدًا مدفوعًا للحظتها.",
  profitCardNoteMarginVsMarkup:
    "الهامش هو الربح كنسبة من سعر البيع، وهامش الربح على التكلفة هو الربح كنسبة من التكلفة. زيادة 15% على التكلفة تظهر كهامش نحو 13% على سعر البيع.",
  profitCardUnknown: "غير معروفة",
  settingsErrLogoBadMagic: "هذا الملف لا يبدو كصورة حقيقية.",
  settingsErrLogoMimeMismatch: "محتوى الملف لا يطابق النوع المعلن.",
  settingsOkName: "تم تحديث الاسم.",
  settingsOkEmail: "تم تحديث البريد الإلكتروني. ستستخدم البريد الجديد في المرة القادمة لتسجيل الدخول.",
  settingsOkEmailUnchanged: "البريد الإلكتروني مضبوط على هذا بالفعل — لا شيء للقيام به.",
  settingsErrNameRequired: "الاسم مطلوب.",
  settingsErrNameTooLong: "الاسم طويل جداً (80 حرفاً كحد أقصى).",
  settingsErrEmailMissing: "البريد الإلكتروني وكلمة المرور الحالية كلاهما مطلوبان.",
  settingsErrEmailInvalid: "هذا لا يبدو كعنوان بريد إلكتروني صحيح.",
  settingsErrEmailCurrentWrong: "كلمة المرور الحالية غير صحيحة.",
  settingsErrEmailTaken: "هذا البريد الإلكتروني مستخدم بالفعل.",
  settingsErrEmailNoPassword: "حسابك لا يحتوي على كلمة مرور بعد — اضبط واحدة أولاً، ثم غيّر بريدك الإلكتروني.",
  changePassword: "تغيير كلمة المرور",
  changePasswordTitle: "تغيير كلمة المرور",
  changePasswordIntro: "حدّث كلمة المرور التي تستخدمها لتسجيل الدخول. ستبقى مسجلاً في هذا الجهاز؛ ستحتاج إلى كلمة المرور الجديدة في المرة القادمة لتسجيل الدخول.",
  currentPassword: "كلمة المرور الحالية",
  newPasswordLabel: "كلمة المرور الجديدة (6 أحرف على الأقل)",
  confirmNewPassword: "تأكيد كلمة المرور الجديدة",
  changePasswordSubmit: "تغيير كلمة المرور",
  passwordChanged: "تم تغيير كلمة المرور.",
  pwdErrCurrent: "كلمة المرور الحالية غير صحيحة.",
  pwdErrShort: "يجب أن تكون كلمة المرور الجديدة 6 أحرف على الأقل.",
  pwdErrMismatch: "كلمة المرور الجديدة والتأكيد غير متطابقتين.",
  pwdErrMissing: "جميع الحقول مطلوبة.",
  roleOWNER: "المالك",
  roleADVISOR: "مستشار الخدمة",
  roleTECH: "الفني",
  roleCASHIER: "أمين الصندوق",
  roleMASTER: "المشرف العام",
  tabDashboard: "لوحة التحكم",
  tabTeam: "الفريق",
  tabJobs: "المهام",
  tabIntake: "الاستقبال",
  tabEstimates: "التقديرات",
  advisorEstimatesTitle: "التقديرات",
  advisorEstimatesIntro: "التقديرات التي سعّرتها أو ما زلت تعمل عليها — تُسلَّم إلى أمين الصندوق لإصدار الفاتورة بعد موافقة العميل.",
  advisorEstSecJobsToPrice: "بطاقات بانتظار التسعير",
  advisorEstSecJobsToPriceHint: "سلّمها الفني — ابدأ مسودة تقدير.",
  advisorEstSecDrafts: "مسودات",
  advisorEstSecDraftsHint: "قيد العمل — أكمل التسعير وأرسل للعميل.",
  advisorEstSecSent: "مرسلة",
  advisorEstSecSentHint: "بانتظار قرار العميل عبر واتساب.",
  advisorEstSecApproved: "موافق عليها",
  advisorEstSecApprovedHint: "وافق العميل — سيُصدر أمين الصندوق الفاتورة بعد اكتمال العمل.",
  advisorEstSecRejected: "مرفوضة",
  advisorEstSecRejectedHint: "رفض العميل — أعد التسعير بمسودة جديدة.",
  advisorEstEmpty: "لا يوجد شيء هنا.",
  tabBookings: "الحجوزات",
  tabWorkshop: "الورشة",
  tabAccounts: "الحسابات",
  tabInventory: "المخزون",
  tabSuppliers: "الموردون",
  tabPurchasing: "المشتريات",
  tabBilling: "الاشتراك",
  // inventory — parts catalog (Phase 1)
  inventoryTitle: "مخزون القطع",
  inventorySubtitle: "كتالوج القطع ومستويات المخزون.",
  addPart: "إضافة قطعة",
  partSku: "رقم القطعة",
  partName: "الوصف",
  partCost: "التكلفة",
  partPrice: "السعر",
  partStock: "المتوفر",
  partReorderLevel: "إعادة الطلب عند",
  openingStock: "المخزون الافتتاحي",
  noPartsYet: "لا توجد قطع بعد. أضف أول قطعة أعلاه.",
  lowStockTag: "منخفض",
  partAutoBadge: "تلقائي",
  savePart: "حفظ القطعة",
  // inventory 1b — edit part + manual stock adjustment
  editPart: "تعديل القطعة",
  saveChanges: "حفظ التغييرات",
  save: "حفظ",
  backToInventory: "← المخزون",
  adjustStock: "تعديل المخزون",
  currentStock: "المخزون الحالي",
  adjustDirection: "إضافة أو خصم",
  adjustAdd: "إضافة",
  adjustRemove: "خصم",
  adjustQty: "الكمية",
  adjustReason: "السبب",
  adjustReasonHint: "مثال: استلام، تالف، تصحيح",
  applyAdjustment: "تطبيق التعديل",
  recentMovements: "آخر تغييرات المخزون",
  noMovements: "لم تُسجَّل تغييرات مخزون بعد.",
  movementWhen: "الوقت",
  movementChange: "التغيير",
  // inventory OCR import — photograph a supplier invoice → review → catalog
  importFromPhoto: "استيراد من صورة الفاتورة",
  importFromPhotoHint: "صوّر فاتورة المورد — نقرأ البنود وتراجعها قبل الحفظ.",
  importScan: "مسح الفاتورة",
  importReviewTitle: "مراجعة القطع المستوردة",
  importDetected: "بنود مكتشفة",
  importFlagged: "تحتاج تدقيق",
  importDetected_zero: "لا يوجد بنود",
  importDetected_one: "بند واحد مكتشف",
  importDetected_two: "بندان مكتشفان",
  importDetected_few: "{n} بنود مكتشفة",
  importDetected_many: "{n} بندًا مكتشفًا",
  importDetected_other: "{n} بند مكتشف",
  importFlagged_zero: "لا شيء يحتاج تدقيق",
  importFlagged_one: "بند واحد يحتاج تدقيق",
  importFlagged_two: "بندان يحتاجان تدقيق",
  importFlagged_few: "{n} بنود تحتاج تدقيق",
  importFlagged_many: "{n} بندًا يحتاج تدقيق",
  importFlagged_other: "{n} بند يحتاج تدقيق",
  importFlaggedHint: "لم يكن التعرف الضوئي واثقًا من هذا البند — يرجى التحقق.",
  importInclude: "إضافة",
  importPartNo: "رقم القطعة",
  importDescription: "الوصف",
  importNamePlaceholder: "الوصف",
  importSkuAuto: "تلقائي",
  importConfirm: "تأكيد والحفظ في الكتالوج",
  importConfirmHint: "تُحفظ الصفوف المحددة فقط. لم يُحفظ شيء حتى الآن.",
  importDiscard: "تجاهل هذا الاستيراد",
  importNoLines: "لم تُقرأ أي بنود من هذه الصورة. تجاهلها وأضف القطع يدويًا، أو جرّب صورة أوضح.",
  importSourcePhoto: "الصورة المصدر",
  importCreated: "تم استيراد {n} قطعة إلى الكتالوج.",
  importSkipped: "تم تخطي {n} (اسم فارغ أو رمز مكرر).",
  // singular forms (n === 1)
  importDetectedOne: "بند مكتشف",
  importFlaggedOne: "يحتاج تدقيق",
  importCreatedOne: "تم استيراد قطعة واحدة إلى الكتالوج.",
  importStatus_CONFIRMED: "تم تأكيد هذا الاستيراد — القطع في كتالوجك.",
  importStatus_DISCARDED: "تم تجاهل هذا الاستيراد.",
  // inventory 1c — supplier directory
  suppliersTitle: "الموردون",
  suppliersSubtitle: "من تشتري منهم القطع.",
  supplierName: "اسم المورد",
  supplierContact: "الشخص المسؤول",
  supplierPhone: "الهاتف",
  supplierEmail: "البريد الإلكتروني",
  supplierTrn: "الرقم الضريبي",
  supplierAddress: "العنوان",
  supplierParts: "القطع",
  saveSupplier: "حفظ المورد",
  noSuppliersYet: "لا يوجد موردون بعد. أضف أول مورد أعلاه.",
  suppliersInactive: "المُزالون",
  backToSuppliers: "← الموردون",
  editSupplier: "تعديل المورد",
  supplierInactiveTag: "مُزال",
  supplierPartsLinked: "قطعة مرتبطة",
  deactivateSupplier: "إزالة المورد",
  deactivateSupplierHint: "يُزيله من قوائمك. يبقى السجل محفوظًا ويمكنك استعادته في أي وقت.",
  restoreSupplier: "استعادة المورد",
  restoreSupplierHint: "إعادة تفعيل هذا المورد.",
  deactivate: "إزالة",
  partSupplier: "المورد",
  partSupplierNone: "— بدون —",
  // inventory 1d — dashboard low-stock reorder shortlist
  lowStockHeading: "مخزون منخفض",
  viewInventory: "عرض المخزون",
  lowStockNone: "كل القطع أعلى من حد إعادة الطلب.",
  moreLowStock: "أخرى تحت حد الطلب",
  // inventory 2a — purchasing (purchase orders)
  purchasingTitle: "المشتريات",
  documentPurchaseOrder: "أمر شراء",
  documentPurchaseOrderDraft: "أمر شراء (مسودة)",
  documentRfq: "طلب عرض سعر",
  printPo: "طباعة",
  sendViaWhatsApp: "إرسال عبر واتساب",
  supplierNoPhoneReason: "رقم هاتف المورد غير مسجّل — أضفه في بيانات المورد لتفعيل الإرسال عبر واتساب.",
  sendHistoryTitle: "سجل الإرسال",
  sendHistoryEmpty: "لا يوجد سجل إرسال بعد.",
  sendChannel_WHATSAPP: "واتساب",
  sendChannel_EMAIL: "بريد إلكتروني",
  sendDocChip_PO: "أمر شراء",
  sendDocChip_RFQ: "طلب عرض سعر",
  sendStatus_HANDED_OFF_whatsapp: "فُتح في واتساب — لم يتم تأكيد التسليم",
  sendStatus_HANDED_OFF_email: "تم التسليم لمزوّد البريد — لم يتم تأكيد التسليم",
  sendStatus_SENT_email: "تم الإرسال بالبريد",
  sendStatus_FAILED: "فشل",
  emailSentOk: "تم الإرسال بالبريد الإلكتروني.",
  emailSentError: "فشل إرسال البريد الإلكتروني.",
  emailErr_generic: "حاول مرة أخرى بعد قليل. إذا استمرت المشكلة، تواصل مع الدعم.",
  emailErr_no_recipient: "لا يوجد بريد إلكتروني مسجّل لهذا المورد — أضفه في بيانات المورد.",
  emailErr_unverified_domain: "نطاق الإرسال غير مُوثَّق بعد. اطلب من المسؤول إكمال توثيق النطاق.",
  emailErr_not_configured: "الإرسال بالبريد الإلكتروني غير مُهيَّأ لهذه الورشة بعد.",
  emailErr_provider_error: "أرجع مزوّد البريد الإلكتروني خطأً. تم تسجيله؛ حاول مرة أخرى بعد قليل.",
  printedOn: "تاريخ الطباعة",
  printSupplierContact: "بيانات المورد",
  printTotalsLabel: "الإجماليات",
  purchasingSubtitle: "أوامر الشراء إلى مورديك.",
  newPurchaseOrder: "أمر شراء جديد",
  newQuotation: "طلب عرض سعر جديد",
  newQuotationHint: "اطلب من المورد تسعير قطع لم نحصل على عرض لها بعد.",
  newPurchaseOrderHint:
    "أنشئ أمر شراء بأسعار معروفة مسبقًا — لمورد ثابت لديك قائمة أسعاره.",
  createQuotation: "إنشاء طلب عرض سعر",
  createPurchaseOrder: "إنشاء أمر شراء",
  orderModeCostRequired: "هذا أمر شراء — أدخل السعر من قائمة أسعار المورد.",
  purchasingOpen: "مفتوحة",
  purchasingClosed: "مغلقة",
  noPurchaseOrders: "لا توجد أوامر شراء بعد.",
  poLineStaleError: "قام شخص آخر بتغيير هذا السطر أثناء فتحه لديك. يعرض الصف أدناه القيم الحالية — أعد تطبيق تعديلك إذا كنت لا تزال ترغب في التغيير.",
  poLineNotFoundError: "تم حذف هذا السطر أثناء فتحه لديك. أعد تحميل الصفحة لعرض الأسطر الحالية.",
  notFoundTitle: "غير موجود",
  notFoundBody: "هذه الصفحة غير موجودة أو انتهت صلاحية الرابط.",
  notFoundGoHome: "الصفحة الرئيسية",
  poDocNumberCol: "رقم المستند",
  poKindCol: "النوع",
  poVehicleCol: "المركبة / المهمة",
  poCreatedCol: "تاريخ الإنشاء",
  poLastSentCol: "آخر إرسال",
  poVehicleMultiple: "{count} مركبات",
  poNotSent: "لم يُرسل",
  poWaitingForQuote: "في انتظار العرض منذ {days} يومًا",
  purchasingFilterKindLabel: "النوع:",
  purchasingFilterKindAll: "الكل",
  purchasingFilterKindPO: "أمر شراء",
  purchasingFilterKindRFQ: "طلب عرض سعر",
  purchasingFilterSentLabel: "الإرسال:",
  purchasingFilterSentAll: "الكل",
  purchasingFilterSentSent: "مُرسل",
  purchasingFilterSentUnsent: "لم يُرسل",
  poSupplier: "المورد",
  poReference: "المرجع",
  supplierRef: "مرجع المورّد",
  poStatus: "الحالة",
  poLines: "البنود",
  poTotal: "الإجمالي",
  poUnitCost: "سعر الوحدة",
  poLineTotal: "إجمالي البند",
  poNote: "ملاحظة",
  poStatus_DRAFT: "مسودة",
  poStatus_ORDERED: "تم الطلب",
  poStatus_PARTIALLY_RECEIVED: "مُستلَم جزئيًا",
  poStatus_RECEIVED: "تم الاستلام",
  poStatus_CANCELLED: "ملغى",
  poStatusDraftSent_RFQ: "أُرسل — بانتظار عرض السعر",
  poStatusDraftSent_PO: "أُرسل — بانتظار تأكيد الطلب",
  // Arabic tab labels — parallel construction to the existing status
  // pills. "بحاجة للإرسال" preserves the imperative tone of "To send"
  // (needs sending, YOUR action). "بانتظار المورّد" matches the
  // detail-page "بانتظار عرض السعر" pattern the operator already
  // sees on the DRAFT-sent status pill.
  poTab_UNSENT_DRAFT: "بحاجة للإرسال",
  poTab_AWAITING_SUPPLIER: "بانتظار المورّد",
  backToPurchasing: "← المشتريات",
  noSuppliersForPo: "أضف موردًا أولًا في",
  choosePlaceholder: "اختر…",
  createDraftPo: "إنشاء مسودة أمر",
  // Estimate → PO conversion (docs/Estimate-to-PO-Spec.md)
  convertFromEstimate: "تحويل من عرض السعر",
  convertFromEstimateTitle: "تحويل عرض السعر إلى أمر شراء",
  convertFromEstimateHint: "ابحث عن وظيفة بالرقم، اختر القطع التي تريد شراءها، ثم حدد المورد.",
  jobCardNumberLabel: "رقم بطاقة العمل",
  jobCardNumberPlaceholder: "مثال: 42 أو JC-2026-0042",
  lookupJob: "بحث",
  jobCardNotFound: "لم يتم العثور على بطاقة العمل في هذا المتجر.",
  jobConfirmHeader: "الوظيفة",
  estimateNone: "لا يوجد عرض سعر لهذه الوظيفة بعد. اطلب من الاستشاري تسعيرها أولاً.",
  estimateAllRejected: "لهذه الوظيفة {count} عرض سعر، لكن رفضها العميل جميعاً.",
  estimateMultipleHeader: "توجد عدة عروض أسعار لهذه الوظيفة — اختر واحداً للتحويل:",
  estimateLinesCount: "{count} بند",
  estimateUseThis: "استخدم هذا ←",
  estimateStatus_APPROVED: "معتمد",
  estimateStatus_SENT: "مُرسَل",
  estimateStatus_DRAFT: "مسودة",
  estimateStatus_REJECTED: "مرفوض",
  estimateChosenHeader: "عرض السعر المستخدم",
  estimateChosenApproved: "مُعتمد بتاريخ {date}",
  estimateChosenSent: "مُرسَل بتاريخ {date}",
  estimateChosenDraft: "مسودة، آخر تعديل {date}",
  partsToConvertHeader: "القطع المطلوب تضمينها",
  partsToConvertHint: "التكلفة تُعبّأ من مخزونك عند توفرها، وإلا فمن التقدير — الوسم أسفل كل تكلفة يبيّن المصدر. الأسعار الظاهرة للعميل لا تُستخدم. عدّل الرقم إن اختلف عن سعر المورد.",
  noConvertibleParts: "لا يحتوي هذا العرض على قطع قابلة للتحويل.",
  skippedNoInventoryHeader: "{count} قطعة متجاهَلة — لا يوجد ربط بالمخزون",
  skippedNoInventoryHint: "أضفها إلى المخزون أولاً",
  goToInventory: "الذهاب إلى المخزون →",
  // See the EN comment — panel removed 2026-07-31; the read-only skip
  // notice replaces the "Add these to inventory" flow.
  skippedFreeTextHeader:
      "{count} قطعة كنص حر — غير موجودة في الكتالوج",
  skippedFreeTextHint:
      "هذه البنود بدون قطعة مربوطة، فلا يمكن إدراجها في أمر الشراء. أنشئ القطعة في المخزون أولاً (أو عدّل بند عرض السعر لربطه بقطعة موجودة)، ثم أعد تحويل عرض السعر.",
  colSku: "الرمز",
  colName: "الاسم",
  colCost: "التكلفة",
  colPrice: "السعر",
  skippedDeclinedHeader: "{count} قطعة رفضها العميل",
  createDraftPoFromEstimate: "إنشاء مسودة أمر شراء",
  createRfqFromEstimate: "إنشاء طلب عرض سعر",
  poRequiresAllPricedReason:
    "أمر الشراء يتطلب سعرًا لكل بند مضمّن — {n} بدون سعر حتى الآن. أدخل السعر، أو انقر إنشاء طلب عرض سعر لإرسال طلب بدلاً من ذلك.",
  lineUnpricedTag: "برجاء إفادتنا بالسعر",
  approxTotal: "إجمالي تقريبي",
  costSourceCatalogue: "من الكتالوج",
  costSourceEstimate: "من التقدير",
  addPoLine: "إضافة بند",
  addPoLinePlaceholder: "اسم القطعة، أو اكتب وصفًا حرًا",
  addPoLineHint:
    "اختر من الكتالوج، أو اكتب وصفًا حرًا لقطعة غير موجودة في المخزون.",
  poDefaultVehicleLegend: "لأي مركبة؟",
  hydrateJobLegend: "تحميل قطع من مهمة (اختياري)",
  hydrateJobLoad: "تحميل",
  hydrateJobLoading: "جاري التحميل…",
  hydrateJobClear: "مسح",
  hydrateJobLoaded: "تم تحميل {count} قطعة من {number} — {vehicle}. يمكنك التعديل أو الحذف قبل الإرسال.",
  hydrateJobNotFound: "بطاقة المهمة رقم {n} غير موجودة في هذا الجراج. راجع الرقم وحاول مجددًا.",
  hydrateJobEmpty: "لا توجد طلبات قطع مفتوحة على هذه المهمة. أضف بنودًا يدويًا أدناه، أو استخدم مهمة أخرى.",
  hydrateJobError: "تعذّر تحميل القطع. يرجى المحاولة مجددًا.",
  hydrateJobColDescription: "القطعة",
  hydrateJobColQty: "الكمية",
  hydrateJobColRemove: "حذف البند",
  hydrateJobSourceFindings: "من ملاحظات الفني",
  hydrateJobSourceRequest: "طلب قطعة",
  poDefaultVehicleHint:
    "اختر لوحة موجودة في المرآب، أو اكتب الصنع والموديل لسيارة غير مسجّلة. كل الحقول اختيارية — عادةً ما يعرف الاستشاري الصنع والموديل فقط عند طلب عرض السعر.",
  vehiclePlateLabel: "اللوحة",
  vehiclePlatePlaceholder: "مثال: A 12345",
  vehicleMakeLabel: "الصنع",
  vehicleModelLabel: "الموديل",
  vehicleYearLabel: "السنة",
  vehicleEngineLabel: "المحرك",
  vehicleVinLabel: "رقم الهيكل",
  vehicleJobNumberLabel: "رقم بطاقة العمل",
  vehicleJobNumberPlaceholder: "مثال: 42",
  techHoursSessionsCol: "الجلسات (بداية ← نهاية)",
  techHoursSessionLive: "قيد التنفيذ",
  movementFromPo: "من أمر الشراء",
  poMovementsTitle: "حركات المخزون من هذا الأمر",
  poMovementsKind: "النوع",
  poMovementsReceipt: "استلام",
  poMovementsReturn: "إرجاع",
  vehicleMatchLabel: "سجل المرآب المطابق:",
  vehicleMatchDismiss: "تجاهل المطابقة",
  customerDetailKicker: "العميل",
  customerEditLegend: "بيانات العميل",
  customerEditHint:
    "هذه الحقول تخصّ العميل وليس السيارة — تحديثها هنا يسري على كل مركباته دفعة واحدة.",
  customerNameLabel: "الاسم",
  customerPhoneLabel: "الهاتف",
  customerPhoneNeedsReview:
    "تعذّر قراءة هذا الرقم كرقم جوال صحيح. سترتد رسائل واتساب إلى شاشة اختيار المستلم حتى يُصحَّح.",
  customerPhonePatternHint:
    "أرقام فقط، مع إمكانية بدء الرقم بـ + أو 0. المسافات والشرطات مسموح بها. جوال إماراتي: 0501234567 أو +971501234567.",
  customerSaveButton: "حفظ",
  customerVehiclesHeading: "المركبات",
  customerNoVehicles: "لا توجد مركبات لهذا العميل بعد.",
  customerJobsHeading: "سجل الزيارات",
  customerNoJobs: "لا توجد زيارات بعد.",
  customerTotalPaid: "إجمالي المدفوع:",
  poAddLineVehicleFor: "لـ",
  poAddLineVehicleChange: "تغيير",
  poAddLineVehicleUseDefault: "استخدم افتراضي المستند",
  poAddLineNoDefaultVehicle: "لا توجد مركبة على المستند. أضف واحدة لكل بند عند الحاجة.",
  noPoLines: "لا توجد بنود بعد. أضف قطعة أعلاه.",
  noPartsForPo: "أضف قطعة إلى الكتالوج أولًا في",
  markOrdered: "تحديد كمطلوب",
  markOrderedNeedsPricesReason:
    "يجب إدخال سعر المورد لكل بند قبل الطلب. الحقل الفارغ يعني أن العرض لم يصل بعد؛ السعر صفر مقبول للبنود ضمن الضمان أو المجاملة.",
  cancelPo: "إلغاء الأمر",
  poReceivingSoon: "استلام المخزون قادم لاحقًا.",
  // 2b الاستلام الجزئي
  poOrdered: "المطلوب",
  poReceived: "المُستلَم",
  noVehicleLinkedShort: "—",
  noVehicleLinkedReason:
    "لا توجد مركبة مرتبطة. هذا الصنف غير مرتبط ببطاقة عمل (مثل مخزون سابق).",
  poOutstanding: "المتبقّي",
  poReceivedLower: "مُستلَم",
  poOutstandingLower: "متبقّي",
  poReceiveHeading: "استلام التوريد",
  poReceiveHint: "أدخل الكمية التي وصلت الآن من كل قطعة. أقل من المطلوب مقبول — استلم الباقي لاحقًا.",
  poReceiveNow: "استلام الآن",
  poReceiveButton: "استلام في المخزون",
  // Direct-fit receive keys (AR 2026-08-16).
  poReceiveDirectOption: "تركيب مباشر (يُركّب على السيارة، ليس للمخزون)",
  poReceiveDirectHelp:
    "قطعة مشتراة لسيارة محددة وتُركّب فورًا، لا تدخل المخزون. تُسجَّل تكلفة المورّد على هذه المهمة.",
  poReceiveStockOption: "قطعة مخزنية (إضافة للمخزون)",
  poReceiveStockHelp:
    "المستهلكات التي تحتفظ بها في المخزن لعدة مهام. اربط هذا البند بقطعة من الكتالوج من صفحة تعديل البند أولاً، ثم عد إلى هنا.",
  poReceiveDirectCostLabel: "التكلفة الفعلية للوحدة",
  poReceiveDirectPartNoLabel: "رقم قطعة المورّد (اختياري)",
  poReceiveCatalogueHint:
    "لديك قطعة في الكتالوج باسم «{name}» ({sku}) — هل هي نفسها؟ إن كانت كذلك، اختر «قطعة مخزنية».",
  poReceivedBanner: "تم الاستلام بالكامل في",
  // 2c مرتجعات الشراء
  poReturned: "المُرتجَع",
  poReturnedLower: "مُرتجَع",
  poReturnableLower: "قابل للإرجاع",
  poReturnHeading: "إرجاع إلى المورد",
  poReturnHint: "أدخل الكمية المُراد إرجاعها من كل قطعة مستلَمة. سينخفض المخزون بهذا القدر، ولا يمكن إرجاع أكثر مما استُلم.",
  poReturnNow: "إرجاع الآن",
  poReturnButton: "إرجاع إلى المورد",
  billing: "الاشتراك والخطة",
  statusLabel: "الحالة",
  renewsLabel: "يتجدد",
  perMonth: "شهر",
  onPilotNotBilled: "أنت في مرحلة تجريبية — دون فوترة.",
  billingManual: "خلال المرحلة التجريبية، تتم الفوترة يدويًا من Garage Os — لن تُحصّل أي مبالغ عبر التطبيق.",
  billingNotConfigured: "لم تُهيّأ الفوترة بعد.",
  subscribe: "اشترك",
  cancelSubscription: "إلغاء الاشتراك",
  tabChats: "المحادثات",
  tabWhatsapp: "واتساب",
  tabMore: "المزيد",
  tabBranches: "الفروع",
  branchesTitle: "الفروع",
  branchesIntro: "لكل فرع طاقمه ورقم واتساب وطابوره الخاص. تراها هنا مجمّعة.",
  addBranch: "إضافة فرع",
  branchName: "اسم الفرع",
  branchMainTag: "الرئيسي",
  assignBranchLabel: "الفرع",
  perBranchBilling: "الفوترة لكل فرع — كل فرع مقعد اشتراك واحد.",
  branchError: "أدخل اسم الفرع.",
  chats: "المحادثات",
  noChats: "لا توجد محادثات بعد.",
  needsHuman: "يحتاج تدخّلًا",
  humanMode: "أنت",
  botMode: "الذكاء",
  send: "إرسال",
  takeOver: "تولّي المحادثة",
  release: "إعادة للذكاء",
  pendingDraft: "مسودة من الذكاء — وافق قبل الإرسال",
  approveSend: "موافقة وإرسال",
  discard: "تجاهل",
  message: "رسالة",
  chatSimulatedTag: "محاكاة",
  draftUnfilledTitle: "لا يمكن إرسال هذه المسودة بعد",
  draftUnfilledBody: "لا تزال هذه المسودة تحتوي على «___» أو علامة «[مسودة]». أدخل القيمة الفعلية أو أزل العلامة، ثم وافق مرة أخرى.",
  waConnect: "ربط واتساب",
  waConnectIntro: "اربط رقم واتساب الخاص بورشتك لإرسال الرسائل من رقمك.",
  waConnected: "متصل",
  waDisconnect: "فصل",
  waConnectHint: "في الإنتاج يفتح هذا نافذة تسجيل واتساب. حاليًا أدخل رقمًا لمحاكاة الربط.",
  waConnectBtn: "ربط واتساب",
  waConnectBtnSandbox: "ربط (تجريبي)",
  waProdMode: "وضع الإنتاج (Meta BSP)",
  waSandboxMode: "الوضع التجريبي (بدون بيانات Meta)",
  waProdHint: "اضغط الزر أدناه لفتح نافذة تسجيل واتساب الرسمية من Meta. أضف رقم واتساب الأعمال داخل النافذة، وسيظل الاتصال محفوظًا بعد إغلاقها.",
  waHelpNeedCreds: "للتبديل إلى وضع الإنتاج، اضبط متغيرات البيئة META_WHATSAPP_APP_ID و META_WHATSAPP_CONFIG_ID و META_WHATSAPP_APP_SECRET (خادم) و NEXT_PUBLIC_META_WHATSAPP_APP_ID و NEXT_PUBLIC_META_WHATSAPP_CONFIG_ID (عميل) في مشروع Vercel، ثم أعد النشر.",
  waStatusTitle: "واتساب",
  waStatusSubtitleAdvisor: "حالة الاتصال والرسائل الصادرة الأخيرة. الربط أو الفصل من إعدادات المالك.",
  waStatusSubtitleCashier: "حالة الاتصال والرسائل الصادرة الأخيرة من الورشة. الربط أو الفصل من إعدادات المالك.",
  waStatusNotConnected: "غير متصل",
  waStatusAskOwner: "يمكن للمالك ربط الرقم من صفحة المالك ← واتساب.",
  waStatusReadyAdvisor: "الاتصال مفعّل. التذكيرات التي ترسلها ستذهب من هذا الرقم.",
  waStatusReadyCashier: "الاتصال مفعّل. الفواتير والتقديرات التي ترسلها ستذهب من هذا الرقم.",
  waStatusOwnerControls: "الربط أو الفصل من صلاحيات المالك على",
  waRecentOutbound: "الرسائل الصادرة الأخيرة",
  waRecentEmpty: "لم يتم إرسال أي رسائل بعد.",
  waStatusGoReminders: "اذهب إلى التذكيرات ←",
  waStatusGoChats: "اذهب إلى المحادثات ←",
  waStatusGoInvoices: "اذهب إلى الفواتير ←",
  waStatusGoEstimates: "اذهب إلى التقديرات ←",

  // friendly status badges (ar)
  fs_WAITING_FOR_TECH: "بانتظار الفني",
  fs_TECH_DIAGNOSING: "الفني يفحص",
  fs_ESTIMATE_UNDER_PROCESS: "تحضير التقدير",
  fs_AWAITING_CUSTOMER_APPROVAL: "بانتظار موافقة العميل",
  fs_APPROVED_IN_PROGRESS: "العمل قيد التنفيذ بعد الموافقة",
  fs_EXTRA_WORK_AWAITING_APPROVAL: "أعمال إضافية — بانتظار الموافقة",
  fs_COMPLETE_AWAITING_INVOICE: "اكتمل العمل — بانتظار الفاتورة",
  fs_AWAITING_PAYMENT: "بانتظار الدفع",
  fs_WAITING_TECH_COMPLETE: "بانتظار الفني: يجب تعليم العمل مكتملاً بصورة أو ملاحظات",
  fs_READY_FOR_PICKUP: "جاهزة للاستلام",
  fs_COMPLETE: "تم الاستلام",
  fs_ON_HOLD: "معلّقة",
  fs_CANCELLED: "ملغاة",
  st_ARRIVED: "وصلت",
  st_INSPECTION: "الفحص",
  st_ESTIMATE: "التقدير",
  st_APPROVED: "موافَق عليه",
  st_REPAIR: "الإصلاح",
  st_TECH_COMPLETE: "اكتمل عمل الفني",
  st_INVOICED: "تمت الفوترة",
  st_DELIVERED: "تم التسليم",
  st_ON_HOLD: "معلّقة",
  st_CANCELLED: "ملغاة",

  signInTitle: "تسجيل دخول الموظفين",
  email: "البريد الإلكتروني",
  password: "كلمة المرور",
  signIn: "تسجيل الدخول",
  invalid: "البريد الإلكتروني أو كلمة المرور غير صحيحة.",
  garageCreated: "تم إنشاء الورشة — سجّل الدخول بحساب المالك.",
  newGarageQ: "ورشة جديدة؟",
  setOneUp: "أنشئ واحدة",

  signupTitle: "أنشئ ورشتك",
  signupIntro: "يُنشئ ورشتك وحساب المالك.",
  garageName: "اسم الورشة",
  vatTrnOptional: "الرقم الضريبي (اختياري)",
  yourNameOwner: "اسمك (المالك)",
  passwordMin6: "كلمة المرور (6 أحرف على الأقل)",
  createGarage: "إنشاء الورشة",
  alreadySetup: "لديك حساب؟",
  emailExists: "هذا البريد مسجّل بالفعل.",
  fillAllFields: "يرجى ملء جميع الحقول (كلمة المرور 6 أحرف على الأقل).",

  team: "الفريق",
  addStaff: "إضافة موظف",
  name: "الاسم",
  tempPassword: "كلمة مرور مؤقتة (6+)",
  add: "إضافة",
  remove: "إزالة",
  ownerTag: "المالك",
  optAdvisor: "مستشار",
  optTechnician: "فني",
  optCashier: "أمين الصندوق",
  optMaster: "المشرف العام",
  estimatePrint: "🖨 طباعة عرض السعر",
  estimateViewPrint: "عرض / طباعة عرض السعر",
  // Estimate-honesty pass — mirror invoice keys (AR 2026-08-15).
  estimateSentAt: "تم التسليم إلى واتساب في",
  estimateDeliveredAt: "استلمه العميل في",
  estimateHandedOffBanner:
    "تم التسليم إلى واتساب — اضغط إرسال داخل واتساب على هاتفك. لم يستلمه العميل بعد، وما زال بإمكانك تعديل التقدير.",
  estimateDeliveredBanner:
    "تم استلامه من قبل العميل. ما زال بإمكانك تعديل التقدير — سيلزم إرسال نسخة جديدة.",
  imOnThisCar: "أعمل على هذه السيارة",
  onThisCarNow: "على هذه السيارة",
  floorNow: "الورشة الآن",
  floorNowEmpty: "لا أحد يعمل على سيارة الآن.",
  floorNowIdle: "بدون سيارة",
  floorNowUnavailable: "الأرضية المباشرة غير متاحة الآن — حاول بعد لحظات.",
  workshopTime: "وقت الورشة",
  workshopTimeSummary: "{time} عبر {n} جلسات",
  workshopTimeNone: "لم يُسجَّل وقت ورشة بعد.",
  workshopTimeLive: "مباشر",
  workshopTimeStale: "جلسة طويلة بشكل غير معتاد",
  wrenchTime: "وقت العمل الفعلي",
  wrenchDay: "اليوم",
  wrenchWeek: "هذا الأسبوع",
  wrenchCar: "سيارة",
  wrenchCars: "سيارات",
  wrenchAvg: "المعدل / سيارة",
  wrenchStaleCount: "{n} جلسة طويلة",
  // Arabic CLDR plural categories for "session" (جلسة).
  wrenchStaleCount_one: "جلسة طويلة واحدة",
  wrenchStaleCount_two: "جلستان طويلتان",
  wrenchStaleCount_few: "{n} جلسات طويلة",
  wrenchStaleCount_many: "{n} جلسة طويلة",
  wrenchStaleCount_other: "{n} جلسة طويلة",
  wrenchExcludedCount: "{n} جلسة مستبعدة ({mins}) — أطول من 8 ساعات، وقت عمل غير معروف",
  viewHours: "عرض الساعات",
  techHoursTitle: "سجل العمل",
  techHoursRange: "الفترة",
  techHoursWeek: "هذا الأسبوع",
  techHoursMonth: "هذا الشهر",
  techHoursTotalTime: "إجمالي الوقت",
  techHoursTotalCars: "سيارات",
  techHoursAvgDay: "المعدل / يوم",
  techHoursDays: "أيام نشطة",
  techHoursJobCol: "أمر #",
  techHoursJob: "أمر #{n}",
  techHoursNone: "لا توجد جلسات عمل في هذه الفترة.",
  techHoursBack: "رجوع",
  techHoursLookupTitle: "ساعات الفنيين",
  techHoursLookupIntro: "استعرض سجل العمل اليومي لأي فني.",
  techHoursPickTech: "الفني",
  techHoursPickFrom: "من",
  techHoursPickTo: "إلى",
  techHoursLookupBtn: "عرض الساعات",
  techHoursNoTechs: "لا يوجد فنيون في هذا المركز بعد.",
  tabHours: "الساعات",
  teamError: "تحقق من الحقول (كلمة المرور 6+).",

  bookTitle: "احجز خدمة",
  bookIntro: "أخبرنا بالمشكلة بكلماتك — وسنقترح الحل.",
  bookPhotoErrorTitle: "لم يتم قبول الصورة",
  bookPhotoErr_generic: "جرّب صورة أخرى، أو تابع بدون صورة.",
  bookPhotoErr_EMPTY: "ملف الصورة فارغ.",
  bookPhotoErr_TOO_LARGE: "حجم الصورة كبير جدًا — الحد الأقصى 5 ميغابايت.",
  bookPhotoErr_BAD_MIME: "نوع الملف غير مدعوم. استخدم PNG أو JPEG أو WEBP.",
  bookPhotoErr_BAD_MAGIC: "الملف لا يبدو أنه صورة حقيقية. جرّب ملفًا آخر.",
  bookPhotoErr_MIME_MISMATCH: "نوع الصورة لا يطابق محتواها. جرّب ملفًا آخر.",
  yourName: "الاسم",
  phone: "الهاتف",
  make: "الماركة",
  model: "الموديل",
  plate: "اللوحة",
  describe: "صف المشكلة بكلماتك (مثال: المكيّف لا يبرّد عند الحرارة)",
  optionalPhoto: "صورة اختيارية",
  getProposal: "احصل على اقتراح",
  thanksGotIt: "شكرًا — استلمنا طلبك",
  youToldUs: "لقد أخبرتنا:",
  ourFirstRead: "قراءتنا الأولية:",
  suggested: "المقترح",
  urgencyLabel: "الأولوية",
  bookingConfirmed: "✅ أكّد المستشار حجزك.",
  bookingRejected: "سيتصل بك المستشار للمناقشة.",
  bookingPending: "🕒 سيؤكّد المستشار قريبًا. هذا اقتراح وليس عرضًا نهائيًا.",

  activeJobs: "المهام النشطة",
  newJobCard: "+ بطاقة مهمة جديدة",
  newBookings: "حجوزات جديدة",
  noActiveJobs: "لا توجد مهام نشطة. أنشئ واحدة لبدء المسار.",
  bookingsIntro: "اقترح الذكاء الاصطناعي — وأنت تؤكّد. التأكيد يُنشئ بطاقة مهمة.",
  confirmCreateJob: "تأكيد ← إنشاء مهمة",
  reject: "رفض",
  noPendingBookings: "لا توجد حجوزات معلّقة.",
  newJobTitle: "بطاقة مهمة جديدة",
  pickVehicle: "اختر المركبة التي وصلت.",
  start: "ابدأ ←",
  noVehicles: "لا توجد مركبات بعد. تصل عبر حجز العميل.",
  backActiveJobs: "← المهام النشطة",

  // new-customer intake (Moulkia OCR + plate lookup)
  newCustomerMoulkia: "عميل جديد (الملكية)",
  moulkiaHint: "صوّر الملكية — سنملأ المالك واللوحة والصنع والطراز والسنة تلقائيًا.",
  moulkiaConsent: "يوافق العميل على استخراج بيانات المركبة والمالك من صورة الملكية.",
  scanMoulkia: "مسح الملكية",
  repeatCustomer: "عميل متكرر (بحث باللوحة)",
  plateLookupHint: "أدخل رقم اللوحة — سنملأ البيانات من السجل الحالي.",
  lookupBtn: "بحث",
  orPickExisting: "أو اختر مركبة موجودة",
  confirmCustomerTitle: "تأكيد البيانات",
  confirmHint: "راجع البيانات المملوءة تلقائيًا، صحّح ما يلزم، ثم ابدأ المهمة.",
  ownerName: "اسم المالك",
  mobile: "رقم الجوال",
  yearLabel: "السنة",
  vinLabel: "رقم الهيكل",
  startJobCard: "بدء بطاقة المهمة",
  sendToTechnician: "إرسال إلى الفني ←",
  handoffTitle: "تم التحويل إلى الورشة",
  handoffShared: "البطاقة الآن في مجموعة الفنيين المشتركة — يمكن لأي فني متاح استلامها.",
  intakePanelTitle: "هذه اللوحة مسجّلة بالفعل",
  intakePanelSubtitle: "أدخلت {plate}. قارنها بالسجل الحالي أدناه واختر ما يناسب.",
  intakePanelRecordHeading: "السجل الحالي",
  intakePanelOwnerLabel: "المالك",
  intakePanelPhoneLabel: "الجوّال",
  intakePanelPlateLabel: "اللوحة",
  intakePanelVehicleLabel: "المركبة",
  intakePanelVinLabel: "الشاصي",
  intakePanelChoiceSameOwnerTitle: "نفس السيارة، نفس المالك",
  intakePanelChoiceSameOwnerSubtitle: "نحمّل بيانات {owner} ونكمل.",
  intakePanelChoiceNewOwnerTitle: "نفس السيارة، تغيّر المالك",
  intakePanelChoiceNewOwnerSubtitle: "نحتفظ بالسيارة وسجلها الكامل. نحدّث اسم المالك ورقم الجوّال.",
  intakePanelChoicePlateMovedTitle: "سيارة مختلفة — نفس رقم اللوحة",
  intakePanelChoicePlateMovedSubtitle: "هذه اللوحة كانت على سيارة {owner}. نُحرّرها ونبدأ إدخالاً جديداً لهذه السيارة.",
  intakePanelChoiceWrongPlateTitle: "لوحة خاطئة — إعادة البحث",
  intakePanelChoiceWrongPlateSubtitle: "نرجع ونصحّح رقم اللوحة.",
  intakeErrorDismiss: "إغلاق",
  handoffSpecific: "تم الإرسال إلى {name}. سيظهر الطلب في طابور ورشته.",
  handoffJobNo: "رقم بطاقة المهمة",
  handoffViewJob: "عرض البطاقة ←",
  handoffCreateAnother: "+ إنشاء بطاقة أخرى",
  handoffBackToActive: "← العودة إلى المهام النشطة",
  sendForEstimate: "إرسال للتقدير ←",
  sentToAdvisorTitle: "تم إرسال الطلب إلى مستشار الخدمة لإعداد التقدير",
  sentToAdvisorSubtitle: "سيقوم مستشار الخدمة بتحديد السعر وإرسال التقدير للعميل.",
  jobApprovedStartWorkTitle: "تمت الموافقة — ابدأ العمل",
  jobApprovedStartWorkSubtitle: "تمت الموافقة على التقدير. ابدأ بالعمل — أضف صورًا أو ملاحظات أو رسائل صوتية أثناء التقدم.",
  beginYourWorkHint: "سيتم تفعيل زر «اكتمل العمل» بعد أول صورة أو ملاحظة أو رسالة صوتية.",
  sentToCashierForInvoiceTitle: "أُرسل إلى أمين الصندوق لإصدار الفاتورة",
  sentToCashierForInvoiceSubtitle: "سيقوم أمين الصندوق بتجهيز الفاتورة النهائية.",
  jobCompleteNoActionTitle: "اكتملت المهمة — لا يوجد إجراء مطلوب",
  jobCompleteNoActionSubtitle: "تم إصدار الفاتورة وإغلاق المهمة. لا شيء يستوجب التنفيذ من جانب الفني.",
  jobCancelledTitle: "تم إلغاء هذه المهمة.",
  sentToCashierViewJob: "عرض البطاقة ←",
  sentToCashierBackToWorkshop: "← العودة إلى الورشة",
  errAlreadySentForEstimate: "هذه البطاقة موجودة لدى مستشار الخدمة أو في مرحلة لاحقة.",
  errNotYourJob: "ليست بطاقتك — فقط الفني الذي استلمها يمكنه إرسالها للتقدير.",
  errCannotComplete: "هذه البطاقة ليست في مرحلة العمل — لا يمكن تحديدها كمكتملة.",
  errCannotReestimate: "هذه البطاقة ليست في مرحلة العمل — لا يمكن إرسالها لإعادة التقدير.",
  sendForReestimate: "إرسال لإعادة التقدير ←",
  sentForReestimateTitle: "تم الإرسال لإعادة التقدير — أعمال إضافية",
  sentForReestimateSubtitle: "سيقوم مستشار الخدمة بتسعير الأعمال الإضافية وإرسال تقدير جديد للعميل للموافقة.",
  extrasPanelTitle: "أعمال إضافية مكتشفة (بانتظار الموافقة)",
  extrasPanelEmpty: "لا توجد إضافات بعد. أضف أي قطع أو مشكلات إضافية وجدتها أثناء العمل المعتمد.",
  extrasDescriptionPlaceholder: "قطعة أو مشكلة إضافية",
  extrasAdd: "أضف إضافة",
  extrasRemove: "إزالة الإضافة",
  extrasSendForApproval: "إرسال {count} إضافة للموافقة ←",
  extrasSendForApproval_one: "إرسال إضافة واحدة للموافقة ←",
  extrasSendForApproval_two: "إرسال إضافتين للموافقة ←",
  extrasSendForApproval_few: "إرسال {n} إضافات للموافقة ←",
  extrasSendForApproval_many: "إرسال {n} إضافة للموافقة ←",
  extrasSendForApproval_other: "إرسال {n} إضافة للموافقة ←",
  errExtraDescriptionRequired: "أضف وصفًا للقطعة أو المشكلة الإضافية.",
  errCannotAddExtra: "يمكن إضافة الإضافات فقط أثناء تنفيذ العمل المعتمد.",
  errNoExtras: "أضف عنصرًا إضافيًا واحدًا على الأقل قبل الإرسال للموافقة.",
  markCompleteAndSend: "إنهاء ←",
  markedCompleteTitle: "تم تحديدها كمكتملة — أُرسلت إلى أمين الصندوق",
  markedCompleteSubtitle: "سيقوم أمين الصندوق بإعداد الفاتورة وإرسالها للعميل.",
  markedCompleteCompletedAt: "اكتملت في",
  waSend: "إرسال عبر واتساب",
  tabAdvisor: "استشاري",
  waSendDisabled: "رقم هاتف العميل غير متوفر أو غير صحيح",
  sendInvoiceToCustomer: "إرسال عبر واتساب ←",
  invoicePreviewNote: "راجع الفاتورة كما يراها العميل قبل الإرسال.",
  invoicePreviewButton: "معاينة ←",
  invoiceViewPrint: "عرض / طباعة الفاتورة",
  invoicePreviewGoBack: "← العودة للتعديل",
  invoiceDownloadPdf: "تحميل PDF",
  invoiceDownloadPdfHint:
    "احفظ نسخة PDF — بنفس التنسيق الذي يراه العميل، جاهزة للإرفاق في أي مكان يقبل الملفات.",
  invoicePreviewMockNote:
    "«إرسال عبر واتساب» يفتح محادثة العميل على هاتفك مع الرسالة جاهزة. ما زال عليك الضغط على إرسال داخل واتساب — لن يستلم العميل شيئًا حتى تفعل ذلك.",
  invoiceSentTitle: "تم تسليم الفاتورة إلى واتساب",
  invoiceSentSubtitle:
    "افتح واتساب على هاتفك واضغط إرسال. لن يستلم العميل شيئًا حتى تفعل ذلك.",
  invoiceSentMockNote:
    "سجّل الدفع هنا بمجرد أن يدفع العميل. إذا قال إنه لم يستلم الفاتورة، أعد إرسالها من واتساب — الرسالة لا تزال في مسودّة المحادثة.",
  invoiceSentAt: "تم التسليم إلى واتساب في",
  invoiceDeliveredAt: "استلمها العميل في",
  invoiceHandedOffBanner:
    "تم التسليم إلى واتساب — اضغط إرسال داخل واتساب على هاتفك. لم يستلمها العميل بعد، وما زال بإمكانك تعديل الفاتورة.",
  invoiceDeliveredBanner:
    "تم استلامها من قبل العميل. تعديل البنود مقفل — استخدم إلغاء وإصدار بديل للتصحيح.",
  invoiceResendViaWhatsApp: "إعادة الإرسال عبر واتساب ←",
  invoiceBadgeVoid: "ملغاة",
  invoiceReplacesLabel: "بديلة عن",
  invoiceReplacedByLabel: "استُبدلت بـ",
  invoiceVoidedAt: "أُلغيت في",
  invoiceVoidCorrectHint:
    "تم تسليم هذه الفاتورة — البنود مقفلة. ألغِها لإصدار بديل مصحّح. تحتفظ الملغاة برقمها للتدقيق؛ يأخذ التصحيح الرقم التالي.",
  invoiceVoidCorrectButton: "إلغاء هذه الفاتورة",
  invoiceReissueHint:
    "تم إلغاء هذه الفاتورة. أصدر تصحيحًا يشمل ما كان يجب أن يظهر في الأصل — ستشير الفاتورة الجديدة إلى هذه ليتمكّن المدقق من تتبع المسار.",
  invoiceReissueButton: "إصدار تصحيح ←",
  invoiceAlreadySent:
    "تم تسليم الفاتورة إلى واتساب — اضغط إرسال في واتساب على هاتفك إن لم تكن قد فعلت ذلك.",
  invoiceBackToInvoice: "← العودة إلى الفاتورة",
  invoiceBackToCashier: "← العودة إلى الحسابات",
  durDiagnosis: "الفحص",
  durPricing: "التسعير",
  durSoFar: "حتى الآن",
  jobTimelineTitle: "مسار العمل",
  jobTimelineClaimedAt: "استلم الفني",
  jobTimelineSentForEstimateAt: "أُرسلت للتقدير",
  jobTimelineNotYet: "—",
  prefilledFromRecord: "مملوء تلقائيًا من السجل الحالي.",
  errConsent: "يرجى تأكيد موافقة العميل أولًا.",
  errNoFile: "اختر صورة الملكية.",
  errFields: "املأ المالك والجوال واللوحة والصنع والطراز.",
  errOcrFailed: "تعذّر قراءة الملكية — يرجى إدخال البيانات يدويًا.",
  errOcrBilling: "خدمة قراءة الملكية غير متاحة مؤقتًا — يرجى التواصل مع صاحب الورشة. أدخل بيانات هذا العميل يدويًا.",
  errOcrTemporary: "تعذّرت قراءة الملكية بسبب مشكلة مؤقتة. حاول مرة أخرى بعد لحظات، أو أدخل البيانات يدويًا.",
  errPlateBelongsToAnotherCustomer: "رقم اللوحة مسجّل بالفعل لعميل مختلف. تحقّق من الرقم الذي أدخلته.",
  tryAgain: "حاول مجددًا",
  // PhotoCapture
  takePhoto: "التقاط صورة",
  takePhotoMoulkia: "تصوير الملكية",
  takePhotoMoulkiaFront: "تصوير وجه الملكية",
  takePhotoMoulkiaBack: "تصوير ظهر الملكية",
  step1of2: "الخطوة 1 من 2 — الوجه (المالك واللوحة)",
  step2of2: "الخطوة 2 من 2 — الظهر (رقم الهيكل، الصنع، الطراز، السنة)",
  frontCaptured: "✓ تم تصوير الوجه",
  backOfMoulkia: "ظهر الملكية",
  skipBackLink: "تخطّي الظهر — سأدخل التفاصيل يدويًا",
  errOcrBackFailed: "تعذّر قراءة ظهر الملكية — يرجى إدخال بيانات المركبة يدويًا.",
  backSkipped: "تم تخطي ظهر الملكية — أدخل بيانات المركبة (رقم الهيكل، الصنع، الطراز، السنة) يدويًا.",
  optimizingPhoto: "جارٍ تحسين الصورة…",
  retake: "إعادة التصوير",
  usePhoto: "استخدام هذه الصورة ←",
  recordVoice: "تسجيل ملاحظة صوتية",
  fileTooBig: "حجم الصورة كبير جدًا — يرجى إعادة التصوير بدقة أقل.",
  wrongFileType: "نوع الملف غير صحيح — يرجى اختيار صورة أو استخدام الكاميرا.",
  moulkiaConsentInline: "بالضغط على التصوير، يوافق العميل على استخراج بيانات مالك المركبة لهذه الخدمة.",
  moulkiaOrManualHint: "لا توجد ملكية؟ استخدم «إدخال يدوي» أدناه.",
  manualEntry: "إدخال يدوي (بدون ملكية)",
  manualEntryHint: "إذا لم يكن لدى العميل ملكية، أدخل بيانات المركبة يدويًا.",
  enterManually: "إدخال يدوي ←",
  manualEntryIntro: "أدخل بيانات المركبة والعميل يدويًا.",
  // reception form
  secCustomer: "العميل",
  secVehicle: "المركبة",
  secComplaint: "الشكوى",
  secCondition: "حالة المركبة عند الاستلام",
  secValuables: "الأغراض الثمينة في المركبة",
  secAssignment: "التخصيص (اختياري)",
  secConsent: "الموافقة",
  jobNoLabel: "رقم بطاقة المهمة",
  timeInLabel: "وقت الدخول",
  advisorLabel: "المستشار",
  branchLabel: "الفرع",
  mileageInLabel: "المسافة (كم)",
  checkInLabel: "الاستلام",
  oilTypeLabel: "نوع الزيت",
  fuelLevelLabel: "مستوى الوقود",
  fuelTypeLabel: "نوع الوقود",
  engineSizeLabel: "حجم المحرك",
  fuelType_PETROL: "بنزين",
  fuelType_DIESEL: "ديزل",
  fuelType_HYBRID: "هجين",
  fuelType_ELECTRIC: "كهربائي",
  fuelType_OTHER: "أخرى",
  vinDecodeBtn: "فك ترميز VIN",
  vinDecoding: "جارٍ فك الترميز…",
  vinInvalid: "يجب أن يتكوّن رقم الشاسيه من 17 حرفًا أبجديًا رقميًا (دون I أو O أو Q).",
  vinNotFound: "لم تُرجع NHTSA بيانات قابلة للاستخدام لهذا الرقم. الإدخال اليدوي متاح.",
  vinNetworkError: "تعذّر الوصول إلى مفكّر الترميز. حاول مجددًا أو أدخل البيانات يدويًا.",
  vinDecodedNote: "اضغط لتعبئة الصانع/الموديل/السنة/نوع الوقود تلقائيًا من VIN.",
  vinFilledFields: "تم استخراجها من VIN — يرجى التحقق",
  tabVehicles: "المركبات",
  vehiclesTitle: "المركبات",
  vehiclesSubtitle: "ابحث برقم اللوحة أو اسم المالك أو الهاتف لعرض السجل الكامل للمركبة.",
  vehiclesSearchPlaceholder: "اللوحة، اسم المالك، أو الهاتف…",
  vehiclesSearchBtn: "بحث",
  vehiclesSearchClear: "مسح",
  vehiclesEmptyHint: "اكتب شيئًا للبحث.",
  vehiclesNoMatch: "لم يتم العثور على مركبات.",
  vehiclesVisitSingular: "زيارة",
  vehiclesVisitPlural: "زيارة",
  vehiclesBackToSearch: "العودة إلى بحث المركبات",
  printLabel: "طباعة",
  vehicleHistoryBack: "العودة إلى المركبة",
  vehicleHistoryTitle: "سجل صيانة المركبة",
  vehicleHistoryCurrentOwner: "المالك الحالي",
  vehicleHistoryChangedHands: "غيّرت هذه المركبة مالكها",
  vehicleHistoryEmpty: "لا توجد زيارات مسجّلة على هذه المركبة بعد.",
  vehicleHistoryColDate: "التاريخ",
  vehicleHistoryColJc: "بطاقة عمل رقم",
  vehicleHistoryColMileage: "قراءة العدّاد عند الدخول",
  vehicleHistoryColWork: "العمل المنفّذ",
  vehicleHistoryColParts: "القطع المركّبة",
  vehicleHistoryColRevenue: "المبلغ",
  vehicleHistoryColCost: "التكلفة",
  vehicleHistoryColMargin: "الهامش",
  vehicleHistoryColStatus: "الحالة",
  vehicleHistoryEstimatedNotInvoiced: "تقدير، لم يُفوتَر بعد",
  vehicleHistoryUnpaid: "غير مسدَّد",
  vehicleHistoryTotalsLabel: "إجمالي مدى الحياة",
  vehicleHistoryVisits: "زيارة",
  vehicleHistoryPrintableLink: "عرض السجل القابل للطباعة",
  vehicleHistoryMileageNoteHeading: "ملاحظة:",
  vehicleHistoryMileageNoteBody:
    "زيارة أو أكثر أدناه تسجّل قراءة عدّاد أقل من زيارة سابقة. مُشار إليها بعلامة † في عمود قراءة العدّاد.",
  vehicleHistoryMileageFootnote:
    "قراءة عدّاد أقل من زيارة سابقة. من الأسباب المحتملة تبديل عدّاد المسافات أو خطأ في الإدخال.",
  tabCustomers: "العملاء",
  customersListHeading: "العملاء",
  customersSearchLabel: "ابحث بالاسم أو الهاتف أو الرقم الضريبي",
  customersSearchPlaceholder: "مثل: أحمد، 971501234567، 100000000000003",
  customersSearchGo: "بحث",
  customersEmptyHint: "لا يوجد عملاء بعد.",
  customersNoMatch: "لا يوجد عملاء يطابقون البحث.",
  customersRecentPrefix: "العملاء الذين تم تحديثهم مؤخرًا، الأحدث أولًا.",
  customersRecentTruncated: "يتم عرض أحدث 50 — ابحث للعثور على سجلات أقدم.",
  customersOpenCta: "فتح",
  customersVehicleSingular: "مركبة",
  customersVehiclePlural: "مركبات",
  statementShortLink: "كشف حساب",
  statementTitle: "كشف حساب العميل",
  statementBackToCustomer: "العودة إلى العميل",
  statementCustomerLabel: "العميل",
  statementCustomerTrnLabel: "الرقم الضريبي",
  statementAsOfLabel: "كشف كما في تاريخ",
  statementVehiclesLabel: "المركبات المملوكة",
  statementNoInvoices: "لا توجد فواتير على مركبات هذا العميل.",
  statementColDate: "التاريخ",
  statementColInvoice: "الفاتورة",
  statementColVehicle: "المركبة",
  statementColTotal: "الإجمالي",
  statementColPaid: "المدفوع",
  statementColOutstanding: "المستحق",
  statementColDaysOverdue: "التقادم",
  statementColCost: "التكلفة",
  statementColMargin: "الهامش",
  statementBucketCurrent: "متداول",
  statementBucket1_30: "1–30 يومًا",
  statementBucket31_60: "31–60 يومًا",
  statementBucket61_90: "61–90 يومًا",
  statementBucket90_plus: "أكثر من 90 يومًا",
  statementAdvancesHeading: "دفعات مقدّمة (دائن)",
  statementAdvanceJobCol: "بطاقة عمل",
  statementAdvanceMethodCol: "طريقة الدفع",
  statementAdvanceAmountCol: "المبلغ",
  statementAgingHeading: "ملخّص التقادم",
  statementInvoicesOutstandingLabel: "فواتير مستحقة",
  statementAdvancesCreditLabel: "دفعات مقدّمة (دائن)",
  statementNetBalanceLabel: "الرصيد الصافي المستحق",
  statementInCreditNote: "العميل دائن — الدفعات المقدّمة تتجاوز الفواتير المستحقة.",
  vehiclesVisits: "الزيارات",
  vehiclesTotalSpend: "إجمالي الإنفاق",
  vehiclesLastVisit: "آخر زيارة",
  vehiclesLastOilChange: "آخر تغيير زيت",
  vehiclesAtMileage: "عند",
  vehiclesNoHistory: "لا توجد زيارات سابقة لهذه المركبة.",
  vehiclesVisitHistory: "سجل الزيارات",
  vehiclesComplaint: "الشكوى",
  vehiclesWorkDone: "العمل المنجز",
  vehiclesPartsReplaced: "القطع المستبدلة",
  vehiclesLabor: "العمالة",
  vehiclesPaid: "المدفوع",
  vehicleBeenHere: "هذه المركبة زارتنا سابقًا.",
  vehicleViewHistory: "عرض السجل",
  vehiclesLoadErrorTitle: "تعذّر تحميل سجل المركبة",
  vehiclesLoadErrorBody: "حدثت مشكلة عند قراءة الزيارات السابقة لهذه المركبة. تفاصيل الخطأ بالأسفل — يُرجى مشاركتها مع المسؤول. يمكنك متابعة استخدام بقية التطبيق.",
  complaintLabel: "ما المشكلة؟",
  exteriorLabel: "الخارج",
  interiorLabel: "الداخل",
  remarksLabel: "ملاحظات",
  valuablesNoteLabel: "أخرى (حدّد)",
  oil_KM_5000: "5,000 كم",
  oil_KM_10000: "10,000 كم",
  oil_NONE: "بدون",
  fuel_EMPTY: "فارغ",
  fuel_QUARTER: "¼",
  fuel_HALF: "½",
  fuel_THREE_QUARTER: "¾",
  fuel_FULL: "ممتلئ",
  ext_NO_DAMAGE: "لا يوجد ضرر",
  ext_SCRATCHES: "خدوش",
  ext_DENTS: "انبعاجات",
  ext_BROKEN_LIGHT: "مصباح مكسور",
  ext_CRACKED_WINDSHIELD: "زجاج أمامي متصدع",
  int_CLEAN: "نظيف",
  int_DIRTY: "متّسخ",
  int_WARNING_LIGHT: "ضوء تحذير مضاء",
  int_OTHER: "أخرى",
  val_NONE: "لا شيء",
  val_DOCUMENTS: "مستندات",
  val_CASH: "نقود",
  val_MOBILE_CHARGER: "شاحن جوال",
  val_OTHER: "أخرى",

  backJob: "← المهمة",
  advanceTo: "تقدّم إلى",
  putOnHold: "تعليق",
  resume: "استئناف",
  sendBackToEstimate: "إعادة إلى التقدير",
  cancelJob: "إلغاء المهمة",
  jobCancelled: "🔴 تم إلغاء هذه المهمة.",
  deliveredComplete: "🟢 تم التسليم — اكتملت هذه المهمة.",
  noFurtherActions: "لا توجد إجراءات أخرى.",
  estimates: "التقديرات",
  newEstimate: "+ تقدير جديد",
  noEstimates: "لا توجد تقديرات بعد.",
  estimate: "تقدير",
  invoiceArrow: "الفاتورة ←",
  techActivity: "نشاط الفني",

  tapJob: "اضغط على مهمة للعمل عليها.",
  noActiveJobsTech: "لا توجد مهام نشطة الآن.",
  backWorkshop: "← الورشة",
  addPhoto: "إضافة صورة",
  requestPart: "طلب قطعة",
  finish: "إنهاء",
  pickPart: "اختر قطعة…",
  request: "طلب",
  activity: "النشاط",
  nothingYet: "لا شيء بعد.",
  upload: "رفع",
  waiting: "في الانتظار",
  inProgressMine: "قيد العمل (مهامك)",
  take: "استلام",
  releaseCar: "إرجاع",
  open: "فتح",
  alreadyTaken: "استلم هذه السيارة فنّي آخر للتو.",
  noWaiting: "لا توجد سيارات في الانتظار.",
  forYou: "لك",
  assignTo: "تعيين إلى",
  unassigned: "غير معيّنة (مجموعة مشتركة)",
  technicianLabel: "الفني",
  working: "يعمل الآن",
  assignedLabel: "معيّنة",
  reassign: "إعادة تعيين",
  priorityLabel: "الأولوية",
  setPriority: "تعيين",
  prNormal: "عادية",
  prUrgent: "عاجلة",
  prEmergency: "طارئة",
  // multiple technicians
  joinJob: "انضمام",
  leaveJob: "مغادرة",
  otherInProgress: "مهام أخرى قيد التنفيذ",
  helpingTag: "مساعد",
  helpersLabel: "المساعدون",
  primaryTag: "أساسي",
  // technician findings & parts required
  complaintFromReception: "شكوى العميل",
  jcFindings: "النتائج والتشخيص",
  findingsLabel: "النتائج",
  diagnosisLabel: "التشخيص",
  saveDraft: "حفظ",
  partsRequired: "القطع المطلوبة",
  partNoLabel: "رقم القطعة",
  addPartLine: "حفظ",
  submitToCashier: "إرسال إلى أمين الصندوق لإعداد التقدير ←",
  submittedToCashier: "تم الإرسال إلى أمين الصندوق",
  findingsLocked: "تم الإرسال — للقراءة فقط.",
  photographDamaged: "صوّر القطع التالفة.",
  noPartsRequired: "لا توجد قطع بعد.",
  catalogPartOptional: "قطعة من الكتالوج (اختياري)",
  catalogPartNoPriceHint: "لا يوجد سعر لهذه القطعة في المخزون — أدخل السعر يدويًا، أو حدّده في المخزون.",
  priceThisPart_unitCost: "التكلفة",
  priceThisPart_unitPrice: "سعر البيع",
  priceThisPart_save: "حفظ",
  priceThisPart_cancel: "إلغاء",
  priceThisPart_priced: "مُسعَّر",
  priceThisPart_markupFromDefault: "من الإعداد الافتراضي للجراج",
  lineFormErrorTitle: "تعذّر حفظ هذا البند",
  lineFormErr_generic: "لم يُحفظ البند. تحقّق من القيم وحاول مجدّدًا.",
  lineFormErr_price_required: "سعر الوحدة مطلوب — الحقل الفارغ ليس صفرًا. أدخل السعر، أو إن كان البند مجانيًا فعلًا (ضمان/مجاملة) فاكتب 0 صراحةً.",
  lineFormErr_price_negative: "لا يمكن أن يكون سعر الوحدة سالبًا. للحصول على خصم، استخدم نوع البند «خصم».",
  lineFormErr_price_not_numeric: "يجب أن يكون سعر الوحدة رقمًا.",
  lineFormErr_desc_required: "الوصف مطلوب.",
  lineFormErr_qty_invalid: "يجب أن تكون الكمية رقمًا موجبًا.",
  lineFormErr_kind_unknown: "اختر نوع بند صحيحًا (عمالة / قطعة / رسوم / خصم).",
  lineFormErr_zero_part_lines_estimate: "يحتوي هذا التقدير على بنود «قطعة» بسعر 0.00 درهم. أدخل أسعارًا حقيقية، أو ضع البنود على «مرفوض»، أو استخدم «إرسال على أي حال» إذا كانت مجانية فعلًا (ضمان/مجاملة).",
  lineFormErr_zero_part_lines_invoice: "ستُصدَر الفاتورة ببنود «قطعة» بسعر 0.00 درهم. أدخل أسعارًا حقيقية، أو ضع البنود على «مرفوض»، أو استخدم «إصدار على أي حال» إذا كانت مجانية فعلًا. رقم الفاتورة يُستهلَك من تسلسل بلا فجوات — التصحيح بعد الإصدار يتطلب إبطالًا وإعادة إصدار.",
  lineFormErr_estimate_already_approved: "لقد وافق العميل على هذا التقدير مسبقًا. إعادة الإرسال ستُلغي الموافقة. افتح محادثة واتساب لإرسال رسالة متابعة بدلًا من إعادة إرسال التقدير.",
  lineFormErr_estimate_already_rejected: "رفض العميل هذا التقدير مسبقًا. أنشئ تقديرًا جديدًا لإرسال عرض مُنقَّح بدلًا من إعادة إرسال هذا.",
  phoneNudgeHeading: "تعذّر قراءة رقم هاتف العميل كرقم صالح. سيطلب منك واتساب اختيار المستلم. صحّح الرقم في سجل العميل حتى يعمل الإرسال مباشرة في المرة القادمة.",
  phoneNudgeRawLabel: "المسجّل:",
  phoneNudgeRawNone: "(لم يُسجَّل)",
  lineFormErr_zeroPartsHeading: "بنود بسعر 0.00 درهم:",
  lineFormErr_sendAnyway: "إرسال على أي حال (هذه مجانية فعلًا)",
  lineFormErr_generateAnyway: "إصدار على أي حال (هذه مجانية فعلًا)",
  zeroPartsPreflightTitle: "بنود «قطعة» بدون سعر",
  zeroPartsPreflightBody: "يحتوي هذا التقدير على بنود «قطعة» بسعر 0.00 درهم. أدخل أسعارًا حقيقية أو ضعها على «مرفوض» قبل الإرسال. إذا كانت مجانية فعلًا (ضمان/مجاملة) فسيُطلب منك التأكيد عند الإرسال.",
  zeroPartsChip: "⚠ بدون سعر",
  techFindingsPanel: "نتائج الفني والقطع المطلوبة",
  enterFindingsFirst: "أدخل النتائج قبل الإرسال.",
  // repair stage
  jcRepair: "الإصلاح — اكتمل العمل",
  workNotesLabel: "ملاحظات إنجاز العمل",
  partsUsed: "القطع المستخدمة",
  noPartsUsed: "لا توجد قطع مستخدمة بعد.",
  markComplete: "اكتمل العمل ←",
  workCompletedBadge: "اكتمل العمل",
  repairLockedNote: "اكتمل العمل — للقراءة فقط.",
  partsUsedPanel: "القطع المستخدمة وملاحظات العمل",
  priceThisPart: "تسعير هذه القطعة ←",
  // quality control
  jcQc: "ضبط الجودة",
  signOffQc: "اعتماد الجودة",
  qcPassedBadge: "اجتاز الفحص",
  qcInspector: "المفتش",
  qc_REPAIR_COMPLETED: "اكتمل الإصلاح",
  qc_ROAD_TEST: "تم اختبار القيادة",
  qc_NO_WARNING_LIGHTS: "لا أضواء تحذير",
  qc_VEHICLE_CLEANED: "تم تنظيف المركبة",
  // voice dictation
  dictateStart: "إملاء",
  dictateStop: "إيقاف",
  dictateListening: "جارٍ الاستماع… (انقر للإيقاف)",
  dictateError: "تعذّر الفهم — حاول مجددًا أو اكتب.",
  // delivery
  jcDelivery: "التسليم",
  mileageOutLabel: "العداد عند التسليم (كم)",
  recordDelivery: "تسجيل التسليم",
  markDelivered: "تأكيد التسليم",
  deliveredByLabel: "سُلِّمت بواسطة",
  deliveredAtLabel: "وقت التسليم",
  collectionAwaiting: "بانتظار تأكيد العميل",
  collectionConfirmed: "أكد العميل الاستلام",
  // customer-facing /c/delivery
  collectionTitle: "تأكيد الاستلام",
  documentDelivery: "تسليم",
  collectionIntro: "يرجى تأكيد استلامك للمركبة.",
  confirmCollectionBtn: "تأكيد الاستلام",
  collectionThanks: "✅ شكرًا — تم تأكيد الاستلام.",
  // bays / ramps
  tabBays: "المنصات",
  baysTitle: "المنصات والرافعات",
  baysIntro: "السعة الفعلية للورشة. عيّن سيارة لمنصة؛ يوضح العدد المساحة المتاحة.",
  addBay: "إضافة منصة",
  bayNamePh: "اسم المنصة (مثل رافعة 1)",
  bayLabel: "المنصة",
  noBay: "بدون منصة",
  baysInUse: "قيد الاستخدام",
  bayEmpty: "فارغ",
  bayMultipleOccupants: "⚠ {n} مهام نشطة على هذا الرصيف — خلل بيانات، يرجى التحقق",
  chatSearchPlaceholder: "ابحث بالعميل أو الهاتف أو الرسالة",
  chatSearchClear: "مسح",
  chatNoSearchResults: "لا توجد محادثات تطابق بحثك.",
  chatUnreadTag: "{n} غير مقروءة",
  chatUnreadTag_one: "رسالة واحدة غير مقروءة",
  chatUnreadTag_two: "رسالتان غير مقروءتين",
  chatUnreadTag_few: "{n} رسائل غير مقروءة",
  chatUnreadTag_many: "{n} رسالة غير مقروءة",
  chatUnreadTag_other: "{n} رسالة غير مقروءة",
  analyticsCsvDownload: "تحميل CSV",
  bayExists: "اسم المنصة موجود بالفعل.",

  costLabel: "التكلفة",
  markupLabel: "نسبة الربح ٪",
  marginLabel: "الهامش",
  colItem: "البند",
  colQty: "الكمية",
  colUnit: "السعر",
  colParticulars: "التفاصيل",
  colSerialNumber: "الرقم التسلسلي",
  // Batch F1 (2026-08-25) — VIN on printed estimate + invoice.
  documentVinLabel: "رقم الهيكل",
  // Batch F2 (2026-08-25) — totals wording on the printed doc.
  totalGrossLabel: "الإجمالي قبل الضريبة",
  totalVatLabel: "ضريبة القيمة المضافة ٥٪",
  totalNetLabel: "الإجمالي الصافي",
  // Batch F2 (2026-08-25) — centred main title on the printed estimate.
  docTitleRepairEstimate: "عرض سعر إصلاح",
  colTotal: "الإجمالي",
  colAmount: "المبلغ",
  colDescription: "الوصف",
  colKind: "النوع",
  colMake: "الصانع",
  colModel: "الطراز",
  colYear: "السنة",
  colActions: "الإجراءات",
  colPart: "القطعة",
  kindLabor: "أتعاب",
  kindPart: "قطعة",
  kindSublet: "أعمال خارجية",
  kindFee: "رسوم",
  kindDiscount: "خصم",
  addLineTitle: "إضافة بند",
  addLineButton: "إضافة",
  removeLine: "حذف",
  noLineItems: "لا توجد بنود بعد.",
  subtotal: "المجموع الفرعي",
  subtotalBeforeDiscount: "المجموع الفرعي (قبل الخصم)",
  subtotalAfterDiscount: "المجموع الفرعي (بعد الخصم)",
  netSubtotal: "الصافي بعد الخصم",
  vat5: "ضريبة القيمة المضافة (5%)",
  discountSectionTitle: "الخصم",
  discountPercentLabel: "خصم %",
  discountAmountLabel: "خصم مبلغ",
  discountApply: "تطبيق",
  discountRemove: "إزالة الخصم",
  discountNone: "لا يوجد خصم",
  discountCurrentPercent: "المُطبَّق حاليًا: {pct}%",
  discountCurrentFixed: "المُطبَّق حاليًا: {amount}",
  discountRow: "الخصم",
  discountRowPercent: "خصم ({pct}%)",
  discountRowFixed: "خصم",
  total: "الإجمالي",
  description: "الوصف",
  addLine: "إضافة بند",
  labor: "عمالة",
  part: "قطعة",
  sublet: "أعمال خارجية",
  fee: "رسوم",
  estimateSectionParts: "قطع الغيار",
  estimateSectionSublet: "أعمال خارجية / مستهلكات / خدمات",
  estimateSectionLabour: "أجور العمالة",
  estimateSectionSubtotal: "المجموع الفرعي",
  estimateTotalsSumOfSections: "مجموع الأقسام",
  estimateRemarksHeading: "ملاحظات",
  estimateRemarksLabel: "ملاحظات",
  estimateRemarksPlaceholder: "مثال: هذا التقدير يشمل فقط الأعمال اللازمة لتشغيل المحرك. سيلزم إجراء فحص إضافي للأعطال الأخرى.",
  estimatePaymentTermsHeading: "شروط الدفع",
  estimatePaymentTermsLabel: "شروط الدفع",
  estimatePaymentTermsPlaceholder: "مثال: 50٪ مقدّم و50٪ عند التسليم",
  estimatePaymentTermsUsingDefaultHint: "الفراغ يستخدم الافتراضي المضبوط في الإعدادات.",
  estimateAdvisorHeading: "المستشار",
  estimateHeaderSaveButton: "حفظ الملاحظات وشروط الدفع",
  settingsPaymentTermsLabel: "شروط الدفع الافتراضية",
  settingsPaymentTermsPlaceholder: "مثال: 50٪ مقدّم و50٪ عند التسليم",
  settingsPaymentTermsHint: "تُطبع على كل تقدير. يمكن للمستشارين تعديلها لكل تقدير على حدة.",
  // Settings — split-terms (AR 2026-08-25).
  settingsSecEstimateTerms: "شروط عرض السعر",
  settingsSecEstimateTermsHint: "تُطبع في أسفل كل عرض سعر. نص حر — اكتب بنودك المرقمة الخاصة.",
  settingsEstimateTermsLabel: "شروط وأحكام عرض السعر",
  settingsEstimateTermsPlaceholder: "١. هذا العرض ساري لمدة ٧ أيام.\n٢. الأسعار قابلة للتغيير.\n٣. …",
  settingsEstimateTermsHint: "يتم الحفاظ على فواصل الأسطر. اترك الحقل فارغاً لإخفاء البنود من عرض السعر.",
  settingsTermsResponsibility: "هذه هي شروط ورشتك الخاصة، ومن مسؤوليتك مراجعتها. لا يُطبع أي شيء على مستند العميل إلا بعد أن تحفظ النص في الحقل أدناه.",
  settingsTermsSuggestedHeading: "نص مقترح — لم يُحفظ بعد",
  settingsTermsSuggestedHint: "هذه بداية مقترحة وليست قيمة محفوظة. لاعتمادها كما هي، اضغط الزر أدناه. أو اكتب نصك الخاص في الحقل واحفظ.",
  settingsTermsAdoptButton: "اعتمد هذا كشروطي",
  // AR 2026-08-25 — print notice on editor pages.
  printEditorNoticeHeading: "شاشة التحرير",
  printEditorNoticeBody: "المستند المخصص للعميل موجود على صفحة المعاينة. عُد إلى التطبيق، افتح عرض السعر أو الفاتورة، اضغط \"معاينة\" ثم اطبع من هناك.",
  settingsEstimateTermsDefaultText:
    "١. عرض السعر هذا ساري لمدة ٧ أيام من التاريخ المذكور أعلاه.\n٢. الأسعار قابلة للتغيير دون إشعار مسبق.\n٣. لا يُقدَّم أي ضمان على القطع الموردة من قبل العميل.\n٤. أي أعمال إضافية تُكتشف أثناء الإصلاح سيتم تسعيرها وتتطلب موافقة العميل قبل التنفيذ.\n٥. المبلغ النهائي للفاتورة قد يختلف عن هذا العرض في حال تفويض أعمال إضافية.\n٦. القطع مُصنَّفة على أنها OEM (أصلية من الشركة الصانعة) أو OES (أصلية من مورد الشركة) أو Aftermarket (بديلة)، كما هو موضح لكل بند.\n٧. يُخوّل العميل الورشة لإجراء اختبار قيادة للمركبة حسب الحاجة للتأكد من الإصلاح.",
  settingsOkEstimateTerms: "تم حفظ شروط عرض السعر.",
  settingsSecInvoiceTerms: "شروط الفاتورة",
  settingsSecInvoiceTermsHint: "تُطبع في أسفل كل فاتورة. نص حر — اكتب بنودك المرقمة الخاصة.",
  settingsInvoiceTermsLabel: "شروط وأحكام الفاتورة",
  settingsInvoiceTermsPlaceholder: "١. ضمان أعمال الورشة على النحو المتفق عليه.\n٢. ضمان القطع وفقاً لسياسة المورد.\n٣. …",
  settingsInvoiceTermsHint: "يتم الحفاظ على فواصل الأسطر. اترك الحقل فارغاً لإخفاء البنود من الفاتورة.",
  settingsInvoiceTermsDefaultText:
    "١. ضمان العمل على النحو المتفق عليه وقت الإصلاح.\n٢. ضمان القطع وفقاً لسياسة الشركة الصانعة أو المورد — تنقل الورشة ما يقدمه المورد فقط، لا أكثر.\n٣. لا يُطبَّق الضمان على القطع التي يوفرها العميل.\n٤. شروط الدفع على النحو المتفق عليه.",
  settingsOkInvoiceTerms: "تم حفظ شروط الفاتورة.",
  documentTermsHeading: "الشروط والأحكام",
  // Batch E — deploy-freshness banner.
  versionCheckMessage: "يتوفر إصدار أحدث. أعد التحميل للحصول على آخر تحديث — قد يُفقد ما لم تحفظه.",
  versionCheckReload: "أعد التحميل",
  versionCheckDismiss: "لاحقاً",
  discount: "خصم",
  skip: "تخطّي",
  restore: "استعادة",
  editLine: "تعديل",
  deleteLine: "حذف",
  saveLine: "حفظ",
  cancelLine: "إلغاء",
  confirmDeleteLine: "هل تريد حذف هذا البند؟",
  onHold: "معلّقة",
  hrAwaitingPart: "بانتظار قطعة",
  hrAwaitingCustomer: "بانتظار العميل",
  hrAwaitingApproval: "بانتظار الموافقة",
  hrOther: "أخرى",

  // Workflow stepper labels
  stage_CHECK_IN: "الاستقبال",
  stage_DIAGNOSIS: "التشخيص",
  stage_ESTIMATE: "تقدير السعر",
  stage_APPROVAL: "الموافقة",
  stage_REPAIR: "الإصلاح",
  stage_COMPLETE: "اكتمل العمل",
  stage_INVOICE: "الفاتورة",
  stage_PAID: "تم الدفع",
  stage_DELIVERED: "تسليم",
  workflowCancelled: "تم إلغاء العمل",
  workflowPaused: "موقوف مؤقتاً",

  // Job timeline (audit log)
  tlTitle: "سجل المهمة",
  tlNoActor: "—",
  tlToday: "اليوم",
  tlRole_ADVISOR: "مستشار",
  tlRole_TECH: "فني",
  tlRole_CASHIER: "محاسب",
  tlRole_OWNER: "المالك",
  tlVehicleCheckedIn: "تم استلام السيارة",
  tlConsentRecorded: "تم تسجيل موافقة الموكل",
  tlClaimedByTech: "استلم الفني المركبة",
  tlPhotoLogged: "تمت إضافة صورة",
  tlVoiceLogged: "تم تسجيل ملاحظة صوتية",
  tlPartRequested: "تم طلب قطعة",
  tlFindingsSubmitted: "تم تسليم الفحص",
  tlSentForEstimate: "تم إرسال إلى مستشار الخدمة للتسعير",
  tlSentForReestimate: "تم إرسال إلى مستشار الخدمة لإعادة التسعير",
  tlEstimateCreated: "تم إنشاء التقدير",
  tlEstimateSent: "تم تسليم التقدير إلى واتساب",
  tlEstimateDelivered: "استلم العميل التقدير",
  tlEstimateApproved: "وافق العميل على التقدير",
  tlWorkCompleted: "اكتمل العمل",
  tlQcPassed: "تم اجتياز فحص الجودة",
  tlInvoiceIssued: "تم إصدار الفاتورة",
  tlInvoiceSent: "تم تسليم الفاتورة إلى واتساب",
  tlInvoiceDelivered: "استلم العميل الفاتورة",
  tlAdvancePayment: "تم استلام دفعة مقدمة",
  tlPaymentRecorded: "تم تسجيل الدفع",
  tlDelivered: "تم تسليم المركبة للعميل",
  tlCollectionConfirmed: "أكد العميل الاستلام",
  tlJobCancelled: "تم إلغاء المهمة",
  tlJobHeld: "تم إيقاف المهمة مؤقتًا",
  paused: "معلّقة",
  approvedOn: "تمت الموافقة",
  holdNote: "ملاحظة (اختياري)",
  skipTo: "تخطّي إلى",
  sendToCustomer: "إرسال للعميل",
  estimatePreviewButton: "معاينة التقدير ←",
  estimatePreviewGoBack: "← العودة للتعديل",
  jobCardPrintTitle: "بطاقة العمل",
  jobCardPrintBtn: "🖨 طباعة بطاقة العمل",
  printJobCard: "🖨 طباعة بطاقة العمل",
  signatureCustomer: "توقيع العميل",
  signatureAdvisor: "توقيع الاستشاري",
  signatureDate: "التاريخ",
  moulkiaConsentRecorded: "تم تسجيل موافقة الملكية",
  disputeShieldNote:
    "يؤكد هذا الإيصال حالة المركبة والأغراض الثمينة عند التسليم. يجب الإبلاغ عن أي مطالبات تلف إضافية قبل استلام المركبة.",
  estimateSendToCustomer: "إرسال التقدير للعميل ←",
  estimateResendViaWhatsApp: "إعادة الإرسال عبر واتساب ←",
  estimatePreviewMockNote: "زر «إرسال التقدير» يفتح واتساب على جهازك مع نص الرسالة جاهزًا — اضغط «إرسال» داخل واتساب لإتمام التسليم.",
  markApproved: "تعيين: موافَق",
  markRejected: "تعيين: مرفوض",
  generateInvoice: "إنشاء فاتورة",
  estimateApprovedSentToTech: "موافَق عليه — أُرسل للفني لإنجاز العمل.",
  continueToWorkshop: "متابعة إلى الورشة ←",
  confirmCancelJob: "إلغاء هذه المهمة؟ ستختفي من الورشة.",
  paginationShowing: "عرض {from}–{to} من {total}",
  paginationRowsPerPage: "عدد الصفوف في الصفحة",
  paginationPrev: "السابق",
  paginationNext: "التالي",
  suppliersInactiveCap: "عرض أول {shown} من {total}.",
  viewInvoice: "عرض الفاتورة ←",
  taxInvoice: "فاتورة ضريبية",
  customerTrnLabel: "الرقم الضريبي للعميل",
  customerTrnEditLegend: "التسجيل الضريبي للعميل (TRN)",
  customerTrnEditHint:
    "إذا كان العميل مسجّلًا في الضريبة، أدخل رقمه الضريبي ليظهر على فاتورته — يحتاج ذلك لاسترداد الضريبة.",
  customerTrnSave: "حفظ",
  customerTrnSaved: "تم الحفظ",
  billTo: "إلى",
  issued: "تاريخ الإصدار",
  due: "تاريخ الاستحقاق",
  clearance: "التخليص",
  paid: "مدفوع",
  balance: "الرصيد",
  recordPayment: "تسجيل دفعة",
  amount: "المبلغ",
  methodCash: "نقدًا",
  methodCard: "بطاقة",
  methodTransfer: "تحويل بنكي",
  methodCardPos: "بطاقة (نقاط بيع)",
  methodOnlineLink: "رابط دفع إلكتروني",
  markAsPaid: "تحديد كمدفوعة",
  soon: "قريبًا",
  qrPlaceholder: "رمز QR (مؤقت)",

  yourEstimate: "تقديرك",
  approve: "موافقة",
  decline: "رفض",
  estimateApprovedMsg: "✅ لقد وافقت على هذا التقدير.",
  estimateDeclinedMsg: "❌ لقد رفضت هذا التقدير.",
  estimateNotReady: "هذا التقدير غير جاهز للموافقة بعد.",
  yourInvoice: "فاتورتك",
  paidThanks: "✅ مدفوعة — شكرًا لك!",
  payAtGarage: "يرجى الدفع في الورشة (نقدًا أو بالبطاقة)",

  accounts: "الحسابات",
  cashierToInvoiceTitle: "جاهز لإصدار الفاتورة",
  cashierGenerateInvoice: "إنشاء الفاتورة ←",
  cashierGoToInvoice: "فتح الفاتورة ←",
  cashierReestimateTitle: "أعمال إضافية — مطلوب إعادة تقدير",
  cashierApprovedEstimatesTitle: "التقديرات المعتمدة",
  cashierApprovedEstimatesEmpty: "لا توجد تقديرات معتمدة بانتظار الفوترة.",
  cashierFilterActive: "مفلتر",
  cashierClearFilter: "إزالة الفلتر ×",
  cashierWaitingDiagnosisTitle: "بانتظار فحص الفني",
  cashierWaitingDiagnosisCaption: "سيقوم مستشار الخدمة بالتسعير بعد انتهاء الفني من الفحص.",
  cashierWorkInProgressTitle: "العمل جارٍ (وافق العميل على التقدير)",
  cashierWorkInProgressCaption: "العمل جارٍ — بانتظار اكتمال عمل الفني.",
  cashierToSendInvoiceTitle: "الفاتورة جاهزة — أرسلها للعميل",
  cashierSendInvoiceToCustomer: "إرسال عبر واتساب ←",
  // Phase 5 — read-only "Coming up" rollup.
  cashierComingUpTitle: "قادمة قريبًا",
  cashierComingUpHint: "يقوم المستشار بالتسعير أو ينتظر العميل — ستظهر هنا بعد الموافقة واكتمال عمل الفني.",
  cashierComingUpEmpty: "لا شيء قيد التحضير الآن.",
  continuePricing: "متابعة التسعير",
  rePrice: "إعادة التسعير",
  pricingByAdvisor: "يحدد مستشار الخدمة الأسعار.",
  createEstimate: "إنشاء تقدير",
  advisorPleasePrice: "سلّم الفني السيارة — حان دورك لتسعير التقدير.",
  advisorDraftInProgress: "مسودة تقدير قيد التحضير — أكمل التسعير وأرسل للعميل.",
  awaitingPriceTag: "بانتظار التسعير",
  mRevenue: "الإيرادات",
  mVatCollected: "الضريبة المحصّلة",
  mCashIn: "النقد الوارد",
  mArOutstanding: "الذمم المستحقة",
  receivables: "الذمم المدينة (غير مدفوعة)",
  noInvoices: "لا توجد فواتير بعد.",
  noUnpaidInvoices: "لا توجد فواتير غير مدفوعة — تم تحصيل الكل.",
  paidInvoicesTab: "الفواتير المدفوعة ←",
  cashierSearchPlaceholder: "ابحث برقم الفاتورة أو المهمة أو اللوحة أو العميل",
  cashierFilterFrom: "من",
  cashierFilterTo: "إلى",
  cashierFilterClear: "مسح",
  cashierTabEstimates: "التقديرات",
  cashierTabInvoices: "الفواتير",
  cashierTabPayments: "المدفوعات",
  cashierTabReports: "التقارير",
  cashierTabReportsHeading: "التقارير",
  counterApprovedEstimates: "موافَق عليها:",
  counterUnpaidInvoices: "فواتير غير مدفوعة:",
  counterPaidInvoices: "فواتير مدفوعة:",
  counterOverdueInvoices: "متأخرة:",
  counterPartiallyPaidInvoices: "مدفوعة جزئيًا:",
  counterReadyForInvoice: "جاهزة للفوترة:",
  invoiceBadgeOverdue: "متأخرة",
  invoiceBadgePartial: "مدفوعة جزئيًا",
  invoiceBadgePaid: "مدفوعة",
  invoiceAdvancePaid: "دفعة مقدّمة",
  invoiceBalanceDue: "الرصيد المستحق",
  advancePaymentsTitle: "الدفعات المقدّمة",
  advancePaymentsCeiling: "إجمالي التقدير المعتمد",
  advancePaymentsFullyCovered: "الدفعة المقدّمة تغطي كامل التقدير. الرصيد المستحق صفر.",
  receiveAdvancePayment: "استلام دفعة مقدّمة",
  method: "طريقة الدفع",
  invoicePrintInvoice: "طباعة الفاتورة",
  invoicePrintReceipt: "طباعة الإيصال",
  invoiceEmailInvoice: "إرسال الفاتورة بالبريد",
  invoiceLinkInvalidTitle: "هذا الرابط لا يعمل",
  invoiceLinkInvalidBody:
    "رابط الفاتورة الذي فتحته غير صالح أو منتهي الصلاحية. الرجاء الرد على رسالة الواتساب التي وصلتك واطلب من الكراج إرسال رابط جديد.",
  invoiceLinkInvalidBodyNamed:
    "رابط الفاتورة الذي فتحته غير صالح أو منتهي الصلاحية. الرجاء الرد على رسالة الواتساب التي وصلتك من {garage} واطلب إرسال رابط جديد.",
  invoiceReceiptTitle: "إيصال",
  invoiceReceiptCustomer: "العميل",
  invoiceReceiptInvoiceNumber: "رقم الفاتورة",
  invoiceReceiptAmountPaid: "المبلغ المدفوع",
  invoiceReceiptDate: "التاريخ",
  invoiceReceiptMethod: "طريقة الدفع",
  invoiceReceiptBalance: "الرصيد",
  invoiceReceiptTotal: "الإجمالي",
  invoiceReceiptThanks: "شكرًا لتعاملكم معنا.",
  paidInvoicesTitle: "الفواتير المدفوعة",
  paidInvoicesEmpty: "لا توجد فواتير مدفوعة بعد.",
  paidInvoicesBack: "← العودة إلى لوحة المعلومات",
  colCustomer: "العميل",
  colVehicle: "المركبة",
  colVehicles: "المركبات في هذا الطلب",
  colVat: "الضريبة",
  colLineTotal: "الإجمالي",
  colMethod: "طريقة الدفع",
  colDatePaid: "تاريخ الدفع",
  paidLower: "مدفوعة",
  dueLower: "تستحق",
  ledgerNote: "تُحتسب جميع الأرقام من قيود دفترية تلقائية — دون مسك دفاتر يدوي.",

  // parts (Tier 1 #3)
  tabParts: "القطع",
  partsQueue: "طلبات القطع",
  partRequests: "طلبات القطع",
  cancelPartRequestConfirm: "إلغاء طلب القطعة؟",
  noOpenParts: "لا توجد طلبات قطع مفتوحة.",
  inStockShort: "متوفر",
  inStockTag: "متوفر",
  outOfStock: "غير متوفر",
  orTypePart: "أو اكتب اسم القطعة",
  requestedByLabel: "طلبها",
  actFulfill: "تسليم",
  actOrder: "طلب توريد",
  actArrived: "تم الوصول",
  actReorder: "خطأ / إعادة طلب",
  actCancelReq: "إلغاء",
  supplierNote: "المورّد / الوقت المتوقع (اختياري)",
  pr_REQUESTED: "مطلوبة",
  pr_ORDERED: "تم الطلب",
  pr_ARRIVED: "وصلت",
  pr_FULFILLED: "تم التسليم",
  pr_CANCELLED: "ملغاة",

  // maintenance reminders (Workflow-Spec step 16)
  tabReminders: "التذكيرات",
  remindersTitle: "تذكيرات الصيانة",
  scheduleReminders: "جدولة تذكيرات الصيانة",
  scheduledReminders: "التذكيرات المجدولة",
  serviceDateLabel: "تاريخ الخدمة",
  scheduleBtn: "جدولة",
  noReminders: "لا توجد تذكيرات مجدولة.",
  dueNow: "مستحقة الآن",
  upcoming: "قادمة",
  sentTab: "مُرسلة",
  sendReminderBtn: "إرسال",
  sendDueNow: "إرسال المستحقة الآن",
  dueOn: "مستحقة",
  sentOn: "أُرسلت",
  cancelReminderBtn: "إلغاء",
  remindersBucketOverdue: "متأخرة",
  remindersBucketDueSoon: "تستحق قريبًا (خلال 7 أيام)",
  remindersBucketDueMonth: "تستحق هذا الشهر",
  remindersBucketUpcoming: "قادمة",
  remindersBulkSendOverdue: "إرسال لكل المتأخرات ←",
  remindersBulkSendOverdueN: "إرسال {n} من الظاهرة ←",
  remindersBulkSendConfirm: "تسليم {n} تذكير عبر واتساب إلى Meta؟ (رسالة واحدة لكل عميل. رابط wa.me لا يؤكد التسليم — انظر docs/business-rules.md قاعدة 4.)",
  remindersSendToCustomer: "إرسال إلى العميل ←",
  remindersReminderSingular: "تذكير",
  remindersReminderPlural: "تذكيرات",
  remindersCarSingular: "سيارة",
  remindersCarPlural: "سيارات",
  remindersFilterPlaceholder: "تصفية حسب الخدمة أو المركبة أو اللوحة أو العميل",
  remindersFilterApply: "تصفية",
  remindersFilterClear: "مسح",
  remindersFilterNoMatch: "لا توجد تذكيرات مطابقة لهذه التصفية.",
  remindersPrevMonth: "الشهر السابق",
  remindersNextMonth: "الشهر التالي",
  remindersJumpToMonth: "اذهب إلى شهر",
  remindersSummaryDueIn: "مستحقة في",
  remindersMonthEmpty: "لا توجد تذكيرات مستحقة في هذا الشهر.",
  remindersOtherMonths: "أشهر أخرى تحتوي على تذكيرات",
  rt_OIL_5000: "زيت (5,000 كم)",
  rt_OIL_10000: "زيت (10,000 كم)",
  rt_BATTERY: "البطارية",
  rt_TIRE_ROTATION: "تدوير الإطارات",
  rt_BRAKES: "الفرامل",
  rt_AC_SERVICE: "صيانة المكيف",
  rt_AIR_FILTER: "فلتر الهواء",
  rt_COOLANT: "سائل التبريد",
  rt_TRANSMISSION: "ناقل الحركة",
  rs_SCHEDULED: "مجدولة",
  rs_SENT: "مُرسلة",
  rs_CANCELLED: "ملغاة",

  // end-of-day view + check-in photo + collection nudge
  tabEod: "نهاية اليوم",
  eodTitle: "نهاية اليوم",
  eodIntro: "ما الذي ما زال مفتوحًا قبل الإغلاق — وعلى ماذا تنتظر كل سيارة.",
  eodEmpty: "لا يوجد شيء مفتوح — كل شيء جاهز. 🎉",
  eod_IN_PROGRESS: "قيد التنفيذ",
  eod_WAITING_PARTS: "بانتظار القطع",
  eod_WAITING_CUSTOMER: "بانتظار العميل",
  eod_READY: "جاهزة للاستلام",
  eod_OTHER: "معلّقة",
  nudgeCollect: "تذكير بالاستلام",
  nudged: "تم التذكير ✓",
  checkInPhotoPrompt: "صوّر السيارة عند الاستلام — يحميك من ادعاءات «أنتم أتلفتموها».",
  addCheckInPhoto: "إضافة صورة الاستلام",
  checkInDone: "صورة الاستلام محفوظة ✓",

  ownerDashboard: "لوحة المالك",
  dashCashLabel: "الكاش المستلَم هذا الشهر",
  dashRevenueLabel: "الإيرادات هذا الشهر",
  dashUnpaidLabel: "فواتير غير مدفوعة",
  dashProfitLabel: "الربح الإجمالي هذا الشهر",
  dashJobsLabel: "المهام المكتملة هذا الشهر",
  dashAgingCurrent: "حالياً",
  dashAging1_30: "1–30 يوم",
  dashAging31_60: "31–60 يوم",
  dashAging61plus: "61+ يوم",
  dashProfitIncompleteValue: "بيانات التكلفة غير مكتملة",
  dashProfitIncompleteSub: "{missing} من {total} تفتقد للتكلفة. الإيرادات {revenue}.",
  dashProfitCompleteSub: "هامش {margin}% · {costed} من {total} مكلَّفة",
  dashTrendUp: "▲ {pct}% مقارنةً بالشهر الماضي",
  dashTrendDown: "▼ {pct}% مقارنةً بالشهر الماضي",
  dashTrendFlat: "= نفس المستوى",
  mRevenueMo: "الإيرادات (شهر)",
  mProfitMo: "الربح (شهر)",
  mCarsToday: "السيارات اليوم",
  mSatisfaction: "الرضا",
  mInventory: "المخزون",
  low: "منخفض",
  thisWeek: "هذا الأسبوع",
  vsLastWeek: "مقارنة بالأسبوع الماضي",
  satisfactionSoon: "يصل تتبّع الرضا مع التقييمات.",
  askCopilot: "اسأل المساعد",
  ask: "اسأل",
  copilotReadonly: "للقراءة فقط · ضمن ورشتك.",
  pilotMetrics: "مقاييس التجربة واستخدام الذكاء الاصطناعي",
  intakeAcceptance: "قبول الاستقبال",
  avgConfirm: "متوسط الحجز ← التأكيد",
  aiCalls: "مكالمات الذكاء (شهر)",
  aiCost: "تكلفة الذكاء (شهر)",
  aiCostNote: "تُقاس تكلفة الذكاء لكل مكالمة حتى لا يتجاوز الاستخدام الاشتراك.",
  noTechActivity: "لم يُسجَّل نشاط للفنيين بعد.",
  colTechnician: "الفني",
  colJobs: "المهام",
  colSteps: "الخطوات",
  colAvgTimeTitle: "متوسط الوقت لكل مهمة منجزة (من الاستلام إلى الإنجاز)",
  colTodayTimeTitle: "إجمالي الوقت على المهام المنجزة اليوم",
  colTodayJobsTitle: "المهام المنجزة اليوم",
  colAvgShort: "متوسط",
  colTodayTime: "اليوم ⏱",
  colTodayJobs: "اليوم #",
  colPhotosTitle: "عدد الصور المسجلة",
  colVoiceTitle: "عدد التسجيلات الصوتية",
  colPartsTitle: "طلبات قطع الغيار",
  colFinishesTitle: "خطوات الإنهاء",
  colApprovedTitle: "تقديرات وافق عليها العميل",
  colRejectedTitle: "تقديرات رفضها العميل",
  advisorActivity: "نشاط مستشار الخدمة",
  noAdvisorActivity: "لا يوجد نشاط للمستشار بعد.",
  colAdvisor: "المستشار",
  colEstimatesSent: "المرسلة",
  colApprovalRate: "النسبة",
  colApprovalRateTitle: "نسبة الموافقة (الموافق عليها ÷ القرارات)",
  colApprovalTimeShort: "متوسط",
  colApprovalTimeTitle: "متوسط الوقت من إرسال التقدير إلى قرار العميل",
  tabLedger: "السجل",
  tabAccounting: "تصدير للمحاسبة",
  tabErpSync: "مزامنة ERPNext",
  acctExportHeading: "تصدير للمحاسبة",
  acctExportIntro: "نزّل بيانات الكراج المالية كملفات CSV لاستيرادها في نظامك المحاسبي. كل عملية تنزيل تُسجَّل.",
  acctExportFromLabel: "من (شامل)",
  acctExportToLabel: "إلى (شامل)",
  acctExportApplyRange: "تطبيق النطاق",
  acctExportDownloadCta: "تنزيل CSV",
  acctExportMappingPointer: "لأكواد الحسابات وأنواعها وقاعدة تقسيم النقدية/البنك، راجع docs/accounting-chart-of-accounts.md في المستودع.",
  acctExportChartLabel: "دليل الحسابات (إعداد)",
  acctExportChartDesc: "إعداد لمرة واحدة لنظامك المحاسبي — الأكواد والأسماء والأنواع والأرصدة الطبيعية. استورد هذا أولًا.",
  acctExportJournalLabel: "دفتر اليومية العام",
  acctExportJournalDesc: "صف واحد لكل قيد دفتري ضمن النطاق: التاريخ، المرجع، كود الحساب، مدين، دائن، العميل، البيان.",
  acctExportInvoicesLabel: "الفواتير",
  acctExportInvoicesDesc: "كل فاتورة صدرت ضمن النطاق مع المجموع الفرعي وضريبة القيمة المضافة والمبلغ الإجمالي والمدفوع والرصيد والرقم الضريبي للعميل عند التوفر.",
  acctExportPaymentsLabel: "المدفوعات",
  acctExportPaymentsDesc: "المدفوعات والدفعات المقدّمة ضمن النطاق. الدفعات المقدّمة تُظهر الفاتورة التي سُوِّيت لاحقًا مقابلها.",
  acctExportCustomersLabel: "العملاء (ضمن النطاق)",
  acctExportCustomersDesc: "فقط العملاء الذين لديهم فواتير ضمن النطاق. ليست قائمة العملاء الكاملة — للحدّ من البيانات الشخصية التي تخرج من النظام.",
  ledgerTitle: "سجل الحسابات",
  ledgerSubtitle: "سجل القيد المزدوج الكامل مع تفاصيل كل دفعة. للقراءة فقط — يكتب النظام القيود ولا تُدخل يدويًا.",
  ledgerFrom: "من",
  ledgerTo: "إلى",
  ledgerApply: "تطبيق",
  ledgerReset: "إعادة تعيين",
  ledgerByAccount: "حسب الحساب",
  ledgerAccount: "الحساب",
  ledgerDebit: "مدين",
  ledgerCredit: "دائن",
  ledgerNet: "الصافي",
  ledgerTrialBalance: "ميزان المراجعة",
  ledgerPayments: "الدفعات الفردية",
  ledgerPaymentsEmpty: "لا توجد دفعات في هذا النطاق الزمني.",
  acctCashBank: "النقد/البنك",
  acctAR: "الذمم المدينة",
  acctVatPayable: "ضريبة القيمة المضافة المستحقة",
  acctSales: "إيرادات المبيعات",
  acctDeposits: "إيداعات العملاء",
  ledgerAdvanceChip: "دفعة مقدمة",
  ledgerMigratedChip: "مُرحَّلة",
  tabAnalytics: "التحليلات",
  analyticsTitle: "التحليلات",
  analyticsSubtitle: "الاتجاهات اليومية — الإيرادات، الضريبة، المهام، متوسط الفاتورة. اختر النطاق أعلاه.",
  analyticsRevenue: "الإيرادات",
  analyticsVat: "الضريبة المحصلة",
  analyticsJobs: "المهام المنشأة",
  analyticsAvgTicket: "متوسط الفاتورة",
  analyticsRevenuePerDay: "الإيرادات يوميًا",
  analyticsVatPerDay: "الضريبة يوميًا",
  analyticsJobsPerDay: "المهام يوميًا",
  analyticsAvgTicketPerDay: "متوسط الفاتورة يوميًا",
  analyticsWindowSelector: "النطاق الزمني",
  periodProfitTitle: "الربح — آخر {days} يومًا",
  periodProfitRevenue: "الإيرادات",
  periodProfitProfit: "الربح",
  periodProfitCoverage: "التغطية",
  periodProfitCoverageOf: "{covered} من {total} فواتير",
  periodProfitFootnote:
    "تُحسب الهوامش بمتوسط تكلفة القطع المرجّح، وليس بتكلفة كل وحدة على حدة. الفواتير التي تنقصها بيانات تكلفة تُستبعد من الربح لكنها تُحتسب ضمن التغطية.",
  partProfitTitle: "ربح لكل قطعة",
  partProfitSubtitle:
    "أي القطع تحقق ربحًا، وأيها تُباع بسعر التكلفة أو أقل. تنطبق قاعدة التغطية نفسها.",
  partProfitLinkFromAnalytics: "تقرير الربح لكل قطعة →",
  partProfitBackToAnalytics: "العودة إلى التحليلات",
  partProfitEmpty:
    "لا توجد بنود قطع في فواتير هذه النافذة. وسِّع النطاق لعرض السجل التاريخي.",
  partProfitEarners: "المُربحة",
  partProfitEarnersSubtitle: "القطع ذات الربح الموجب، الأكبر أولًا.",
  partProfitEarnersEmpty: "لا توجد قطع محققة للربح في هذه النافذة.",
  partProfitLosers: "عند التعادل أو خاسرة",
  partProfitLosersSubtitle:
    "قطع تُباع بسعر التكلفة المرجّحة أو أقل. الأكبر خسارة أولًا.",
  partProfitLosersEmpty:
    "لا توجد قطع بيعت عند التكلفة أو أقل في هذه النافذة.",
  partProfitUnknowns: "التغطية ناقصة",
  partProfitUnknownsSubtitle:
    "يوجد بند فاتورة واحد على الأقل لهذه القطعة تنقصه بيانات التكلفة. لا يمكن حساب الربح والهامش حتى تكتمل تكاليف كل البنود.",
  marginChipLabel: "هامش",
  min: "دقيقة",
  sampleUpDown: "هل نحن في ارتفاع أم انخفاض هذا الأسبوع؟",
  sampleProfit: "كم الربح هذا الشهر؟",
  sampleOwes: "من لديه مستحقات علينا؟",
  ansProfit: "ربح هذا الشهر حتى الآن هو {v} (الإيرادات ناقص تكلفة القطع).",
  ansProfitIncomplete: "لا يمكن إعطاء رقم للربح بعد — {missing} من أصل {total} فاتورة هذا الشهر تفتقد لتكلفة القطع. الإيرادات حتى الآن {revenue}.",
  ansNobodyOwes: "لا أحد مدين لك حاليًا — كل الفواتير مدفوعة. 🟢",
  ansOwes: "لدى {n} عميل/عملاء مستحقات بإجمالي {total} — {list}.",
  ansTrend: "أنت {dir} هذا الأسبوع: {a} مقابل {b} الأسبوع الماضي ({d}).",
  ansInvoiceSummary:
    "هذا الشهر: {n} فاتورة بإجمالي {total} ({paid} مدفوعة، {sent} مرسلة، {draft} مسودة، {voided} ملغاة).",
  ansVatMonth: "الضريبة المحصلة هذا الشهر: {v}.",
  ansAdvancesNone: "لا توجد دفعات مقدمة معلقة — كل دفعة طُبقت على فاتورة.",
  ansAdvancesOpen: "{n} دفعة مقدمة معلقة بإجمالي {total} — {list}.",
  ansTechRanking: "أفضل الفنيين: {list}.",
  ansTechRankingNone: "لا يوجد نشاط للفنيين بعد.",
  ansAdvisorRanking: "أفضل المستشارين: {list}.",
  ansAdvisorRankingNone: "لا يوجد نشاط للمستشارين بعد.",
  ansLedgerBalance: "النقد المتوفر: {cash} · الذمم المدينة: {ar}.",
  dirUp: "في ارتفاع",
  dirDown: "في انخفاض",
  dirFlat: "ثابت",
  copilotUnknown: "يمكنني الإجابة عن ربح هذا الشهر، أو من لديه مستحقات علينا، أو إن كنا في ارتفاع أم انخفاض هذا الأسبوع.",
};

export const messages = { en, ar } as const;

export type MessageKey = keyof typeof en;

/** Maps a JobStatus value to its translation key, e.g. "ARRIVED" -> "st_ARRIVED". */
export function statusKey(status: string): MessageKey {
  return ("st_" + status) as MessageKey;
}

/** Maps a PartRequestStatus to its translation key, e.g. "ORDERED" -> "pr_ORDERED". */
export function partStatusKey(status: string): MessageKey {
  return ("pr_" + status) as MessageKey;
}

/** Maps a ReminderType to its translation key, e.g. "BATTERY" -> "rt_BATTERY". */
export function reminderTypeKey(type: string): MessageKey {
  return ("rt_" + type) as MessageKey;
}

/** Maps a ReminderStatus to its translation key, e.g. "SENT" -> "rs_SENT". */
export function reminderStatusKey(status: string): MessageKey {
  return ("rs_" + status) as MessageKey;
}
