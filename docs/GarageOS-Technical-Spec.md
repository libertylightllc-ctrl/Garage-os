...
- **Customer.phone is stored E.164, flagged when it can't be.** Every write path (public booking, advisor intake, advisor customer-edit, WhatsApp webhook) routes through `normalizeCustomerPhoneForWrite` — resolves to E.164 (`971501234567`) when the input is dialable, stores the raw input with `phoneNeedsReview=true` when it isn't. The send path degrades to the WhatsApp contact picker on unresolvable phones; the flag surfaces the reason on the customer detail page so an advisor can fix it. **Merging duplicate customers is a separate decision, not part of phone-normalisation work.** The `@@unique([garageId, phone])` constraint means any future backfill from legacy 9-digit shape to E.164 will hit unique collisions where the same person exists twice — those are two rows to merge (customer identity call, not a data migration), not a rewrite to solve. AR 2026-08-23.
